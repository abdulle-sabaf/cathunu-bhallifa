...
\begin{xt}
    ...
    \begin{align*}
        1).& \int_a^{1} \frac{\ln{x}}{x} , \; a\to 0^+  &2).& \int_1^{a} \mathrm{e}^{-x},  \; a\to +\infty \\[3pt]
        3).& \int_0^{a} \frac{1}{\sqrt{1 - x^2}}, \; a\to 1^-   & 4).& \int_a^{1} \sin{\frac{1}{x}}, \; a\to 0^+ 
    \end{align*}
    \indent 2. 计算以下广义积合。\\    
    \begin{align*}
        1).& \int_0^{+\infty} \frac{1}{x\ln^2{x}} ,  &2).& \int_0^{+\infty} x\mathrm{e}^{-x^2} \\[3pt]
        3).& \int_0^{+\infty} \frac{1}{(1 + \mathrm{e}^{x})^2},  & 4).& \int_0^{+\infty} \frac{\mathrm{e}^{-\sqrt{x}}}{\sqrt{x}}
    \end{align*}
...
\begin{xt}
    ...
    \begin{align*}
        1)\;\;& \frac{1}{x\ln^2{x}} ,  &2)\;\;& \frac{\mathrm{e}^{-x^2}}{\sqrt{x}}, \\[3pt]
        3)\;\;& \sin{\frac{1}{x}},  & 4)\;\;& \frac{1}{\mathrm{e}^x  - 1}.
    \end{align*}
    \indent 2. 以下积合是否存在？\\    
    \begin{align*}
        1)\;\;& \int_2^{+\infty} \frac{1}{\sqrt{x(x - 1)(x - 2)}},  &2)\;\;& \int_0^{\frac{\pi}{2}} \frac{1}{\sin{x}\cos{x}(1 - \mathrm{e}^{-x})}, \\[3pt]
    ...
    \begin{align*}
        1)\;\;& \frac{\ln{x}}{x^a} ,  &2)\;\;& \frac{\mathrm{e}^{-x} - 1}{x^a}, \\[3pt]
    ...
    \indent 6.2. 证明：$\mathrm{e}^{-t\sin{t}}$在$\mathbb{R}^+$上不可积。
...
让我们考虑函数
$$f_0: t\mapsto U\mathrm{e}^{-\frac{t}{RC}}$$
$f_0(0) = U$，而它的微变函数是
$$\partial f_0(t) = -\frac{U}{RC} \mathrm{e}^{-\frac{t}{RC}}. $$
因此，对任何$t\geqslant 0$，
...
从前面的例子可以知道，$t\mapsto \mathrm{e}^{-at}$是它的解。

由于我们没有规定$f$的初始值，所以对任何常数$k$，$t\mapsto k\mathrm{e}^{-at}$都是它的解。此外，如果$f_1$和$f_2$都是解，
那么：
...
$S$关于函数的加法和数乘，构成平直空间。我们称$S$是方程的\textbf{解空间}。这样说来，如果把$f_0:t\mapsto \mathrm{e}^{-at}$看作向量，
...
$$S = \left\{\left.t\mapsto k\mathrm{e}^{-at} \, \right| \, k\in\mathbb{R}\right\}.$$
...
方程\eqref{2-2b}的解，形式为$t\mapsto k\mathrm{e}^{-\int_0^t a}$。
我们猜测，方程\eqref{2-2c}的解和它“长得很像”，特别是关于$a$的部分“$\mathrm{e}^{-\int_0^t a}$”。
我们猜测，方程\eqref{2-2c}的解形如$t\mapsto k(t) g(t)$，其中$g: t\mapsto \mathrm{e}^{-\int_0^t a}$是方程\eqref{2-2b}的解。
...
因此，
$$ \partial k(t) = \frac{b(t)}{g(t)} = b(t) \mathrm{e}^{\int_0^t a}. $$
于是通过求积合可以得出函数$k$：
$$ k(t) = \int_0^t b(u) \mathrm{e}^{\int_0^u a(v)}\mathrm{d}v \mathrm{d}u. $$
代入$f$，得到$f$的表达式：
$$ f(t) = \mathrm{e}^{-\int_0^t a} \int_0^t b(u) \mathrm{e}^{\int_0^u a(v)}\mathrm{d}v \mathrm{d}u. $$
$f$就是方程\eqref{2-2c}的一个特解。

综上所述，方程\eqref{2-2c}的解集可以写成维直空间：
$$\left\{\left.\mathrm{e}^{-\int_0^t a(u)\mathrm{d}u} \left(k + \int_0^t b(u) \mathrm{e}^{\int_0^u a(v)}\mathrm{d}v \mathrm{d}u\right) \, \right| \, k\in \mathbb{R}\right\}$$
...
\begin{so}
    ...
    解这个微变方程，得到特定解：
    \begin{align*}
        v(t) &= \mathrm{e}^{-bt} \int_0^t mg \mathrm{e}^{\int_0^u bs\mathrm{d}s} \mathrm{d}u\\
        &= mg \cdot \mathrm{e}^{-bt} \int_0^t \mathrm{e}^{bu}\mathrm{d}u \\
        &= mg \cdot \mathrm{e}^{-bt} \frac{\mathrm{e}^bt - 1}{b} \\
        &= \frac{mg}{b}\left(1 - \mathrm{e}^{-bt}\right).
    \end{align*}
    于是一般解为
    $$ v(t) = \frac{mg}{b}\left(1 - \mathrm{e}^{-bt}\right) + k\mathrm{e}^{-bt}. $$
    其中$k$为常数。
    
    雨滴初速度$v(0) = v_0$，因此解得$k = v_0$，于是得到雨滴速度关于时间的函数：
    $$ v(t) = \frac{mg}{b}\left(1 - \mathrm{e}^{-bt}\right) + v_0\mathrm{e}^{-bt}. $$
    积合可得雨滴位移函数$x(t)$：
    $$ x(t) = \frac{mg}{b}\left(t + \frac{\mathrm{e}^{-bt} - 1}{b}\right) - \frac{v_0\left(\mathrm{e}^{-bt} - 1\right)}{b}. $$
    雨滴从高处下落，在$x=2000$米处到达地面，代入各个物理量数值，到达地面用时满足方程：
    $$ 3.267t + 10222 \left(\mathrm{e}^{-0.0003t} - 1\right) = 2000.  $$
    解得
    ...
    可得：
    $$ p(z) = P_0\mathrm{e}^{-\frac{Mg}{RT}z}.$$

    \indent 2.2. 解方程
    $$ \partial p(z) = -\frac{Mg}{RT_0(1 - az)} p. $$
    可得：
    \begin{align*}
        p(z) &= P_0\mathrm{e}^{\int_0^z -\frac{Mg}{RT_0(1 - ax)}\mathrm{d}x} \\
        &= P_0\mathrm{e}^{\frac{Mg}{RT_0}\int_0^z \frac{1}{ax - 1}\mathrm{d}x} \\
        &= P_0\mathrm{e}^{\frac{Mg}{RT_0}\frac{1}{a}\left(\ln{|az - 1|} - \ln{1}\right)} \\
        &= P_0\mathrm{e}^{\frac{Mg\ln{1 - az}}{aRT_0}} \\
    ...
    可得：
    \begin{align*}
        p(z) &= P_0\mathrm{e}^{\int_0^z -\frac{Mg_0r_T^2}{RT(r_T + x)^2}\mathrm{d}x} \\
        &= P_0\mathrm{e}^{\frac{Mg_0r_T^2}{RT}\int_0^z -\frac{1}{(r_T + x)^2}\mathrm{d}x} \\
        &= P_0\mathrm{e}^{\frac{Mg_0r_T^2}{RT}\left(\frac{1}{z + r_T} - \frac{1}{r_T}\right)} \\
        &= P_0\mathrm{e}^{-\frac{Mg_0r_Tz}{RT(z + r_T)}}.
    \end{align*}
    大气压$p$关于海拔高度$z$的函数为：
    $$ p(z) = P_0 \mathrm{e}^{-\frac{Mg_0r_Tz}{RT(z + r_T)}} . $$
...
\begin{xt}
    ...
    \begin{array}[]{l}
        ...
        2).\;\forall t \geqslant 0,\,\,\,\partial f(t) - \frac{1}{t + 1} f(t) = \mathrm{e}^{-t}. \\
        3).\;\forall t\in \mathbb{R},\,\,\,\partial f(t) + 2t f(t) =\mathrm{e}^{t - t^2}, \; f(0) = 1. 
    \end{array}
    $$
    \indent 3. 人空腹喝酒之后，体内可检测的酒精浓度随时间$t$（单位：小时）变化，可用函数$g(t)$（单位：克每升）表示。$g(t)$满足以下方程：
    $$ \forall t\geqslant 0,\,\,\,\partial g(t) + g(t) = a \mathrm{e}^{-t}.$$
    其中常数$a$表示人体代谢酒精的能力。\\
...
$$ f_1:\,\,\, t \mapsto \mathrm{e}^{r_1t}, \quad f_2:\,\,\, t \mapsto \mathrm{e}^{r_2t}, $$
它们作为基底生成平面：
$$ \left\{\left.t \mapsto A\mathrm{e}^{r_1t} + B\mathrm{e}^{r_2t}\,\right|\, A, B \in \mathbb{R}\right\}$$

如果特征式$x^2 + ax + b$有二重根$r = -\frac{a}{2}$，那么我们找到了方程的一个解：
$$ f_1:\,\,\, t \mapsto \mathrm{e}^{rt}. $$
...
$$ \left\{\left.t \mapsto (At + B)\mathrm{e}^{rt}\,\right|\, A, B \in \mathbb{R}\right\}$$
...
的解。它可以写成：
$$ \partial h (t) = k \mathrm{e}^{-\int_0^t \frac{2 \partial e_1(u) + a}{e_1(u)} \mathrm{d}u}. $$
于是
$$0 = \partial h (0) = k \mathrm{e}^0 = k.$$
...
\begin{so}
    ...
    若二次式有根，根为负数。方程的解为：
    $$ t \mapsto A\mathrm{e}^{r_1t} + B\mathrm{e}^{r_2t} $$
    其中$r_1<0$、$r_2<0$为特征式的相异两根，或
    $$ t \mapsto (At + B)\mathrm{e}^{rt} $$
    其中$r = -\frac{R}{2L} < 0$为特征式的二重根。

    考虑初始情况，经计算，可以求出$A$、$B$的值。

    特征式有相异根的情况下，电势差为：
    $$ u(t) = \frac{E}{r_2 - r_1}\left(r_2\mathrm{e}^{r_1t} - r_1\mathrm{e}^{r_2t}\right) $$ 

    特征式有二重根的情况下，电势差为：
    $$ u(t) =  E(1 - rt)\mathrm{e}^{rt}$$
    ...
    我们猜测方程的解为：
    $$ u: \,\,\,t \mapsto \left(A\sin{(\omega t)} + B\cos{(\omega t)}\right)\mathrm{e}^{rt} $$
    ...
    可以算出
    \begin{align*}
        \partial u(t) &= ru(t) + \omega \left(A\cos{(\omega t)} - B\sin{(\omega t)}\right)\mathrm{e}^{rt} \\
        &= ru(t) + \omega v(t),
    \end{align*}

    其中$v(t)$的微变为：
    \begin{align*}
        \partial v(t) &= rv(t) + \omega^2 \left(- A\sin{(\omega t)} - B\cos{(\omega t)}\right)\mathrm{e}^{rt} \\
        &= rv(t) - \omega^2 u(t),
    \end{align*}
    ...
    $$ u(t) =  E\left(\cos{(\omega t)} - \frac{r}{\omega}\sin{(\omega t)}\right)\mathrm{e}^{rt}. $$

    可以看出$u$是某个周期函数和指数递减函数$t\mapsto \mathrm{e}^{rt}$的乘积。我们称这种情况为振荡衰减情况。
...
\begin{xt}
    ...
    $$
    \indent 3. 写出以函数空间
    $$ \left\{\left.t \mapsto A\mathrm{e}^{-2t} + B\mathrm{e}^{3t}\,\right|\, A, B \in \mathbb{R}\right\} $$
    为解空间的微变方程。\\
...
就得到：
$$ Ax^2 + Cy^2 + F' = 0.$$
其中$F' = F - \frac{D^2}{4A} - \frac{\mathrm{e}^2}{4C}$。两边除以$AC$并移项，得到：
...
如果$D\neq 0$，按$\displaystyle\left(\frac{\mathrm{e}^2 - 4CF}{4CD},\,\,-\frac{E}{2C}\right)$平移，就得到：
$$ Cy^2 = -Dx. $$
即
$$ y^2 = -\frac{D}{C}x. $$
因此曲线为抛物线。$C=0$而$A\neq 0$的情形也类同。
...
$$ t\mapsto u(t) + \imath v(t)$$
满足我们对$t\mapsto \mathrm{e}^{\imath t}$函数性质的某些期望。

比如，参考函数$t\mapsto \mathrm{e}^{at}$的性质，我们希望$f: t\mapsto \mathrm{e}^{\imath t}$满足：
\begin{enumerate}
    \item $f(0) = 1$；
...
又比如，由于对实数来说，$a^t = \mathrm{e}^{t\ln{a}}$，所以我们希望有：
$$ \imath^t = \mathrm{e}^{t\ln{\imath}} $$
因为$\sin^2{t} + \cos^2{t} = 1$，我们希望有：
$$ \sin^2{(\imath t)} + \cos^2{(\imath t)} = 1 $$

回到$t\mapsto \mathrm{e}^{\imath t}$，是否有这样的复值函数呢？来看上面提到的复值函数：
\begin{align*}
...
$$ \forall \, t \; \in \mathbb{R}, \quad  \mathrm{e}^{\imath t} = \cos{t} + \imath \sin{t}, $$
那么它满足前面我们希望它满足的所有性质。

更一般地来说，给定复数$z = a + b\imath$，那么可以定义：
$$ \forall \, t \; \in \mathbb{R}, \quad  \mathrm{e}^{z t} = \mathrm{e}^{at + b\imath t} = \mathrm{e}^{at} \left( \cos{bt} + \imath \sin{bt} \right), $$

可以看到，复值的指数函数和实数指数函数有很大的不同。直观上来说，$t\mapsto \mathrm{e}^{t}$是单调递增的，而$t\mapsto \mathrm{e}^{\imath t}$
是一个周期函数，它的轨迹在复平面上是单位圆。随着$t$的变化，函数绕着单位圆不断“转圈”。这是两种完全不同的行为。

比如，我们可以从上面推出：
$$ \mathrm{e}^{\frac{\imath\pi}{2}} = \imath, \quad \mathrm{e}^{\imath\pi} = -1, \quad \mathrm{e}^{\frac{3\imath\pi}{2}} = -\imath, \quad \mathrm{e}^{2\imath\pi} = 1, $$

再来看复数的对数。$\mathrm{e}^{\frac{\imath\pi}{2}} = \imath$启发我们，可以定义$\ln{\imath} = \frac{\imath\pi}{2}$。
这样定义似乎没什么问题，但我们发现：
$$ \mathrm{e}^{\frac{5\imath\pi}{2}} = \mathrm{e}^{\frac{\imath\pi}{2}} \times \mathrm{e}^{2\imath\pi} = \imath \times 1 = \imath. $$
于是又可以定义$\ln{\imath} = \frac{\imath\pi}{2}$。一般来说，考虑复数$z$的“极坐标”表示：
$$ z = |z| \mathrm{e}^{\imath \theta}$$
...
\begin{et}
    设$t$为实数，给出与复值指数函数兼容的函数：$f:\;t\mapsto \sin{\imath t}$、$g:\;t\mapsto \cos{\imath t}$。
\end{et}

\begin{so}
    根据复值指数函数的形式：
    $$ \mathrm{e}^{\imath t} = \cos{t} + \imath \sin{t}, $$
    有：
    $$ \sin{t} = \frac{\mathrm{e}^{\imath t} - \mathrm{e}^{-\imath t}}{2\imath}.$$
    可以设想令
    $$ f(t) = \sin{\imath t} = \frac{\mathrm{e}^{\imath (\imath t)} - \mathrm{e}^{-\imath (\imath t)}}{2\imath} = \imath \frac{\mathrm{e}^{t} - \mathrm{e}^{-t}}{2}.$$
    同理，由于
    $$ \cos{t} = \frac{\mathrm{e}^{\imath t} + \mathrm{e}^{-\imath t}}{2},$$
    可以设想令
    $$ g(t) = \cos{\imath t} = \frac{\mathrm{e}^{\imath (\imath t)} + \mathrm{e}^{-\imath (\imath t)}}{2} = \frac{\mathrm{e}^{t} + \mathrm{e}^{-t}}{2}.$$
    可以验证：
    $$ f(0) = \sin{(\imath \cdot 0)} = 0, \quad g(0)=\cos{(\imath \cdot 0)} = 1,$$
    函数$f$、$g$都是光滑函数，且：
    \begin{align*}
        \partial f(t) &= \imath \frac{\mathrm{e}^{t} + \mathrm{e}^{-t}}{2} = \imath g(t) = (\partial\sin)(\imath t)\cdot (\partial(\imath t)), \\
        \partial g(t) &= \frac{\mathrm{e}^{t} - \mathrm{e}^{-t}}{2} = -\imath f(t) = (\partial\cos)(\imath t)\cdot (\partial(\imath t)).
    \end{align*}
    最后，它们满足：
    \begin{align*}
        \forall \; t\in \mathbb{R}, \quad & \sin^2{(\imath t)} + \cos^2{(\imath t)} = \left(\imath \frac{\mathrm{e}^{t} - \mathrm{e}^{-t}}{2}\right)^2 + \left( \frac{\mathrm{e}^{t} + \mathrm{e}^{-t}}{2}\right)^2 \\
        =\;& -\frac{\mathrm{e}^{2t} + \mathrm{e}^{-2t} - 2}{4} + \frac{\mathrm{e}^{2t} + \mathrm{e}^{-2t} + 2}{4} \\
        =\;& 1.
    \end{align*}
...
\begin{sk}
    \mbox{} \\
    \indent 1. 为什么$\mathrm{e}^{b\imath t} = \cos{bt} + \imath \sin{bt}$而不是$\cos{t} + \imath b\sin{t}$？\\
    \indent 2. $\mathrm{e}^x$是级数$\sum_{n=0}^{\infty} \frac{x^n}{n!}$的和。这个关系在$x$为复数的时候是否成立？\\
