...
\begin{ex}
    ...
    $$ f_n: x \mapsto x^n. $$
    对$[0,1)$中任意$x$，由于$0\leqslant x<1$，$x^n$随着$n$增大趋于$0$。
    因此，$\{f_n\}_{n\in\mathbb{N}}$逐点收敛到$[0,1)$上的零函数。

    不过，对任意正整数$n$，取$x_n = 1 - \frac{1}{n}$，则
    $$f_n(x_n) = \left(1 - \frac{1}{n}\right)^n.$$
    我们知道数列$\left\{\left(1 - \frac{1}{n}\right)^n\right\}$趋于$\frac{1}{e}$，
    因此$n$足够大时，总有$f_n(x_n) > \frac{1}{2e} > 0$。这说明函数列$\{f_n\}_{n\in\mathbb{N}}$
    并不一致收敛到零函数。
    
    直观来看，$f_n$的图像在靠近$1$时就会“翘尾巴”，即便$n$越大时，“尾巴”也越来越陡，但$x$趋于$1$时总收敛于$1$。
...
如果函数列一致收敛的话，我们可以证明，连续函数的函数列，极限也是连续函数。
