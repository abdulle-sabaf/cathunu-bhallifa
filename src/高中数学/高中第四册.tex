...
按照定义，幂级数是多项式函数的逐点收敛的极限。那么，是否可以将它看作多项式函数的一致收敛的极限呢？

\begin{tm}
    设幂级数$f: x\mapsto \sum_{n\in\mathbb{N}} a_n x^n$的收敛半径为$R$，则$f$在区间$(-R;R)$上为连续函数。
\end{tm}
...
举例来说，正弦函数$x\mapsto \sin{x}$在局部（$x=0$处）可以写成：
$$ \sin{x} \approx x - \frac{x^3}{6} + \frac{x^5}{120} + \cdots + \frac{x^{2n+1}}{(2n+1)!}  $$
其差异为关于$x^{2n+1}$的高阶无穷小。也就是说，我们可以大致认为，在$0$附近有：
$$ \sin{x} = \sum_{n=0}^{\infty} \frac{x^{2n+1}}{(2n+1)!}.  $$
这是否成立？对于任意$x$是否也成立？

我们把能够在一定区间内展开为幂级数的函数称为\textbf{可展函数}。
经典函数中的指数函数、对数函数、正弦函数、余弦函数都可以在其定义域上展开为幂级数，因此都是可展函数。
...
\begin{tm}{\textbf{一致连续下的微变}}
    ...
    已知函数列$\{f_n\}_{n\in\mathbb{N}}$中每一项都在区间$I$上可微，且微变函数连续。
    设函数列$\{f_n\}_{n\in\mathbb{N}}$逐点收敛到函数$f$，且函数列$\{\partial f_n\}_{n\in\mathbb{N}}$在$I$的任何闭子区间上一致收敛到函数$g$，
...
\begin{proof}
    ...
    令$B\subseteq I$是包含$c$和$x$的闭子区间，$\partial f_n$是$B$上的连续函数，因此根据一致连续下的积合定理：
    \begin{align*}
