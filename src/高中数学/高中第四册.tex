...
% Figure: 广义可积函数
\definecolor{color_pOne_line}{RGB}{108, 41, 51}
\definecolor{color_pOne_fill}{RGB}{201, 221, 34}
\definecolor{color_pTwo_line}{RGB}{175, 39, 43}
\definecolor{color_pTwo_fill}{RGB}{255, 241, 67}
\definecolor{color_pThree_line}{RGB}{200, 60, 35}
\definecolor{color_pThree_fill}{RGB}{255, 137, 54}
\definecolor{color_pFour_line}{RGB}{255, 78, 32}
\definecolor{color_pFour_fill}{RGB}{255, 33, 33}

\definecolor{color_aOne_line}{RGB}{10, 163, 68}
\definecolor{color_aOne_fill}{RGB}{80, 252, 160}
\definecolor{color_aTwo_line}{RGB}{76, 141, 174}
\definecolor{color_aTwo_fill}{RGB}{123, 207, 166}
\definecolor{color_aThree_line}{RGB}{75, 92, 196}
\definecolor{color_aThree_fill}{RGB}{68, 206, 246}
...
\begin{figure}[h] %this figure will be at the right
    ...
    \begin{scope}[x=5cm,y=0.5cm,shift={(0,0)}]
        \def\xmin{-1.05};
        \def\xmax{1.2};
        \def\ymin{-9.6};
        \def\ymax{9.6};
        ...
        \def\xm{0.2};
        \def\ym{2};
        \foreach \k in {-5,-4,...,-1,1,2,...,5} {  % 平行x轴网格
            \draw[help lines,gray!60] (\xm*\k,\ymin) -- (\xm*\k,\ymax) ;
        }
        \foreach \k in {-4,-3,...,-1,1,2,...,4} {  % 平行y轴网格
            \draw[help lines,gray!60] (\xmin,\ym*\k) -- (\xmax,\ym*\k);
        }
        \draw[-Stealth, thick] (\xmin, 0) -- (\xmax,0) node[above] {$x$};
        \draw[-Stealth, thick] (0,\ymin) -- (0,\ymax) node[left] {$y$};
        
        \def\ca{0.5};
        \def\cb{0.2};
        \def\cc{0.12};
        \foreach \x in {0.2,0.4,0.6,0.8,1} {  % x轴正向
            \draw (\x,\ym*\cc) -- (\x,-\ym*\cc); 
            \node[black] at (\x, -\ym*\ca) {$\mbox{\tiny $\x$}$};
        }
        \foreach \x in {0.2,0.4,0.6,0.8,1} {  % x轴负向
            \draw (-\x,\ym*\cc) -- (-\x,-\ym*\cc); 
            \node[black] at (-\x-\xm*\cb, -\ym*\ca) {$\mbox{\tiny $-\x$}$};
        }
        \foreach \y in {2,4,6,8} {  % 轴正向
            \draw (\xm*\cc,\y) -- (-\xm*\cc,\y);
            \node[black] at (-\xm*\ca, \y) {$\mbox{\tiny $\y$}$};
        }
        \foreach \y in {2,4,6,8} {
            \draw (\xm*\cc,-\y) -- (-\xm*\cc,-\y);
            \node[black] at (-\xm*\ca-\xm*\cb, -\y) {$\mbox{\tiny $-\y$}$};
        }
        % 作图
        \def\a{0.1042};
        \def\b{1};
        \fill[BrickRed, opacity=0.2, domain=\a:\b, samples=200]
        (\a,0)
        -- plot (\x,{1/\x})
        -- (\b,0)
        -- cycle;
        \fill[BrickRed, opacity=0.2] (0,0) rectangle (\a,1/\a);
        \draw[BrickRed, very thick, domain=\a:\b, samples=200]
        plot (\x,{1/\x});
        \draw[BrickRed, very thick, domain=-\a:-\b, samples=200]
        plot (\x,{1/\x});
        \draw[very thick, fill=white] (0.32,-8) rectangle (1,-4.2);
        \node at (0.66,-6.1) {$\mbox{\footnotesize $\displaystyle x\mapsto \frac{1}{x}$}$};
    \end{scope}
    \begin{scope}[x=1cm,y=4cm,shift={(10,-0.25)}]
        \def\xmin{-2.3};
        \def\xmax{8.3};
        \def\ymin{-0.8};
        \def\ymax{1.4};
        ...
        \def\xm{1};
        \def\ym{0.25};
        \foreach \k in {-2,-1,1,2,...,8} {  % 平行x轴网格
            \draw[help lines,gray!60] (\xm*\k,\ymin) -- (\xm*\k,\ymax) ;
        }
        \foreach \k in {-3,-2,-1,1,2,...,5} {  % 平行y轴网格
            \draw[help lines,gray!60] (\xmin,\ym*\k) -- (\xmax,\ym*\k);
        }
        \draw[-Stealth, thick] (\xmin, 0) -- (\xmax,0) node[above] {$x$};
        ...
        \def\cb{0.2};
        \def\cc{0.12};
        \foreach \x in {1,2,...,8} {  % x轴正向
            \draw (\x,\ym*\cc) -- (\x,-\ym*\cc); 
            \node[black] at (\x, -\ym*\ca) {$\mbox{\tiny $\x$}$};
        }
        \foreach \x in {1,2} {  % x轴负向
            \draw (-\x,\ym*\cc) -- (-\x,-\ym*\cc); 
            \node[black] at (-\x-\xm*\cb, -\ym*\ca) {$\mbox{\tiny $-\x$}$};
        }
        \foreach \y in {0.5,1} {  % y轴正向
            \draw (\xm*\cc,\y) -- (-\xm*\cc,\y);
            \node[black] at (-\xm*\ca, \y) {$\mbox{\tiny $\y$}$};
        }
        \foreach \y in {0.5} {
        ...
        -- (\b,0)
        -- cycle;
        \draw[BrickRed, thick, domain=\a:\b, samples=200]
        plot (\x,{1/(\x*\x)});
        \draw[very thick, fill=white] (4.9,0.8) rectangle (8.1,1.3);
        \node at (6.5,1.05) {$\mbox{\footnotesize $\displaystyle x\mapsto \frac{1}{x^2}$}$};
    \end{scope}
    \end{tikzpicture}
    % \caption*{\texttt{左}$x\mapsto\frac{1}{x}$（左）、$x\mapsto\frac{1}{x^2}$（右）}
\end{figure}
...
\begin{figure}[h] %figure a2
    \vspace{4pt}
    \centering
    % \includegraphics[width=\textwidth]{tu/广义积分2.png}
    \begin{tikzpicture}
    % ========== 参数定义 ==========
    \def\pOne{0.5}   % p 值：最上层
    \def\pTwo{1}     % p 值
    \def\pThree{1.5} % p 值
    \def\pFour{2}    % p 值：最下层

    \def\aOne{0.6667}   % a 值：最上层
    \def\aTwo{0.5}   % a 值
    \def\aThree{0.3} % a 值：最下层
    \def\opa{0.2}
    \def\mw{5.824};
    \def\mh{4.2};
    % ========== 左图：x^{-p} 积分区域 ==========
    ...
    % 定义x轴路径
    \path[name path=xaxis] (axis cs:0,0) -- (axis cs:10.4,0);

    % 绘制曲线
    \addplot[
        name path=pOne, 
        draw=color_pOne_line, samples=200, domain=1:10.4
    ] {x^(-\pOne)};  % p = \pOne

    \addplot[
        name path=pTwo, 
        draw=color_pTwo_line, samples=200, domain=1:10.4
    ] {x^(-\pTwo)};   % p = \pTwo

    \addplot[
        name path=pThree, 
        draw=color_pThree_line, samples=200, domain=1:10.4
    ] {x^(-\pThree)}; % p = \pThree

    \addplot[
        name path=pFour, 
        draw=color_pFour_line, samples=200, domain=1:10.4
    ] {x^(-\pFour)};  % p = \pFour

    % 填充分层
    \addplot[color_pFour_fill, opacity=\opa] fill between[of=pFour and xaxis, soft clip={domain=1:10.4}];
    \addplot[color_pThree_fill, opacity=\opa] fill between[of=pThree and pFour, soft clip={domain=1:10.4}];
    \addplot[color_pTwo_fill, opacity=\opa] fill between[of=pTwo and pThree, soft clip={domain=1:10.4}];
    \addplot[color_pOne_fill, opacity=\opa] fill between[of=pOne and pTwo, soft clip={domain=1:10.4}];

    % 重绘曲线
    \addplot[color_pOne_line, samples=200, domain=1:10.4] {x^(-\pOne)};
    \addplot[color_pTwo_line,  samples=200, domain=1:10.4] {x^(-\pTwo)};
    \addplot[color_pThree_line,   samples=200, domain=1:10.4] {x^(-\pThree)};
    \addplot[color_pFour_line,    samples=200, domain=1:10.4] {x^(-\pFour)};

    % 标注
    \draw[very thick, fill=white] (7.6,0.7) rectangle (10.2,1.41);
    \node[color_pOne_line, above right] at (axis cs:7.8,1.2) {$\mbox{\scriptsize $p=\pOne$}$};
    \node[color_pTwo_line,  above right] at (axis cs:7.8,1.04) {$\mbox{\scriptsize $p=\pTwo$}$};
    \node[color_pThree_line,   above right] at (axis cs:7.8,0.88) {$\mbox{\scriptsize $p=\pThree$}$};
    \node[color_pFour_line,    above right] at (axis cs:7.8,0.72) {$\mbox{\scriptsize $p=\pFour$}$};
    ...
    \addplot[
        name path=aOne, 
        draw=color_aOne_line, samples=200, domain=0:10.4
    ] {\aOne^x};   % a = \aOne

    \addplot[
        name path=aTwo, 
        draw=color_aTwo_line, samples=200, domain=0:10.4
    ] {\aTwo^x};   % a = \aTwo

    \addplot[
        name path=aThree, 
        draw=color_aThree_line, samples=200, domain=0:10.4
    ] {\aThree^x}; % a = \aThree

    % 填充分层
    \addplot[color_aThree_fill, opacity=\opa] fill between[of=aThree and xaxis, soft clip={domain=0:10.4}];
    \addplot[color_aTwo_fill, opacity=\opa] fill between[of=aTwo and aThree, soft clip={domain=0:10.4}];
    \addplot[color_aOne_fill, opacity=\opa] fill between[of=aOne and aTwo, soft clip={domain=0:10.4}];

    % 重绘曲线
    \addplot[color_aOne_line,   samples=200, domain=0:10.4] {\aOne^x};
    \addplot[color_aTwo_line,  samples=200, domain=0:10.4] {\aTwo^x};
    \addplot[color_aThree_line, samples=200, domain=0:10.4] {\aThree^x};

    % 标注
    \draw[very thick, fill=white] (7.6,0.72) rectangle (10.2,1.41);
    \node[color_aOne_line,   above right] at (axis cs:7.8,1.08) {$\mbox{\scriptsize $\displaystyle a=\frac{2}{3}$}$};
    \node[color_aTwo_line,  above right] at (axis cs:7.8,0.91) {$\mbox{\scriptsize $a=\aTwo$}$};
    \node[color_aThree_line, above right] at (axis cs:7.8,0.75) {$\mbox{\scriptsize $a=\aThree$}$};
...
\begin{et}
    ...
    \indent 1. 证明无穷级数$\sum u_n$收敛。\\
    \indent 2. 证明广义积合 $\int_0^{+\infty} f(x) \sin{(x)} \di{x}$ 收敛。\\
    \indent 3. 若有$a>0$使得只要$x > a$，就有$\displaystyle f(x) > \frac{1}{x}$。证明$\int_0^{+\infty} f(x) \sin{(x)} \di{x}$不绝对可积。
\end{et}
...
\begin{sk}
    \mbox{} \\
    \indent 1. 以上估计$\gamma$的上界时，为什么可以让$a=0.2$？能否使用更小的$a$？ \\
\end{sk}

\begin{xt}
    \mbox{} \\
    \indent 1. 设$f$是定义在$\mathbb{R}$上的函数：
    $$ f: \; x\mapsto \frac{1}{1 + x^4 \sin^2{(x)}}. $$
    \indent 1.1. 设$\sum I_n$为关于$f$的无穷级数，通项为：
    ...
    \indent 1.2. 使用换元$u = 2k^2\pi x$，证明：
    $$ \forall n\in \mathbb{N}, \; \,I_n \leqslant \frac{1}{n^2}.  $$
    \indent 1.3. 证明$f$在$\mathbb{R}$上可积。\\
    \indent 2. 记调和级数的前$n$项部分和为$S_n$。数列$\{u_n\}_{n\in\mathbb{N}}$、$\{v_n\}_{n\in\mathbb{N}}$的通项为：
    ...
    \indent 2.2. 设数列$\{ r_n \}_{n\in\mathbb{N}}$的通项为：
    $$ \forall n\in\mathbb{Z}^+,\quad  r_n = \sum_{k=n}^{+\infty} \frac{1}{k^2}, $$
    给出$r_n$的等价无穷小。\\
    \indent 2.3. 证明：
    $$ S_n = \ln{n} + \gamma + \frac{1}{2n} + \olim{\frac{1}{n}}. $$
    \indent 3. 我们想求$\displaystyle \frac{\sin{x}}{x}$在$\mathbb{R}^+$上的积合$I$。为此，我们借助于积合序列：
    $$ \forall n\in\mathbb{N},\quad  I_n = \int_0^{\frac{\pi}{2}} \frac{\sin{((2n+1)x)}}{\sin{x}} \di{x}, \; J_n = \int_0^{\frac{\pi}{2}} \frac{\sin{((2n+1)x)}}{x} \di{x} .$$
    \indent 3.1. 证明：对任意正整数$n$，确实可以如上定义积合$I_n$和$J_n$。\\
    \indent 3.2. 证明：$\forall n \in\mathbb{N},\;\; I_n - I_{n+1} = 0$，并求出$I_n$的值。 \\
    \indent 3.3. 设$g: \;\left[0;\frac{\pi}{2}\right] \rightarrow \mathbb{R}$是连续可微的函数。通过换元积合法证明：
    $$ \lian{n\to\infty} \int_0^{\frac{\pi}{2}} g(t)\sin{((2n+1)t)} \di{t} = 0. $$
    \indent 3.4. 证明：函数$\displaystyle x\mapsto \frac{1}{x} - \frac{1}{\sin{x}}$通过在$x=0$处补充定义，可以变为在$\left[0;\frac{\pi}{2}\right]$上连续可微的函数。\\
    \indent 3.5. 证明：$n$趋于无穷时，$I_n$和$J_n$的差趋于$0$。\\
    \indent 3.6. 证明：
...
如右图，根据题目条件，可以将城墙看作圆形。记圆心为$O$，以$O$为原点，东西、南北向各作坐标轴，得到直角坐标系$xOy$。
城门就是坐标轴与圆形的交点。

\begin{wrapfigure}[10]{r}{0.4\textwidth} %this figure will be at the right
    \vspace{-30pt}
    \flushright
    ...
    \begin{scope}[shift={(0,0)}]
        ...
        \coordinate (A) at (0,8);
        \coordinate (B) at (-6,0);
        \coordinate (P) at (-3.84,2.88);
        \coordinate (C) at (0,\R);
        \coordinate (D) at (-\R,0);
        \draw[blue,thick] (O) circle (\R);
        \draw[orange,thick] (A) -- (B);
        \draw[gray!60!black,thick,dashed] (O) -- (P) node[midway, above] {$r$};
        \draw[red,thick] (A) -- (C) node[midway, right, black] {$160$};
        \draw[red,thick] (B) -- (D) node[midway, below, black] {$60$};
        \draw[fill=OliveGreen] (O) circle (3pt) node[above right] {$O$};
        \draw[fill=OliveGreen] (A) circle (3pt) node[above left] {$A$};
        \draw[fill=OliveGreen] (B) circle (3pt) node[above] {$B$};
        \draw[fill=OliveGreen] (C) circle (3pt);
        \draw[fill=OliveGreen] (D) circle (3pt);
        \draw[fill=OliveGreen] (P) circle (3pt) node[above left] {$P$};
        \coordinate (N) at (5,7);
        \def\r{1.2};
        \def\ang{30};
        \def\a{0.5}
        \draw[very thick,gray!50!black] ($(N) + (0,\r/3)$) circle (\r/2);
        \draw[fill=black] (N) -- ($(N) + (-\ang:\a*\r)$) -- ($(N) + (90:\r)$) --cycle;
        \draw[fill=white] (N) -- ($(N) + (\ang-180:\a*\r)$) --  ($(N) + (91:\r)$) --  ($(N) + (90:\r)$) -- cycle;
        \node at ($(N) + (0,\r+0.5)$) {\texttt{北}};
    \end{scope}
    \end{tikzpicture}
\end{wrapfigure}

设圆的半径为$r$，则北门、西门的坐标分别是$(0, \,\, r)$、$(-r, \,\, 0)$。
设甲乙恰能相望时，分别位于点$A:(0, \,\, r+160)$、$B:(-r-60, \,\, 0)$。
则按题目描述，$AB$与圆相切，设切点为$P$。$\triangle AOB$、$\triangle AOP$是直角三角形，且
...
\begin{figure}[h] 
    ...
    \begin{scope}[shift={(0,0)}]
        ...
        \def\ym{1};
        \foreach \k in {-5,-4,...,-1,1,2,...,5} {  % 平行x轴网格
            \draw[help lines,gray!30] (\xm*\k,\ymin) -- (\xm*\k,\ymax) ;
        }
        \foreach \k in {-4,-3,...,-1,1,2,...,5} {  % 平行y轴网格
            \draw[help lines,gray!30] (\xmin,\ym*\k) -- (\xmax,\ym*\k);
        }
        \draw[-Stealth, thick] (\xmin, 0) -- (\xmax,0) node[above] {$1$};
        \draw[-Stealth, thick] (0,\ymin) -- (0,\ymax) node[right] {$\imath$};
        
        \def\ca{0.3};
        \def\cb{0.12};
        \def\cc{0.1};
        \foreach \x in {1,2,...,5} {  % x轴正向
            \draw (\x,\ym*\cc) -- (\x,-\ym*\cc); 
            \node[black] at (\x, -\ym*\ca) {$\mbox{\scriptsize $\x$}$};
        }
        \foreach \x in {1,2,...,5} {  % x轴负向
            \draw (-\x,\ym*\cc) -- (-\x,-\ym*\cc); 
            \node[black] at (-\x-\xm*\cb, -\ym*\ca) {$\mbox{\scriptsize $-\x$}$};
        }
        \foreach \y in {1,2,...,5} {  % y轴正向
            \draw (\xm*\cc,\y) -- (-\xm*\cc,\y);
            \node[black] at (-\xm*\ca, \y) {$\mbox{\scriptsize $\y\imath$}$};
        }
        \foreach \y in {1,2,...,4} {  % y轴负向
            \draw (\xm*\cc,-\y) -- (-\xm*\cc,-\y);
            \node[black] at (-\xm*\ca-\xm*\cb, -\y) {$\mbox{\scriptsize $-\y\imath$}$};
        }
        % 作图
        \draw[-Stealth, thick, Blue!80] (0,0) -- (3,2) node[above]{$\mbox{\footnotesize $3+2\imath$}$};
        \draw[-Stealth, thick, Blue!80] (0,0) -- (-2,5) node[above]{$\mbox{\footnotesize $-2+5\imath$}$};
        \draw[-Stealth, thick, Blue!80] (0,0) -- (-2.6,-1.2) node[below left]{$\mbox{\footnotesize $-2.6-1.2\imath$}$};
        \draw[-Stealth, thick, Blue!80] (0,0) -- (2,-1.7) node[below]{$\mbox{\footnotesize $2-1.7\imath$}$};
    \end{scope}
    \end{tikzpicture}
    \caption*{\texttt{复平面}}
\end{figure}
...
$$ \forall k\in\hin{1}{n}, \quad z_k = \sqrt[n]{r}\left(\cos{\frac{2k\pi}{n}} + \imath \sin{\frac{2k\pi}{n}}\right) = \sqrt[n]{r}\omega_n^k. $$
我们把这$n$个解都称为$a$的$n$次方根。

同理，如果复数$z$可以表示为$z = r(\cos{\alpha} + \imath \sin{\alpha})$，那么$x^n = z$的解为：
$$ \forall k\in\hin{1}{n}, \; z_k = \sqrt[n]{r}\left(\cos{\frac{2k\pi + \alpha}{n}} + \imath \sin{\frac{2k\pi + \alpha}{n}}\right) = \sqrt[n]{r}\left(\cos{\frac{\alpha}{n}} + \imath \sin{\frac{\alpha}{n}}\right)\omega_n^k. $$
...
\begin{wrapfigure}[5]{r}{0.32\textwidth} %this figure will be at the right
    ...
    \begin{scope}[shift={(0,0)}]  % x=5cm,y=0.5cm,
        ...
        \def\cb{0.2};
        \def\cc{0.12};
        \foreach \x in {1,2,3,4} {  % x轴正向
            \draw (\x,\ym*\cc) -- (\x,-\ym*\cc); 
            \node[black] at (\x, -\ym*\ca) {$\mbox{\tiny $\x$}$};
        }
        \foreach \x in {1,2,...,4} {  % x轴负向
            \draw (-\x,\ym*\cc) -- (-\x,-\ym*\cc); 
            \node[black] at (-\x-\xm*\cb, -\ym*\ca) {$\mbox{\tiny $-\x$}$};
        }
        \foreach \y in {1,2,3} {  % y轴正向
            \draw (\xm*\cc,\y) -- (-\xm*\cc,\y);
            \node[black] at (-\xm*\ca, \y) {$\mbox{\tiny $\y$}$};
        }
        \foreach \y in {1,2,3} {  % y轴负向
            \draw (\xm*\cc,-\y) -- (-\xm*\cc,-\y);
            \node[black] at (-\xm*\ca-\xm*\cb, -\y) {$\mbox{\tiny $-\y$}$};
        }
        % 作图
        \draw[Blue, very thick] (0,0) ellipse (3 cm and 2 cm);
    \end{scope}
    \end{tikzpicture}
\end{wrapfigure}
...
\begin{wrapfigure}[5]{r}{0.4\textwidth} %this figure will be at the right
    ...
    \begin{scope}[shift={(0,0)}]  % x=5cm,y=0.5cm,
        ...
        % 作图
        % \draw[Blue, thick] (0,0) ellipse (3 cm and 2 cm);
        \draw[Red,thick,dashed] (-4.95,-3.3) -- (5,3.333);
        \draw[Red,thick,dashed] (-4.95,3.3) -- (5,-3.333);
        \draw[domain=-72.5:72.5,very thick,OliveGreen] plot({1.5*sec(\x)},{tan(\x)});
        \draw[domain=107.5:252.5,very thick,OliveGreen] plot({1.5*sec(\x)},{tan(\x)});
    \end{scope}
    \end{tikzpicture}
\end{wrapfigure}
...
\begin{wrapfigure}[5]{l}{0.39\textwidth} 
    \vspace{-24pt}
    \flushright
    ...
    \begin{scope}[shift={(0,0)}]  % x=5cm,y=0.5cm,
        ...
        \draw[Red,thick,dashed] (-4.95,-3.3) -- (5,3.333);
        \draw[Red,thick,dashed] (-4.95,3.3) -- (5,-3.333);
        \draw[domain=-72.5:72.5,very thick,OliveGreen] plot({1.5*tan(\x)},{sec(\x)});
        \draw[domain=107.5:252.5,very thick,OliveGreen] plot({1.5*tan(\x)},{sec(\x)});
    \end{scope}
    \end{tikzpicture}
\end{wrapfigure}

双曲线由两个不相连的分支构成，一般按位置称为上下支或左右支。曲线不是闭合的，而是无界的，
关于$x$轴和$y$轴对称。
...
% 抛物线例图
\begin{wrapfigure}[5]{r}{0.45\textwidth} %this figure will be at the right
    \vspace{-20pt}
    % \flushright
    ...
    \begin{scope}[shift={(0,0)}]
        ...
        % 作图
        \draw[domain=-3.16:3.16,very thick,BrickRed] plot({\x*\x},{\x});
    \end{scope}
    \end{tikzpicture}
\end{wrapfigure}
...
\begin{wrapfigure}[9]{l}{0.33\textwidth} %this figure will be at the right
    \vspace{-8pt}
    \flushleft
    ...
    \begin{scope}[shift={(0,0)}]
        ...
        \def\cb{0.2};
        \def\cc{0.12};
        \foreach \x in {1,2,3} {  % x轴正向
            \draw (\x,\ym*\cc) -- (\x,-\ym*\cc); 
            \node[black] at (\x, -\ym*\ca) {$\mbox{\tiny $\x$}$};
        }
        \foreach \x in {1,2,3} {  % x轴负向
            \draw (-\x,\ym*\cc) -- (-\x,-\ym*\cc); 
            \node[black] at (-\x-\xm*\cb, -\ym*\ca) {$\mbox{\tiny $-\x$}$};
        }
        \foreach \y in {1,2,...,9} {  % y轴正向
            \draw (\xm*\cc,\y) -- (-\xm*\cc,\y);
            \node[black] at (-\xm*\ca, \y) {$\mbox{\tiny $\y$}$};
        }
        % \foreach \y in {1,2,3} {  % y轴负向
...
\begin{figure}[h] 
    ...
    \begin{scope}[shift={(0,0)}]  % x=5cm,y=0.5cm,
        ...
        \def\cb{0.12};
        \def\cc{0.1};
        \foreach \x in {1,2,...,6} {  % x轴正向
            \draw (\x,\ym*\cc) -- (\x,-\ym*\cc); 
            \node[black] at (\x, -\ym*\ca) {$\mbox{\tiny $\x$}$};
        }
        \foreach \x in {1,2,...,9} {  % x轴负向
            \draw (-\x,\ym*\cc) -- (-\x,-\ym*\cc); 
            \node[black] at (-\x-\xm*\cb, -\ym*\ca) {$\mbox{\tiny $-\x$}$};
        }
        \foreach \y in {1,2,...,5} {  % y轴正向
        ...
        \coordinate (A) at (2.58,1.14);
        \coordinate (B) at (1.11,2.08);
        \draw[BrickRed, thick] (0,0) ellipse (3 cm and 2.2361 cm);
        \draw[dashed,thick,orange] (F1) ++(-\v:2*\a) arc [start angle=-\v, end angle=180+\v, radius=2*\a];
        \draw[Blue,dotted,thick] (B) circle (2.26);
        \draw[Turquoise,dotted,thick] (A) circle (1.28);
        \draw[Turquoise,thick] (F1) -- (A) (F2) -- (A);
        \draw[Turquoise,thick,dashed] (A) -- (3.82,1.46);
        \draw[blue,thick] (F1) -- (B) (F2) -- (B);
        \draw[blue,thick,dashed] (B) -- (2.99,3.33);
        \draw[fill=Red] (F1) circle (3pt);
        \draw[fill=Red] (F2) circle (3pt);
        \draw[fill=OliveGreen] (A) circle (3pt);
        \draw[fill=Blue] (B) circle (3pt);
        \draw[fill=OliveGreen] (3.82,1.46) circle (3pt);
        \draw[fill=Blue] (2.99,3.33) circle (3pt);
    \end{scope}
    \end{tikzpicture}
...
\begin{figure}[h] 
    ...
    \begin{scope}[shift={(0,0)}]
        ...
        \coordinate (F1) at (-\c, 0);
        \coordinate (F2) at (\c, 0);
        \coordinate (A) at (4.24,-2.64);
        \coordinate (B) at (5.07,3.6);
        \draw[domain=-56.4:67.3,very thick,BrickRed] plot({\a*sec(\x)},{\b*tan(\x)});
        \draw[domain=123.6:247.3,very thick,BrickRed] plot({\a*sec(\x)},{\b*tan(\x)});
        \draw[dashed,thick,orange] (-4,0) ++ (-\va:2*\a) arc [start angle=-\va, end angle=160, radius=2*\a];
        \draw[dashed,thick,orange] (-4,0) ++ (200:2*\a) arc [start angle=200, end angle=180+\va, radius=2*\a];
        \draw[Blue,dotted,thick] (B) ++ (90+\vb:3.76) arc [start angle=90+\vb, end angle=450-\vb, radius=3.76];
        \draw[Turquoise,dotted,thick] (A) ++ (-\vc:2.65) arc [start angle=-\vc, end angle=180+\vc, radius=2.65];
        \draw[Turquoise,thick] (F1) -- (A) (F2) -- (A);
        \draw[Turquoise,thick,dashed] (A) -- (1.71,-1.83);
        \draw[blue,thick] (F1) -- (B) (F2) -- (B);
        \draw[blue,thick,dashed] (B) -- (1.58,2.22);
        \draw[fill=Red] (F1) circle (3pt);
        \draw[fill=Red] (F2) circle (3pt);
        \draw[fill=OliveGreen] (A) circle (3pt);
        \draw[fill=Blue] (B) circle (3pt);
        \draw[fill=OliveGreen] (1.71,-1.83) circle (3pt);
        \draw[fill=Blue] (1.58,2.22) circle (3pt);
    \end{scope}
    \end{tikzpicture}
...
\begin{figure}[h] 
    ...
    \begin{scope}[shift={(0,0)}]
        ...
        \def\cb{0.2};
        \def\cc{0.12};
        \foreach \x in {2,4,...,22} {  % x轴正向
            \draw (\x,\ym*\cc) -- (\x,-\ym*\cc); 
            \node[black] at (\x, -\ym*\ca) {$\mbox{\tiny $\x$}$};
        }
        \foreach \x in {2,4} {  % x轴负向
            \draw (-\x,\ym*\cc) -- (-\x,-\ym*\cc); 
            \node[black] at (-\x-\xm*\cb, -\ym*\ca) {$\mbox{\tiny $-\x$}$};
        }
        \foreach \y in {2,4,...,8} {  % y轴正向
            \draw (\xm*\cc,\y) -- (-\xm*\cc,\y);
            \node[black] at (-\xm*\ca, \y) {$\mbox{\tiny $\y$}$};
        }
        \foreach \y in {2,4} {  % y轴负向
            \draw (\xm*\cc,-\y) -- (-\xm*\cc,-\y);
            \node[black] at (-\xm*\ca-\xm*\cb, -\y) {$\mbox{\tiny $-\y$}$};
        }
        % 作图
        ...
        \def\r{0.4};
        \coordinate (F1) at (\c, 0);
        \coordinate (A) at (1.33,-2.31);
        \coordinate (B) at (3.44,3.71);
        \draw[domain=-3:4.5,very thick,BrickRed] plot({\x*\x},{\a*\x});
        \draw[fill=Blue!30,opacity=0.6] (-1, 3.71) rectangle (-1+\r,3.71-\r);
        \draw[fill=OliveGreen!30,opacity=0.6] (-1, -2.31) rectangle (-1+\r,-2.31-\r);
        \draw[orange!40!brown, very thick] (-1, \ymin) -- (-1, \ymax);
        \draw[Blue,dotted,thick] (B) circle (4.44);
        \draw[Turquoise,dotted,thick] (A) circle (2.33);
        \draw[Turquoise,thick] (F1) -- (A);
        \draw[Turquoise,thick,dashed] (A) -- (-1, -2.31);
        \draw[blue,thick] (F1) -- (B);
        \draw[blue,thick,dashed] (B) -- (-1, 3.71);
        \draw[fill=Red] (F1) circle (4.5pt);
        \draw[fill=OliveGreen] (A) circle (4.5pt);
        \draw[fill=Blue] (B) circle (4.5pt);
        \draw[fill=OliveGreen] (-1, -2.31) circle (4.5pt);
        \draw[fill=Blue] (-1, 3.71) circle (4.5pt);
    \end{scope}
    \end{tikzpicture}
...
\begin{wrapfigure}[10]{r}{0.35\textwidth} %this figure will be at the right
    \vspace{-30pt}
    \flushright
    \includegraphics[width=0.3\textwidth]{tu/复杂闭曲线.pdf}
...
\begin{tikzpicture}
    ...
    \draw [->,line width=1pt,>={Stealth[]}] (0,0) -- (\maxDots*\xunit,0) node[above left] {\texttt{总点数}};
    ...
    \node [below, align=center] at (\maxDots*\xunit*0.5, -5cm) {\texttt{投掷两个骰子时所有的结果以及总点数遵从的分布}};
...
\begin{so}
    对任意正整数$n$，设掷硬币第$n$次时首次出现正面，那么前$n-1$次都是反面。于是随机变量“首次出现正面的次数”$X$遵从的分布为\footnote{这里$\Pr{X=n}$中的$\mathbb{P}$其实是随机变量$X$对应的概率映射，$\Pr{X=n}$应该写成$\Pr{\{s\in\Omega\,| \, X(s) = n\}}$。但为了方便，我们这里用$\mathbb{P}$直接表示随机变量值的分布。这是一种“符号的滥用”，需要注意区分。下同。}：
    $$ \Pr{X=n} = (1 - p)^{n-1} p.$$
    因此我们讨论的是以下的分布：
...
\begin{tikzpicture}
    ...
    \addplot+[ybar, samples at={1,2,...,12}, fill=blue!30] 
        (\x, {binomial(\x)});
        
    \end{axis}
    
    \node [below] at (8.4cm,-0.18cm) {$\ldots$};

    \node [above] at (8cm,0.14cm) {\texttt{随机变量值}$n$};

    \node [below, align=center] at (5.2cm, -1cm) {\texttt{系数}$p=0.4$\texttt{的等比分布（只显示$n\leqslant 12$的项）}};
...
\begin{so}
    ...
    \begin{align*}
        ...
        &= a.
    \end{align*}
    也就是说，$X$的期望是$a$，符合我们的假设。我们把$a$称为乍生分布的系数。于是该县明年每天出生的婴儿数遵从系数$a=10$的乍生分布。
\end{so}
...
\begin{tikzpicture}
    ...
    \addplot+[ybar, samples at={0,...,20}, fill=blue!30] 
        (\x, {poisson(10)});
    
    \end{axis}
    
    \node [below] at (0,-0.06cm) {$0$};
    
    \node [below] at (10.22cm,-0.18cm) {$\ldots$};

    \node [above] at (9.6cm,0.14cm) {\texttt{随机变量值}$n$};

    \node [below, align=center] at (6cm, -1cm) {\texttt{系数}$a=10$\texttt{的乍生分布（只显示$n\leqslant20$的项）}};
    
\end{tikzpicture}

乍生分布的假设是一段时间内发生的机会“均匀”的事件：事件发生的概率与时间段长度成正比。这有些近似于我们想要的“在无限个选项中均匀地随机选一个”的实验。但要注意的是，它要遵循的前提假设是：事件发生是瞬间的，与前后是否发生无关。
...
\begin{sk}
    \mbox{}\\
    \indent 1. $\mathbb{N}$上的实随机变量$X$的值总大于等于零，但它的期望等于$0$，那么它什么时候大于$0$？它大于$0$的概率是多少？它能不能有大于$0$的值？\\
    \indent 2. $\mathbb{N}$上的实随机变量$X$的方差等于$0$，它的值有什么性质？
\end{sk}
...
\begin{sk}
    \mbox{} \\
    \indent 1. 是否有值的分布相同，但在概率空间中的分布不同的随机变量？如果有，举出例子。\\
    \indent 2. 乍生分布的例子里，为什么可以假设足够短时间内发生多于一次事件的概率，相比发生至多一次的概率，可以忽略？\\
    \indent 3. 给定无限集合$\Omega$及其分划$\Omega = \sum_{i\in\mathbb{N}} S_n$，给定收敛正项级数$\sum a_n$，其级数和为$1$。是否有随机变量$X$和概率映射$\mathbb{P}$，使得：
...
\begin{xt}
    \mbox{} \\
    \indent 1. 给定$0<p<1$，\\
    \indent 1.1. 计算$S_2 = \sum_{n=1}^{+\infty} n(n - 1)(1 - p)^{n-1} p$。\\
    \indent 1.2. 随机变量$X$遵从系数为$p$的等比分布，计算它的方差：
    $$ \mathrm{Var}(X) = \mathbb{E}(X - \mathbb{E}(X))^2.$$
    \indent 2. 计算乍生分布的二次方望，并用它来验证乍生分布的方差公式。 \\
\end{xt}
...
\end{cases}
\]
\(X\) 的边际分布为 \(\forall x\in\hin{1}{6},\; \mathbb{P}_X(x) = \frac{1}{6}\)，这正是单枚骰子的均匀分布。
...
\end{cases}
\]
\(X\) 的边际分布仍为 \(\forall x\in\hin{1}{6},\; \mathbb{P}_X(x) = \frac{1}{6}\)。而$Z$的边际分布为：
\[
...
\begin{sk}
    \mbox{} \\
    \indent 1. 协方差和相关系数的性质让你想到哪个学过的概念？对比相关概念，讨论它们的共同之处。\\
    \indent 2. 除了协方差和相关系数，还有哪些方法来表示两个随机变量的关联？做一个调查并选其中一种方法进行了解。它和协方差（相关系数）相比，有哪些不同？
\end{sk}

\begin{xt}
    \mbox{} \\
    \indent 1. 设随机变量 \(X\) 和 \(Y\) 的联合分布为：
    \begin{center}
        \begin{tabular}{c|cc}
            & \(Y=0\) & \(Y=1\) \\ \hline
            \(X=0\) & 0.2 & 0.3 \\
