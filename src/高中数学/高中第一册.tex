...
\begin{so}  
    ...
    \begin{align}
        C_n^1 d = 80 \time 0.01 &= 0.8 \notag \\
        C_n^2 d = \frac{80 \times 79}{2\times 1} \time 0.01^2 &= 0.316 \notag \\ 
        C_n^3 d = \frac{80 \times 79\times 78}{3\times 2\times 1}80 \time 0.01^3 &= 0.08216 \notag
    \end{align}
...
\begin{xt}
    ...
    \indent 2. 设有正整数$n > 1$，实数$x > -1$，用归纳法证明：$(1 + x)^n > 1 + nx.$
\end{xt}
...
\begin{so}
    ...
    根据二项式定理：
    \begin{align}
        \left(1+\frac{1}{n}\right)^n &= 1 + C_n^1\cdot \frac{1}{n} + C_n^2\cdot \left(\frac{1}{n}\right)^2 + \cdot + C_n^n\cdot \left(\frac{1}{n}\right)^n \notag \\
    ...
    对于其他$0$的有理数列$\{x_n\}_{n\in\mathbb{Z}^+}$，$\{2^{x_n}\}_{n\in\mathbb{Z}^+}$是否也趋于$1$呢？我们可以拿它和$\{a_n\}$作比较。
...
一般来说，我们把$1$称为函数$f_{\mathbb{Q}}$在$0$这一点的极限。
...
\begin{xt}
    ...
    \indent 2.3. 证明：函数$f$在$a$处有极限，当且仅当它在$a$处有左极限和右极限。\\
...
我们证明了函数$f:x\mapsto x^2$在任意实数$a$处连续。这种连续性质不再是局部的，而是全局的。
...
% \begin{proof}
%     不妨设$f(a) < v < f(b)$，我们要找到$(a, b)$中某个数$u$，使得$f(u) = v$。使用二分法来找出这个$u$。

%     我们的搜索范围是区间$(a,\,\, b)$，左端是$a$，右端是$b$。我们在这两端之间找$u$。我们记$a_0 = a$，$b_0 = b$。

%     考察$u_1 = \frac{a+b}{2}$。如果$f(u_1) = v$，那么我们就找到了$u$。如果$f(u_1) \neq v$，则要么$f(u_1) < v$，要么$f(u_1) > v$。
%     如果$f(u_1) < v$，则考虑在区间$(u_1,\,\, b)$搜索。如果$f(u_1) < v$，则考虑在区间$(a, \,\,u_1)$搜索。要注意的是，无论在哪个区间搜索，区间长度都是$(a, \,\,b)$的一半。
%     把新区间的两端记为$a_1$和$b_1$，则$f(a_1) < v$，$f(b_1) > v$。

%     不断重复这个操作，则要么某次搜索时恰好找到$u$，要么每次搜索的区间$(a_n,\,\, b_n)$长度都减半，而且$f(a_n) < v$，$f(b_n) > v$。
%     按照定义，每次搜索的区间都是原来区间的子区间，所以，只要给定正整数$N$，那么对所有$m>n>N$，$a_n\leqslant a_m<b_m\leqslant b_n$都在区间$(a_N, b_N)$里。也就是说，总有：
%     $$ |a_m - a_n| < |b_N - a_N| = \frac{b - a}{2^N}, \quad |b_m - b_n| < |b_N - a_N| = \frac{b - a}{2^N}. $$   
%     因此，如果考虑数列$\{a_n\}$和$\{b_n\}$，它们一个是单调递增数列，一个是单调递减数列，而且都是自敛数列。设它们的极限分别是$a_\infty$、$b_\infty$，
%     则由于数列$\{b_n - a_n\}$趋于$0$，按照实数的定义，两数列的极限是同一个实数：$a_\infty = b_\infty$（参见附录一）。

%     由于$f(a_n)$总小于$v$，所以$f(a_\infty) \leqslant v$。同理，由于$f(b_n)$总大于$v$，所以$f(a_\infty) \geqslant v$。
...
\begin{xt}
    ...
    ，且$g$在$b$处连续。\\
    \indent 3.1. 证明：复合函数$g\circ f$在$a$处有极限，极限是$g(b)$。\\
    \indent 3.2. 举例说明，如果$g$在$b$处有极限$v$，但不连续，那么复合函数$g\circ f$在$a$处的极限不一定是$v$。\\
    \indent 4. 证明：如果函数$f$在区间$[a, \,\,b]$上连续且严格单调，那么$f(a)$和$f(b)$之间任一个数$v$，总恰好是$(a, \,\,b)$中一个数$u$经过$f$得到的值：$f(u) = v$。\\
    \indent 5. 实变函数$f$在点$a$处连续，$f(a)>0$。证明：$f$在$a$附近大于$0$。
\end{xt}
...
比如，我们经常接触到的曲线，大多是连续不断的。我们可以说，这些曲线是实数区间经过连续函数映射
的图像。

以圆周为例子。圆周对应的映射是：$\gamma: t\mapsto (\cos{2\pi t}, \,\, \sin{2\pi t})$。
我们来证明它是连续曲线。为此，我们要证明正弦函数和余弦函数在其定义域上是连续函数。
...
使用复合函数的连续性质，我们可以轻松推出：余弦函数在定义域上也处处连续。进一步可推出，圆周对应的映射：$\gamma$也是连续函数。

\begin{xt}
    \mbox{} \\
    \indent 1. 证明：正切函数、余切函数在其定义域上是连续函数。\\
    \indent 2. 是否有在实数集上处处连续的函数$f$，使得$f$在正切函数的定义域上的值恰好与正切函数完全相同？
...
\begin{tm}{函数在一点的极限唯一}\label{tm:b-0-10}
...
\begin{tm}{单调函数的极限}\label{tm:b-0-30}
...
\begin{proof}
    ...
    下面证明$b$是$f$在$a$处的右极限。

    根据$b$的定义，对任意$r>0$，$b - r$不是$I_a^{\text{左}}$的上界，因此总有$x_r\in I_a^{\text{左}}$使得$f(x_r)>b-r$。
    而$f$是单调函数，所以对任意$x_r<x<a$，总有
    $$ b - r < f(x_r) \leqslant f(x) \leqslant b.$$
    这就说明$b$是$f$在$a$处的右极限。 

    同理，考虑单调递增函数$f$在$a$右旁的表现，用类似的方法可以证明它在$a$处有左极限。
