...
集合不是记录聚在一起的事物的唯一方式。比如，\textbf{有序组}可以记录元素的顺序；\textbf{族}则允许相同元素重复。
...
数列的项和序数有一一对应的关系，这告诉我们，数列的本质是正整数集或其子集$\hin{1}{N}$到数域的函数。
定义域是$\hin{1}{N}$的数列，项数有限，称为\textbf{有穷数列}；定义域是正整数集的数列，项数无限，叫做\textbf{无穷数列}。
我们一般把数列记作：
$$ a_1, a_2, \cdots, a_n, \cdots $$
其中$a_n$是数列的第$n$项。项数$n$也叫做\textbf{下标}。
为了方便，我们在行文中会把以上数列记作$\{a_n\}_{n\in\mathbb{Z}^+}$（无穷数列）
或$\{a_n\}_{n\in\hin{1}{N}}$（有穷数列），或简单记作$\{a_n\}$\footnote{这种记法容易与集合混淆。约定：大括号内只有带下标的小写字母时，除非另作说明，否则优先考虑为数列。}。
...
\begin{figure}[h]
    ...
    \begin{scope}[shift={(0,0)}]
        ...
        \def\ymax{8.6};
        % 坐标轴（带箭头）
        \foreach \k in {-4,-3,-2,-1,1,2,...,8} {
            \draw[help lines,gray!60] (\xmin,\k) -- (\xmax,\k) ;
        }
        \foreach \k in {-6,-5,...,-1,1,2,...,9} {
            \draw[help lines,gray!60] (\k,\ymin) -- (\k,\ymax);
        }
        \draw[-Stealth, thick] (\xmin, 0) -- (\xmax,0) node[above] {$x$};
        \draw[-Stealth, thick] (0,\ymin) -- (0,\ymax) node[right] {$y$};
        \foreach \k in {1,2,...,8} { 
            \node at (-0.3,\k) {$\mbox{\tiny $\k$}$};
            \node at (\k,-0.3) {$\mbox{\tiny $\k$}$};
        }
        \node at (9,-0.3) {$\mbox{\tiny $9$}$};
        \foreach \k in {-4,-3,-2,-1} {  % ,
            \node at (-0.45,\k) {$\mbox{\tiny $\k$}$};
        }
        % 作图
        \draw[red, thick, domain=\ymin:\ymax, samples=200]
        plot (\x,{\x});
        \draw[OliveGreen, thick, domain=-2.898:2.898, samples=200]
        plot (\x,{\x*\x});
        \draw[blue, thick, domain=-1.64:2.03, samples=200]
        plot (\x,{\x*\x*\x});
        \draw[SpringGreen, thick, domain=-1.701:1.701, samples=200]
        plot (\x,{\x*\x*\x*\x});
        \draw[Purple, thick, domain=-1.345:1.53, samples=200]
        plot (\x,{\x*\x*\x*\x*\x});
        \draw[Dandelion, thick, domain=-1.4256:1.4257, samples=200]
        plot (\x,{\x*\x*\x*\x*\x*\x});
        \draw[very thick, fill=white] (5,3.2) rectangle (8.8,4.6);
        \node at (6.9, 3.9) {$\mbox{\small $f_n:\,x\mapsto x^n$}$};
        \draw[very thick, fill=white] (4,-4) rectangle (8.4,2);
        \draw[red,ultra thick] (4.5,1.5) -- (5.4,1.5);
        \draw[OliveGreen,ultra thick] (4.5,0.5) -- (5.4,0.5);
        \draw[blue,ultra thick] (4.5,-0.5) -- (5.4,-0.5);
        \draw[SpringGreen,ultra thick] (4.5,-1.5) -- (5.4,-1.5);
        \draw[Purple,ultra thick] (4.5,-2.5) -- (5.4,-2.5);
        \draw[Dandelion,ultra thick] (4.5,-3.5) -- (5.4,-3.5);
        \foreach \k in {1,2,...,6} { 
            \node at (6.6,2.5-\k) {$\mbox{\footnotesize $n=\k$}$};
            \node at (-\k-0.15,-0.3) {$\mbox{\tiny $-\k$}$};
        }
    \end{scope}
    \end{tikzpicture}
...
\begin{figure}[h]
    ...
    \begin{scope}[shift={(0,0)}]
        ...
        \def\cf{0.06};
        % 坐标轴（带箭头）
        \foreach \k in {-1.5,-1,-0.5,0.5,1,...,2} { % ymin -> ymax
            \draw[help lines,gray!60] (\xmin,\k) -- (\xmax,\k) ;
        }
        \foreach \k in {-2,-1.5,-1,-0.5,0.5,1,...,2.5} { % xmin -> xmax
            \draw[help lines,gray!60] (\k,\ymin) -- (\k,\ymax);
        }
        \draw[-Stealth, thick] (\xmin, 0) -- (\xmax,0) node[above] {$x$};
        \draw[-Stealth, thick] (0,\ymin) -- (0,\ymax) node[right] {$y$};
        \foreach \k in {0.5,1,...,2} { 
            \node at (-\ca,\k) {$\mbox{\scriptsize $\k$}$};
            \node at (\k,-\ca) {$\mbox{\scriptsize $\k$}$};
            \node at (-\k-\cf,-\ca) {$\mbox{\scriptsize $-\k$}$};
        }
        \node at (2.5,-\ca) {$\mbox{\scriptsize $2.5$}$};
        \foreach \k in {-1.5,-1,-0.5} {  % ,
            \node at (-\ca-\cf,\k) {$\mbox{\scriptsize $\k$}$};
        }
        % 作图
        \draw[red, thick, domain=\xmin:-0.5, samples=200]
        plot (\x,{1/\x});
        \draw[red, thick, domain=0.4:\xmax, samples=200]
        plot (\x,{1/\x});
        \draw[OliveGreen, thick, domain=\xmin:-0.633, samples=200]
        plot (\x,{1/(\x*\x)});
        \draw[OliveGreen, thick, domain=0.633:\xmax, samples=200]
        plot (\x,{1/(\x*\x)});
        \draw[blue, thick, domain=\xmin:-0.794, samples=200]
        plot (\x,{1/(\x*\x*\x)});
        \draw[blue, thick, domain=0.737:\xmax, samples=200]
        plot (\x,{1/(\x*\x*\x)});
        \draw[SpringGreen, thick, domain=\xmin:-0.796, samples=200]
        plot (\x,{1/(\x*\x*\x*\x)});
        \draw[SpringGreen, thick, domain=0.796:\xmax, samples=200]
        plot (\x,{1/(\x*\x*\x*\x)});
        \draw[Purple, thick, domain=\xmin:-0.871, samples=200]
        plot (\x,{1/(\x*\x*\x*\x*\x)});
        \draw[Purple, thick, domain=0.833:\xmax, samples=200]
        plot (\x,{1/(\x*\x*\x*\x*\x)});
        \draw[Dandelion, thick, domain=\xmin:-0.859, samples=200]
        plot (\x,{1/(\x*\x*\x*\x*\x*\x)});
        \draw[Dandelion, thick, domain=0.859:\xmax, samples=200]
        plot (\x,{1/(\x*\x*\x*\x*\x*\x)});
        \draw[very thick, fill=white] (1.2,1.8) rectangle (2.8,2.4);
        \node at (1.95, 2.1) {$\mbox{ $\displaystyle f_{-n}:\,x\mapsto \frac{1}{x^n}$}$};
        \def\xpd{0.13};
        \def\xlb{0.4};
        \def\xmw{1.1};
        \def\ylb{-1.95};
        ...
        \def\ymw{0.4};
        \def\bar{0.23};
        \draw[very thick, fill=white] (\xlb,\ylb) rectangle (\xlb+2*\xmw,\ylb+2*\ymw+2*\ypd);
        \draw[blue,ultra thick] (\xlb+\xpd,\ylb+\ypd) -- (\xlb+\xpd+\bar,\ylb+\ypd);
        \draw[OliveGreen,ultra thick] (\xlb+\xpd,\ylb+\ypd+\ymw) -- (\xlb+\xpd+\bar,\ylb+\ypd+\ymw);
        \draw[red,ultra thick] (\xlb+\xpd,\ylb+\ypd+2*\ymw) -- (\xlb+\xpd+\bar,\ylb+\ypd+2*\ymw);
        \draw[Dandelion,ultra thick] (\xlb+\xpd+\xmw,\ylb+\ypd) -- (\xlb+\xpd+\bar+\xmw,\ylb+\ypd);
        \draw[Purple,ultra thick] (\xlb+\xpd+\xmw,\ylb+\ypd+\ymw) -- (\xlb+\xpd+\bar+\xmw,\ylb+\ypd+\ymw);
        \draw[SpringGreen,ultra thick] (\xlb+\xpd+\xmw,\ylb+\ypd+2*\ymw) -- (\xlb+\xpd+\bar+\xmw,\ylb+\ypd+2*\ymw);
        \foreach \k in {1,2,3} { 
            \node at (0.5*\xmw+\xlb+\xpd,\ylb+\ypd+3*\ymw-\k*\ymw) {$\mbox{\footnotesize $n=\k$}$};
        }
        \foreach \k in {4,5,6} { 
            \node at (1.5*\xmw+\xlb+\xpd,\ylb+\ypd+6*\ymw-\k*\ymw) {$\mbox{\footnotesize $n=\k$}$};
        }
    \end{scope}
  \end{tikzpicture}
    \caption*{\texttt{整幂函数}$x\mapsto x^{-n}$\texttt{的图像}}
...
$\sqrt{ax^2 + bx + c}$型函数。这类函数是二次函数开方得到的。一般形式为：
$$ x\mapsto \sqrt{ax^2 + bx + c}.$$
其中$a, b, c$是常数系数。比如，$x\mapsto \sqrt{x^2 + 2x - 3}$就是$\sqrt{ax^2 + bx + c}$类函数。

$\frac{\sqrt{ax + b}}{\sqrt{cx + d}}$型函数。这类函数是两个$\sqrt{ax + b}$类函数的商。一般形式为：
$$ x\mapsto \frac{\sqrt{ax + b}}{\sqrt{cx + d}}.$$
其中$a, b, c, d$是常数系数。比如，$x\mapsto \frac{2x - 3}{\sqrt{x + 3}}$就是$\frac{\sqrt{ax + b}}{\sqrt{cx + d}}$类函数。

\begin{figure}[h]
    \vspace{4pt}
    \centering
    \includegraphics[width=0.96\textwidth]{tu/无理函数1.png}
...
\begin{figure}[h] %this figure will be at the right
    ...
    \begin{axis}[
        ...
        legend style = {at={(0.96,0.98)}, anchor=north east, cells={anchor=west}},
        tick label style = {font=\scriptsize}
    ]
    \addplot [
        OliveGreen, thick, smooth,
        domain = -2*pi-1.5:3*pi+3
    ] {sin(deg(x))};
    \addlegendentry{$\mbox{\footnotesize $\,\,\sin (x)$}$}

    \addplot [
        red, thick, smooth,
        domain = -2*pi-1.5:3*pi+3
    ] {cos(deg(x))};
    \addlegendentry{$\mbox{\footnotesize $\,\,\cos (x)$}$}

    \addplot [
        Tan, thick, smooth,
        domain = -pi/2+0.1:pi/2-0.1
    ] {tan(deg(x))};
    \addlegendentry{$\mbox{\footnotesize $\,\,\tan (x)$}$}

    \addplot [
        blue!70, thick, smooth,
        domain = 0.1:pi-0.1
    ] {tan(90-deg(x))};
    \addlegendentry{$\mbox{\footnotesize $\,\,\cot (x)$}$}

    \addplot [
        Tan, thick, smooth,
        domain = -1*pi-pi/2+0.1:-1*pi+pi/2-0.1
    ] {tan(deg(x))};

    \addplot [
        Tan, thick, smooth,
        domain = -2*pi-pi/2+0.1:-2*pi+pi/2-0.1
    ] {tan(deg(x))};
    ...
    \addplot [
        Tan, thick, smooth,
        domain = 3*pi-pi/2+0.1:3*pi+pi/2-0.1
    ] {tan(deg(x))};

    \addplot [
        Tan, thick, smooth,
        domain = 4*pi-pi/2+0.1:3*pi+3
    ] {tan(deg(x))};

    \addplot [
        blue!70, thick, smooth,
        domain = -2*pi-1.5:-2*pi-0.1
    ] {tan(90-deg(x))};

    \addplot [
        blue!70, thick, smooth,
        domain = -2*pi+0.1:-2*pi+pi-0.1
    ] {tan(90-deg(x))};

    \addplot [
        blue!70, thick, smooth,
        domain = -1*pi+0.1:-1*pi+pi-0.1
    ] {tan(90-deg(x))};
...
这里我们使用\textbf{弧度制}代替角度制。也就是说，不用角度，而用弧度，
也就是角在单位圆中对应的弧长来表示角的大小。对于零角与平角之间的角，以其顶点为圆心作单位圆，作为圆心角，它在单位圆中对应的弧长和角的大小成正比。角度和弧度的转换可以用以下公式：
...
$\sqrt{2}$弧度对应$\frac{180\sqrt{2}}{\pi}\approx81.03$角度，等等。更一般的角度，也可以按照以上公式转换。

从图中可以看到，正弦函数和余弦函数是有界函数，定义域是$\mathbb{R}$，值域是$[\,-1;1\,]$。
正切函数和余切函数是无界函数。正切函数的定义域是$\{x\in\mathbb{R}\,|\,x\neq (k+\frac{1}{2})\pi, \;\,\, k\in\mathbb{Z}\}$，余切函数的定义域是$\{x\in\mathbb{R}\,|\,x\neq k\pi, \;\,\, k\in\mathbb{Z}\}$。正弦、正切、余切函数是奇函数，余弦函数是偶函数。

此外，对任意实数$x$，任意整数$n$，都有$\sin(x+2n\pi) = \sin{x}$，$\cos(x+2n\pi) = \cos{x}$。对各自定义域中任意实数$x$，任意整数$n$，都有$\tan(x+n\pi) = \tan{x}$，$\cot(x+n\pi) = \cot{x}$。我们把这样的函数称为周期函数。
\begin{df}
    给定函数$f$。如果有$T>0$使得其定义域中任何$x$，都满足
    $$ f(x + T) = f(x),$$
...
\begin{wrapfigure}[8]{r}{0.52\textwidth} %this figure will be at the right
    ...
    \begin{scope}[shift={(0,0)}]
        ...
        \def\dpi{1.5708}
        \foreach \k in {1,2} {
            \draw[help lines,gray!60] (\k*\dpi,\ymin) -- (\k*\dpi,\ymax) ;
        }
        \draw[help lines,gray!60] (\xmin,1) -- (\xmax,1) ;
        \draw[-Stealth, thick] (\xmin, 0) -- (\xmax,0) node[above] {$x$};
        \draw[-Stealth, thick] (0,\ymin) -- (0,\ymax) node[right] {$y$};
        % 作图
        \coordinate (O) at (0,0);
        \coordinate (A) at (\dpi,1);
        \draw[Blue, ultra thick, domain=0:2*\dpi, samples=200]
        plot (\x,{sin(deg(\x))});
        \draw[red!50!orange,ultra thick,opacity=0.7] (O) -- (0.8,0.8);
        \draw[red!50!orange,ultra thick,opacity=0.7] (O) -- (-0.2,-0.2);
        \draw[red!50!orange,ultra thick,opacity=0.7] (A) -- (\dpi-0.8,1);
        \draw[red!50!orange,ultra thick,opacity=0.7] (A) -- (\dpi+0.8,1);
        \draw[gray, dotted] (1,0) -- (1,1.8);
        \draw[Tan,very thick,dashed] (0.8,0.8) -- (1.8,1.8);
        \node[Tan!50!black] at (2.1, 1.7) {$\mbox{\footnotesize $y = x$}$};
        \node at (0, -0.2) {$\mbox{\footnotesize $0$}$};
        \node at (1, -0.2) {$\mbox{\footnotesize $1$}$};
        \node at (-0.14, 1) {$\mbox{\footnotesize $1$}$};
         \node at (\dpi, -0.2) {$\mbox{\footnotesize $\frac{\pi}{2}$}$};
        \node at (2*\dpi, -0.2) {$\mbox{\footnotesize $\pi$}$};
        \fill[Goldenrod] (A) circle (1.2pt);
        \draw[black] (A) circle (1.2pt);
    \end{scope}
    \end{tikzpicture}
...
\begin{figure}[htbp]
    ...
    \begin{tikzpicture}[scale=0.8]
        ...
        \def\ydn{1.8}   % 单元格高度
        \def\pad{0.2}
        \draw[black, thick] (0, 0) -- (\wzuo + \wyou, 0);
        \draw[black, thick] (0, \htop) -- (\wzuo + \wyou, \htop);
        \draw[black, thick] (0, -\hbot) -- (\wzuo + \wyou, -\hbot);
        \draw[black, thick] (\wzuo, \htop) -- (\wzuo, -\hbot);
        \node[black] at (0.5*\wzuo, 0.5*\htop) {$x$};
        \node[black] at (0.5*\wzuo, -0.5*\hbot) {$\sin{(x)}$};
        \node[black] at (\wzuo+\wgap, 0.5*\htop) {$-\pi$};
        \node[black] at (\wzuo+\wgap+0.3*\wyou - 0.3*\wgap, 0.5*\htop) {$-\frac{\pi}{2}$};
        \node[black] at (\wzuo+\wgap+0.6*\wyou-0.6*\wgap, 0.5*\htop) {$\frac{\pi}{2}$};
        \node[black] at (\wzuo+\wgap+0.9*\wyou-0.9*\wgap, 0.5*\htop) {$\pi$};
        \node[black] at (\wzuo+\wgap, -\yup) {$0$};
        \node[black] at (\wzuo+\wgap+0.3*\wyou - 0.3*\wgap, -\ydn) {$-1$};
        \node[black] at (\wzuo+\wgap+0.6*\wyou-0.6*\wgap, -\yup) {$1$};
        \node[black] at (\wzuo+\wgap+0.9*\wyou-0.9*\wgap, -\ydn) {$0$};
        \draw[-Stealth,thick] (\wzuo+\wgap+\pad,-\yup) -- (\wzuo+\wgap+0.3*\wyou - 0.3*\wgap-2*\pad, -\ydn+\pad);
        \draw[-Stealth,thick] (\wzuo+\wgap+0.3*\wyou - 0.3*\wgap + 2*\pad, -\ydn+\pad) -- (\wzuo+\wgap+0.6*\wyou - 0.6*\wgap-\pad, -\yup);
        \draw[-Stealth,thick] (\wzuo+\wgap+0.6*\wyou - 0.6*\wgap+\pad, -\yup) -- (\wzuo+\wgap+0.9*\wyou - 0.9*\wgap-\pad, -\ydn);
    \end{tikzpicture}
\end{figure}
...
\begin{figure}[htbp]
    ...
    \begin{tikzpicture}[scale=0.8]
        ...
        \draw[black, thick] (\wzuo, \htop) -- (\wzuo, -\hbot);
        \node[black] at (0.5*\wzuo, 0.5*\htop) {$x$};
        \node[black] at (0.5*\wzuo, -0.5*\hbot) {$\cos{(x)}$};
        \node[black] at (\wzuo+\wgap, 0.5*\htop) {$-\pi$};
        \node[black] at (\wzuo+\wgap+0.45*\wyou - 0.45*\wgap, 0.5*\htop) {$0$};
        \node[black] at (\wzuo+\wgap+0.9*\wyou-0.9*\wgap, 0.5*\htop) {$\pi$};
        \node[black] at (\wzuo+\wgap, -\ydn) {$-1$};
        \node[black] at (\wzuo+\wgap+0.45*\wyou-0.45*\wgap, -\yup) {$1$};
        \node[black] at (\wzuo+\wgap+0.9*\wyou-0.9*\wgap, -\ydn) {$-1$};
        \draw[-Stealth,thick] (\wzuo+\wgap + 2*\pad, -\ydn+\pad) -- (\wzuo+\wgap+0.45*\wyou - 0.45*\wgap-\pad, -\yup);
        \draw[-Stealth,thick] (\wzuo+\wgap+0.45*\wyou - 0.45*\wgap+\pad, -\yup) -- (\wzuo+\wgap+0.9*\wyou - 0.9*\wgap-2*\pad, -\ydn);
    \end{tikzpicture}
\end{figure}
...
另一方面，复合函数$f\circ g$为$x\mapsto \sqrt{2x - 1}$。由于$g$的定义域和值域都是$\mathbb{R}$，而$f$的定义域是$[\,0; +\infty)$，我们需要缩小$g$的定义域，使得$g$的值域落在$f$的定义域中。也就是说，我们把$g$的定义域缩小到集合：$\{x \,|\, 2x - 1 \in [\,0;+\infty)\}$。
...
\begin{figure}[h] %this figure will be at the right
    ...
    \begin{scope}[shift={(0,0)}]
        ...
        \def\cf{0.08};
        % 坐标轴（带箭头）
        \foreach \k in {-1.5,-1,-0.5,0.5,1,...,3} { % ymin -> ymax
            \draw[help lines,gray!60] (\xmin,\k) -- (\xmax,\k) ;
        }
        \foreach \k in {-2,-1.5,-1,-0.5,0.5,1,...,3.5} { % xmin -> xmax
            \draw[help lines,gray!60] (\k,\ymin) -- (\k,\ymax);
        }
        \draw[-Stealth, thick] (\xmin, 0) -- (\xmax,0) node[above] {$x$};
        \draw[-Stealth, thick] (0,\ymin) -- (0,\ymax) node[right] {$y$};
        \foreach \k in {0.5,1,...,3} { 
            \node at (-\ca,\k) {$\mbox{\tiny $\k$}$};
            \node at (\k,-\ca) {$\mbox{\tiny $\k$}$};
        }
        \node at (3.5,-\ca) {$\mbox{\tiny $3.5$}$};
        \foreach \k in {-1.5,-1,-0.5} {  % ,
            \node at (\k-\cf,-\ca) {$\mbox{\tiny $\k$}$};
            \node at (-\ca-\cf,\k) {$\mbox{\tiny $\k$}$};
        }
        \node at (-2-\cf,-\ca) {$\mbox{\tiny $-2$}$};
        % 作图
        \draw[blue, thick, domain=-1.78:1.78, samples=200]
        plot (\x,{\x*\x});
        \draw[OliveGreen, thick, domain=0:4, samples=200]
        plot (\x,{sqrt(\x)});
        \draw[very thick, fill=white] (1.2,-1.8) rectangle (3.4,-0.4);
        \draw[blue,ultra thick] (1.45, -0.8) -- (1.85, -0.8);
        \draw[OliveGreen,ultra thick] (1.45, -1.4) -- (1.85, -1.4);
        \node at (2.55, -0.8) {$\mbox{\small $\displaystyle y = x^2$}$};
        \node at (2.55, -1.4) {$\mbox{\small $\displaystyle y = \sqrt{x}$}$};
    \end{scope}
  \end{tikzpicture}
...
我们说这样的$g$是$f$的\textbf{左逆映射}。$f$把某个元素$x$对应到目标函数$y$，而$g$把$f$的目标元素$y$“逆转”为原来的$x$。
...
\begin{figure}[h] %this figure will be at the right
    ...
    \begin{scope}[shift={(0,0)}]
        ...
        \node at (-2-\cf,-\ca) {$\mbox{\tiny $-2$}$};
        % 作图
        \draw[blue, thick, domain=-2.05:2.05, samples=200]
        plot (\x,{\x*\x - 1});
        \draw[OliveGreen, thick, domain=-1:4, samples=200]
        plot (\x,{sqrt(\x+1)});
        \draw[dashed, gray, thick] (-1.9, -1.9) -- (3.2, 3.2);
        \draw[very thick, fill=white] (1.2,-1.8) rectangle (3.8,-0.4);
        \draw[blue,ultra thick] (1.45, -0.8) -- (1.85, -0.8);
        \draw[OliveGreen,ultra thick] (1.45, -1.4) -- (1.85, -1.4);
        \node at (2.8, -0.8) {$\mbox{\small $\displaystyle y = x^2-1$}$};
        \node at (2.8, -1.4) {$\mbox{\small $\displaystyle y = \sqrt{x+1}$}$};
    \end{scope}
    \end{tikzpicture}
...
\begin{so}
    ...
    $r$为偶数时，$f$在$\mathbb{R}$上不为单射，我们只考虑区间$[0, +\infty)$的情况。按照幂函数的定义，
    在我们考虑的范围内，$g: x\mapsto x^{\frac{1}{r}}$使得
    \begin{align*}
        \forall x > 0, \;\,\, g(f(x)) &= \left(x^r\right)^{\frac{1}{r}} = x  \\
        \forall x > 0, \;\,\, f(g(x)) &= \left(x^{\frac{1}{r}}\right)^{r} = x  
    \end{align*} 
    因此，$g$是$f$的反函数。\\  
    再来看$r$为有理数的情况。设$r = \frac{p}{q}$，其中$p,q$为互素的整数，则$\frac{1}{r} = \frac{q}{p}$。$f$在区间$[\,0;+\infty)$上严格单调，我们只考虑$[\,0;+\infty)$的情况。
    \begin{align*}
        \forall \, x > 0, \quad \left(x^r\right)^{\frac{1}{r}} &= \left(x^{\frac{p}{q}}\right)^{\frac{q}{p}} = \left(\left(\left(x^{p}\right)^\frac{1}{q}\right)^q\right)^\frac{1}{p}  \\
    ...
    \end{align*} 
    这说明，$g$是$f$的反函数。\\  
    \indent 3. $f$在其定义域内不是单射，我们只考虑区间$[\,0;1\,]$的情况。
...
\begin{xt}
    ...
    \indent 2. 求以下函数的反函数：\\
    \indent 2.1. $\forall x\in \mathbb{R}, \quad x\mapsto x^3$ \\
    \indent 2.2. $\forall x\in \mathbb{R}^+, \quad x\mapsto \frac{1}{x + 1}$ \\
    \indent 2.3. $\forall x\in [\,0;1\,], \quad x\mapsto \sqrt{1 - x^3} $
\end{xt}
...
\begin{figure}[h] %this figure will be at the right
    ...
    \begin{axis}[
        axis lines = middle,
        xlabel = $x$, ylabel = $y$,
        xmin = -3*pi/4-0.1, xmax = 3*pi/4+0.5,
        ymin = -2*pi/3, ymax = 2*pi/3,
        samples = 250, % 增加采样点以保证 arcsin 端点平滑
        width=11cm, height=10cm,
        % x 轴刻度：严格按 π/4 单位设置
        xtick = {-3*pi/4, -pi/2, -pi/4, 0, pi/4, pi/2, 3*pi/4},
        xticklabels = {$-\frac{3\pi}{4}$, $-\frac{\pi}{2}$, $-\frac{\pi}{4}$, $0$,
                      $\frac{\pi}{4}$, $\frac{\pi}{2}$, $\frac{3\pi}{4}$},
        % y 轴刻度：严格按 π/4 单位设置（自动截断至可视范围）
        ytick = {-pi/2, -pi/4, 0, pi/4, pi/2},
        yticklabels = {$-\frac{\pi}{2}$, $-\frac{\pi}{4}$, $0$,
                      $\frac{\pi}{4}$, $\frac{\pi}{2}$},
        grid = both,
        grid style = {dashed, OrangeRed!60},
        legend style = {at={(0.98,0.2)}, anchor=north east, cells={anchor=west}},
        tick label style = {font=\small,color=BrickRed}
    ]

    \pgfplotsinvokeforeach{1,2}{
      \draw[Cyan!60,dashed] (axis cs:#1, -2*pi/3) -- (axis cs:#1, 2*pi/3);
      \draw[Cyan!60,dashed] (axis cs:-#1, -2*pi/3) -- (axis cs:-#1, 2*pi/3);
      \draw[Cyan!60,dashed] (axis cs:-3*pi/4-0.1,#1) -- (axis cs:3*pi/4+0.5, #1);
      \draw[Cyan!60,dashed] (axis cs:-3*pi/4-0.1,-#1) -- (axis cs:3*pi/4+0.5, -#1);
      \node[RoyalBlue,above,font=\footnotesize] at (axis cs:#1,-0.05) {$#1$};
      \node[RoyalBlue,above,font=\footnotesize] at (axis cs:-#1-0.05,-0.05) {$-#1$};
      \node[RoyalBlue,left,font=\footnotesize] at (axis cs:-0.02,#1) {$#1$};
      \node[RoyalBlue,left,font=\footnotesize] at (axis cs:-0.06,-#1) {$-#1$};
    }

    % 正弦函数：仅绘制 [-π/2, π/2] 区间（弧度输入需转换为度数）
    \addplot [
        blue, very thick, smooth,
        domain = -pi/2:pi/2
    ] {sin(deg(x))};
    \addlegendentry{$y = \underline{\sin} (x)$}

    % 反正弦函数：定义域 [-1,1]，输出需从度数转为弧度
    \addplot [
        red, very thick, smooth,
        domain = -1:1
    ] {asin(x)/deg(1)};
    \addlegendentry{$y = \arcsin (x)$}

    \addplot [
        blue, very thick, smooth, dashed,
        domain = -3*pi/4-0.1:-pi/2
    ] {sin(deg(x))};
    
    \addplot [
        blue, very thick, smooth, dashed,
        domain = pi/2:3*pi/4+0.5
    ] {sin(deg(x))};
    \addplot [Fuchsia!60, dashed, thick, domain=-2*pi/3:2*pi/3] {x};
...
从图像中可以看出，反正弦函数也是严格单调递增的函数。它的定义域是$[\,-1; 1\,]$、值域是$\left[\,-\frac{\pi}{2}; \frac{\pi}{2}\,\right]$。
$\underline{\sin}$是奇函数，因此，反正弦函数也是奇函数。$x > 0$时，反正弦函数
总在直线$l$上方，增长越来越快；
$x < 0$时，反正弦函数总在直线$l$下方，增长越来越慢。

反正弦函数的定义域是正弦函数的值域。因此，$\sin{(\arcsin{x})} = x$对$[\,-1; 1\,]$中的数总成立。
但反正弦函数的值域只是正弦函数的定义域的真子集，因此，$\arcsin{(\sin{x})} = x$只对$\left[\,-\frac{\pi}{2}; \frac{\pi}{2}\,\right]$中的数成立。
...
\begin{figure}[h] %this figure will be at the right
    ...
    \begin{axis}[
        axis lines = middle,
        xlabel = $x$, ylabel = $y$,
        xmin = -pi/2-0.1, xmax = pi+0.5,
        ymin = -pi/2, ymax = pi+0.3,
        samples = 250, % 增加采样点以保证 arcsin 端点平滑
        width=11cm, height=10cm,
        % x 轴刻度：严格按 π/4 单位设置
        ...
        % y 轴刻度：严格按 π/4 单位设置（自动截断至可视范围）
        ytick = {-pi/4, 0, pi/4, pi/2, 3*pi/4, pi},
        yticklabels = {$-\frac{\pi}{4}$, $0$,
                      $\frac{\pi}{4}$, $\frac{\pi}{2}$,$\frac{3\pi}{4}$, $\pi$},
        grid = both,
        grid style = {dashed, OrangeRed!60},
        legend style = {at={(0.85,0.96)}, anchor=north east, cells={anchor=west}},
        tick label style = {font=\small,color=BrickRed}
    ]

    \pgfplotsinvokeforeach{-1,1,2,3}{
      \draw[Cyan!60,dashed] (axis cs:#1, -pi/2) -- (axis cs:#1, pi+0.3);
      \draw[Cyan!60,dashed] (axis cs:-pi/2-0.1,#1) -- (axis cs:pi+0.5, #1);
      \node[RoyalBlue,left,font=\footnotesize] at (axis cs:-0.02,#1) {$#1$};
    }
    \pgfplotsinvokeforeach{1,2,3}{
      \node[RoyalBlue,below,font=\footnotesize] at (axis cs:#1,-0.05) {$#1$};
    }
    \node[RoyalBlue,below,font=\footnotesize] at (-1.075,-0.05) {$-1$};

    \addplot [
        blue, very thick, smooth,
        domain = 0:pi
    ] {cos(deg(x))};
    \addlegendentry{$y = \underline{\cos} (x)$}
    ...
    % 可选：添加对称轴 y=x 以直观展示反函数关系（灰色虚线）
    \addplot [Fuchsia!60, dashed, thick, domain=-2*pi/3:pi+0.3] {x};
...
\begin{figure}[h] %this figure will be at the right
    ...
    \begin{axis}[
        axis lines = middle,
        xlabel = $x$, ylabel = $y$,
        xmin = -5*pi/2-0.5, xmax = 5*pi/2+1,
        ymin = -2*pi-0.3, ymax = 2*pi+0.3,
        samples = 300, % 增加采样点以保证 arcsin 端点平滑
        width=12.5cm, height=9.56cm,
        % x 轴刻度：严格按 π/4 单位设置
        xtick = {-5*pi/2, -2*pi, -3*pi/2, -pi, -pi/2, 0, pi/2, pi, 3*pi/2, 2*pi, 5*pi/2},
        ...
        grid = both,
        grid style = {dashed, gray!30},
        legend style = {at={(0.85,0.16)}, anchor=north east, cells={anchor=west}},
        tick label style = {font=\small,color=BrickRed}
    ]
    ...
    \addlegendentry{$y = \underline{\tan} (x)$}

    \addplot [
        red, very thick, smooth,
        domain = -5*pi/2-0.5:5*pi/2+0.5
    ] {atan(x)/deg(1)};
    ...
    \addplot [
        blue, smooth, dashed,
        domain = -5*pi/2-0.5:-5*pi/2-0.1
    ] {tan(deg(x))};
    \addplot [
        blue, smooth, dashed,
        domain = 5*pi/2+0.1:5*pi/2+1
    ] {tan(deg(x))};

    \addplot [Fuchsia!60, dashed, thick, domain=-2*pi-0.3:2*pi+0.3] {x};
...
\begin{figure}[h] %this figure will be at the right
    ...
    \begin{axis}[
        axis lines = middle,
        xlabel = $x$, ylabel = $y$,
        xmin = -2*pi-0.5, xmax = 3*pi+1,
        ymin = -2*pi-0.3, ymax = 2*pi+0.3,
        samples = 300, % 增加采样点以保证 arcsin 端点平滑
        width=12.5cm, height=9.56cm,
        % x 轴刻度：严格按 π/4 单位设置
        xtick = {-2*pi, -3*pi/2, -pi, -pi/2, 0, pi/2, pi, 3*pi/2, 2*pi, 5*pi/2, 3*pi},
        xticklabels = {$-2\pi$, $-\frac{3\pi}{2}$,$-\pi$, $-\frac{\pi}{2}$, $0$, $\frac{\pi}{2}$,$\pi$, $\frac{3\pi}{2}$, $2\pi$, $\frac{5\pi}{2}$,$3\pi$},
        % y 轴刻度：严格按 π/4 单位设置（自动截断至可视范围）
        ytick = {-2*pi, -3*pi/2, -pi, -pi/2, 0, pi/2, pi, 3*pi/2, 2*pi},
        yticklabels = {$-2\pi$, $-\frac{3\pi}{2}$,$-\pi$, $-\frac{\pi}{2}$, $0$, $\frac{\pi}{2}$,$\pi$, $\frac{3\pi}{2}$, $2\pi$},
        grid = both,
        grid style = {dashed, gray!30},
        legend style = {at={(0.92,0.16)}, anchor=north east, cells={anchor=west}},
        tick label style = {font=\small,color=BrickRed}
    ]
    ...
    \addlegendentry{$y = \underline{\cot} (x)$}

    \addplot [
        red, very thick, smooth,
        domain = -2*pi-0.5:3*pi+0.7
    ] {-atan(x)/deg(1)+pi/2};
    ...
    \addplot [
        blue, smooth, dashed,
        domain = 3*pi+0.1:3*pi+1
    ] {tan(90-deg(x))};

    \addplot [Fuchsia!60, dashed, thick, domain=-2*pi-0.3:2*pi+0.4] {x};
...
$$\lian{n\to\infty}\, a_n = x \quad \mbox{或} \quad \{a_n\} \rightarrow x. $$
...
\begin{figure}[h] %this figure will be at the right
    ...
    \begin{scope}[shift={(0,0)}]
        ...
        \def\r{0.15};
        % 坐标轴（带箭头）
        \draw[-Stealth, thick] (\xmin, 0) -- (\xmax,0) node[above] {$n$};
        \draw[-Stealth, thick] (0,\ymin) -- (0,\ymax) node[right] {$a_n$};

        % 刻度与数字
        \foreach \x in {5,10,15,20,30,40} {
            \draw (0.5*\x,0.1) -- (0.5*\x,-0.1); 
            \node[black] at (0.5*\x, -0.35) {$\mbox{\scriptsize $\x$}$};
        }

        % 作图          
        % 散点数据
        \def\points{
          (0.5,5.42),(1.0,2.04),(1.5,4.81),(2.0,4.83),(2.5,0.98),
          (3.0,3.96),(3.5,4.11),(4.0,3.49),(4.5,3.98),(5.0,4.15),
          (5.5,3.88),(6.0,3.13),(6.5,4.21),(7.0,3.99),(7.5,4.08),
          (8.0,4.24),(8.5,3.90),(9.0,3.95),(9.5,3.97),(10.0,3.81),
          (10.5,3.95),(11.0,4.01),(11.5,3.97),(12.0,4.04),(12.5,4.08),
          (13.0,4.03),(13.5,4.05),(14.0,4.05),(14.5,3.98),(15.0,3.98),
          (15.5,3.98),(16.0,3.94),(16.5,3.96),(17.0,3.99),(17.5,3.98),
          (18.0,4.01),(18.5,4.04),(19.0,4.03),(19.5,4.04),(20.0,4.04),
        }

        \fill[CadetBlue!40,opacity=0.3] (10.7,\x-3*\r) rectangle (20.5,\x+3*\r);
        
        % 绘制散点
        \foreach \p in \points {
          \fill[BrickRed, draw=black] \p circle (2pt);
        }
        \draw[red,dotted] (0,\x) -- (\xmax,\x);
        \draw[Blue,dashed] (0,\x-\r) -- (\xmax,\x-\r);
        \draw[Blue,dashed] (0,\x+\r) -- (\xmax,\x+\r);
        \node[red] at (-1.5, \x) {$\mbox{\footnotesize $x$}$};
        \node[Blue] at (-1.5, \x+1) {$\mbox{\footnotesize $x+r$}$};
        \node[Blue] at (-1.5, \x-1) {$\mbox{\footnotesize $x-r$}$};
        \draw[-Stealth,red] (-1.1, \x) -- (-0.1, \x);
        \draw[-Stealth,Blue] (-0.8, \x+0.75) -- (-0.1, \x+\r+0.04);
        \draw[-Stealth,Blue] (-0.8, \x-0.75) -- (-0.1, \x-\r-0.04);
    \end{scope}
    \end{tikzpicture}
...
\begin{et}
数列$\{a_n\}$的通项是$\displaystyle a_n = \frac{1}{n^2}$，它是否有极限？如果有极限，极限是多少？    
\end{et}
\begin{so}
    $\{a_n\}$每项都是正数。
...
\begin{wrapfigure}[9]{r}{0.33\textwidth} %this figure will be at the right
    ...
    \begin{scope}[shift={(0,0)}]
        ...
        % 散点数据
        \def\points{
          (0.33,0.29),(0.67,0.49),(1.00,0.91),(1.33,1.57),(1.67,1.21),
          (2.00,1.80),(2.33,2.65),(2.67,2.56),(3.00,3.06),(3.33,3.80),
          (3.67,3.91),(4.00,4.35),(4.33,4.50),(4.67,4.97),(5.00,5.63)
        }
        ...
        \def\npoints{
          (0.33,-0.58),(0.67,-0.82),(1.00,-1.01),(1.33,-1.56),(1.67,-1.15),
          (2.00,-1.79),(2.33,-2.03),(2.67,-2.76),(3.00,-2.73),(3.33,-3.49),
          (3.67,-4.12),(4.00,-3.77),(4.33,-3.89),(4.67,-4.90),(5.00,-5.52)
        }
        \foreach \p in \npoints {
          \fill[CadetBlue, draw=black] \p circle (2pt);
        }
    \end{scope}
    \end{tikzpicture}
...
\begin{et}
    设数列$\left\{\frac{1}{n}\right\}_{n\in\mathbb{Z}^+}$的部分和数列为$\{a_n\}$，证明：$\{a_n\}$趋于正无穷大。
\end{et}
\begin{proof}
    按照定义，$ \forall \, n\in\mathbb{Z}^+,\,\,\,a_n = 1 + \frac{1}{2} + \cdots + \frac{1}{n}$。作差：$a_{n+1} - a_n = \frac{1}{n+1} > 0$，所以$\{a_n\}$单调递增。\\
...
\begin{tm}
    ...
    \end{align*}
\end{tm}
特别地，令$\{b_n\}$是常数列，就得到数乘对极限的影响：
$$ \forall \,\, t \in \mathbb{R}, \;\,\,  \lim_{n\to\infty} t \cdot a_n = t \cdot \lim_{n\to\infty} a_n. $$
\begin{proof}
    \mbox{} \\
    ...
    如果$t > 0$，按照定义，对$\forall r > 0$，由于$\frac{r}{t} > 0$，总有正整数$n$使得
    ...
    \begin{align*}
        \forall \,\, m \geqslant n_a, \;\,\, |a_m - a| & \leqslant r_1,  \\
        \forall \,\, m \geqslant n_b, \;\,\, | b_m - b| & \leqslant r_1,  
    \end{align*}
    因此，
    \begin{align*}
        \mbox{}\quad\forall \,\, m \geqslant n_a + n_b, \;\,\, |(a_m - a)(b_m - b) | \leqslant r_1^2.  
    \end{align*}
    按定义$r_1\leqslant 1$，所以$r_1^2 \leqslant r$。这说明数列$\{(a_n - a)(b_n - b)\}$趋于$0$。而$\{b\cdot a_n\}$和$\{a \cdot b_n\}$都趋于$ab$，
    常数列$\{ab\}$也趋于$ab$，所以根据前面证明的极限加减法，数列
    $$\{a_nb_n\} = \{(a_n - a)(b_n - b)\} + \{b\cdot a_n\} + \{a \cdot b_n\} - \{ab\}$$
    趋于$0 + ab + ab - ab = ab$。
\end{proof}

加法、减法、乘法都可以对数列的极限做运算。那么除法是否可以呢？
具体来说，若数列$\{a_n\}$趋于$a$，$\{b_n\}$趋于$b$，$\{\frac{a_n}{b_n}\}$是否趋于$\frac{a}{b}$呢？

显然，$b=0$的时候，$\frac{a}{b}$无定义，所以先考虑$b$不等于$0$的情况。当然，$b_n$有可能是$0$。但既然极限$b\neq 0$，那么按定义，当$n$足够大时，$b_n$足够接近$b$，比如介于$b$和$\frac{b}{2}$之间。这时的$b_n$就不再为$0$了。于是我们可以只对足够大的$n$定义$\frac{1}{b_n}$。比如，$\forall \; \frac{|b|}{2}>r>0$，有正整数$N$使得$n>N$时总有$|b_n - b| < r$，这时$|b_n| > ||b| - r| > \frac{|b|}{2}$。于是：
$$ \forall\; n>N, \,\;\, \left| \frac{1}{b_n} - \frac{1}{b} \right| = \frac{|b_n - b|}{|b|\cdot|b_n|} < \frac{|b_n - b|}{|b|\cdot\frac{|b|}{2}} < \frac{2r}{b^2}.$$
这说明$\{\frac{1}{b_n}\}$趋于$\frac{1}{b}$。$\{\frac{a_n}{b_n}\}$可以看作$\{a_n \cdot \frac{1}{b_n}\}$，因而趋于$\frac{a}{b}$。

如果$b=0$，即数列$\{b_n\}$为无穷小，需要考虑的问题就更多了。同样先考虑倒数数列$\left\{\frac{1}{b_n}\right\}$。为了方便讨论，我们不妨设$\{b_n\}$中没有$0$。正数列$\{|b_n|\}$趋于$0$，因此$\left\{\left|\frac{1}{b_n}\right|\right\}$趋于正无穷大。问题在于$b_n$在$0$附近，所以可正可负。于是，我们对$\{b_n\}$再加一个限制：从某一项开始，$\{b_n\}$的项正负号不变，比如总大于$0$。这时必有$\left\{\frac{1}{b_n}\right\}$趋于正无穷。如果$\{a_n\}$的极限不是$0$，那么$n$足够大时$a_n$的正负号稳定不变，于是$\frac{a_n}{b_n}$的正负号也不变。这时$\{\frac{a_n}{b_n}\}$趋于无穷大，正负号取决于$n$足够大时通项的正负号。

最后考虑$\{a_n\}$也趋于$0$的情况。这时，$\{\frac{a_n}{b_n}\}$的大体行为取决于$a_n$、$b_n$之间的大小关系，不再是可以统一处理的问题。我们把这种情况下的$\{\frac{a_n}{b_n}\}$称为关于数列$\{a_n\}$和$\{b_n\}$的\textbf{不定式}。不定式是否有极限，极限是什么，我们将在以后具体分析。
...
$$ (a,\;\, b) \leftrightharpoons (c, \;\, d)\quad \mbox{当且仅当} \quad a\cdot d = b \cdot c.$$
可以证明，二元关系$\leftrightharpoons$是等价关系：
\begin{enumerate}
    \item $\forall \,\, a\in \mathbb{Z}, \;\, b\in\mathbb{Z}^+, \;\,\, (a,\;\, b) \leftrightharpoons (a, \,\, b)$。
    \item $\forall \,\, a,c\in \mathbb{Z}, \,\, b,d\in\mathbb{Z}^+$，如果$(a,\;\, b) \leftrightharpoons (c, \;\, d)$，则$ (c, \;\, d)\leftrightharpoons (a,\;\, b)$。
    \item $\forall \,\, a,c,e\in \mathbb{Z}, \;\, b,d,f\in\mathbb{Z}^+$，如果$(a,\;\, b) \leftrightharpoons (c, \;\, d)$、$ (c, \;\, d)\leftrightharpoons (e,\;\, f)$，则$(a,\;\, b) \leftrightharpoons (e,\;\, f)$。
\end{enumerate}
...
\begin{so}
    ...
    我们可以取$N = q$。由于整数$p > 0$，总有$p \geqslant 1$，即$r = \frac{p}{q} \geqslant \frac{1}{N}$。因此，当$n\geqslant m>N$的时候，总有
...
\begin{so}
    ...
    \begin{align*}
        a_n - 1 &= \left(\frac12\right)^{n-1}\cdot (a_1 - 1) = \frac{1}{2^{n-1}}  \\
        a_n &= 1 + \frac{1}{2^{n-1}} 
    \end{align*}
...
\begin{po}\textbf{实数的根本性质}\\
    任何自敛的有理数列总有极限，我们定义实数为自敛有理数列的极限。
\end{po}
...
这个等价关系把每个实数对应到一个等价类，由所有趋于这个实数的自敛有理数列构成。
...
\begin{enumerate}
    ...
    \item $\forall a, b$，$a < b$，$b <a$，$a = b$恰有一个成立。
...
\begin{tm}
    ...
    \end{itemize}  
\end{tm}
可以证明，这样定义的四则运算，满足加法结合律、乘法结合律、加法交换律、乘法交换律，以及乘法对加法的分配律，并且与实数的相等、不等关系兼容（具体证明见附录A第二节）。
...
\begin{tm}
    自敛的实数列总有极限，极限为实数。
\end{tm}
而另一方面，有极限的数列总是自敛的。所以，\textbf{实数列如果有极限，总是实数}。实数不仅是任意自敛有理数列的极限，也是任意自敛实数列的极限。
我们把这个性质称为\textbf{实数的完备性}或\textbf{致密性}，或者说实数集对于极限操作是\textbf{完备}的，不会像自敛有理数列那样出现极限不在数集里的问题。
...
\begin{sk}
    ...
    2. 如果我们把趋于无穷大的数列也视为有“极限”，能否构造包含无穷大的“扩展实数集”？这会带来哪些便利与问题？
\end{sk}
...
\begin{xt}
    ...
    \indent 1.2. 证明：存在正整数$n_1$，使得$n>n_1$时总有$|v_n| \leqslant 2r$。\\
...
\begin{figure}[h]
    ...
    \begin{scope}[shift={(0,0)}]
        ...
        \def\M{6.4}
        % 坐标轴（带箭头）
        \draw[-Stealth] (\xmin, \ymin) -- (\xmax+0.5,\ymin);
        \draw[-Stealth] (0,\ymin) -- (0,\ymax);
        \node at (\xmax+0.8, \ymin) {$\mbox{\footnotesize $n$}$};
        % 作图          
        % 散点数据
        \def\points{
          (0.20,0.0),(0.40,0.63),(0.60,1.08),(0.80,1.40),(1.00,1.65),
          (1.20,1.86),(1.40,2.03),(1.60,2.19),(1.80,2.32),(2.00,2.44),
          (2.20,2.54),(2.40,2.64),(2.60,2.73),(2.80,2.81),(3.00,2.89),
          (3.20,2.96),(3.40,3.03),(3.60,3.09),(3.80,3.15),(4.00,3.21),
          (4.20,3.26),(4.40,3.31),(4.60,3.36),(4.80,3.41),(5.00,3.45),
          (5.20,3.50),(5.40,3.54),(5.60,3.58),(5.80,3.62),(6.00,3.65),
          (6.20,3.69),(6.40,3.72),(6.60,3.76),(6.80,3.79),(7.00,3.82),
          (7.20,3.85),(7.40,3.88),(7.60,3.91),(7.80,3.94),(8.00,3.97),
          (8.20,4.00),(8.40,4.02),(8.60,4.05),(8.80,4.07),(9.00,4.10),
          (9.20,4.12),(9.40,4.14),(9.60,4.17),(9.80,4.19),(10.00,4.21),
          (10.20,4.23),(10.40,4.26),(10.60,4.28),(10.80,4.30),(11.00,4.32),
          (11.20,4.34),(11.40,4.36),(11.60,4.37),(11.80,4.39),(12.00,4.41),
          (12.20,4.43),(12.40,4.45),(12.60,4.46),(12.80,4.48),(13.00,4.50),
          (13.20,4.52),(13.40,4.53),(13.60,4.55),(13.80,4.56),(14.00,4.58),
          (14.20,4.59),(14.40,4.61),(14.60,4.62),(14.80,4.64),(15.00,4.65),
          (15.20,4.67),(15.40,4.68),(15.60,4.70),(15.80,4.71),(16.00,4.72),
          (16.20,4.74),(16.40,4.75),(16.60,4.76),(16.80,4.78),(17.00,4.79)
        }

        \def\afterpoints{
          (17.40,4.82),(17.60,4.83),(17.80,4.84),(18.00,4.85),(18.20,4.86),
          (18.40,4.88),(18.60,4.89),(18.80,4.90),(19.00,4.91),(19.20,4.92),
          (19.40,4.93),(19.60,4.95),(19.80,4.958),(20.00,4.964),(20.20,4.97)
        }
        
        % 刻度与数字
        \foreach \x in {5,10,20,40,...,100} {
            \draw (0.2*\x,\ymin+0.1) -- (0.2*\x,\ymin-0.1); 
            \node[black] at (0.2*\x, \ymin-0.35) {$\mbox{\tiny $\x$}$};
        }
        \draw[Magenta,very thick] (0,0) -- (\xmax,0);
        \foreach \k in {1,2,...,11,14,15} {
          \draw[dotted,thick, gray!50] (0,0.4*\k) -- (\xmax,0.4*\k);
        }
        \draw[OliveGreen!70!black,very thick] (0,\M) -- (\xmax,\M);
        \draw[dashed, Red, thick] (0,4.8) -- (\xmax,4.8);
        \draw[dashed, ForestGreen, thick] (0,5.2) -- (\xmax,5.2);
        % 绘制散点
        \foreach \p in \points {
          \fill[white, draw=blue] \p circle (2pt);
        }
        \foreach \p in \afterpoints {
          \fill[Goldenrod, draw=BrickRed] \p circle (2pt);
        }
        \fill[Red, draw=BrickRed] (17.20,4.80) circle (2.1pt); 
        \node[OliveGreen!70!black] at (-0.5, \M) {$\mbox{\footnotesize $M$}$};
        \node[OliveGreen!70!black] at (\xmax+0.6, \M) {$\mbox{\footnotesize $y_k$}$};
        \node[Magenta] at (-0.5, 0) {$\mbox{\footnotesize $a_1$}$};
        \node[Magenta] at (\xmax+0.6, 0) {$\mbox{\footnotesize $y_0$}$};
        \node[Red] at (-0.5, 4.8) {$\mbox{\footnotesize $y_{i}$}$};
        \node[ForestGreen!70!black] at (\xmax+0.89, 5.2) {$\mbox{\footnotesize $y_{i+1}$}$};
    \end{scope}
    \end{tikzpicture}
\end{figure}
...
记$y_i=a_1 + i r_k$是“往$M$走”第$i$步达到的值，$y_0=a_1$，$y_k=M$。我们的结论就可以写成：总有某个$i$，$y_i$不是上界，而在下一步，$y_{i+1}=a_1 + (i+1) r_k$是上界。这说明数列中有$a_{N(k)}$大于等于$y_i$，
却小于$y_{i+1}$。由于数列单调递增，$\forall n\geqslant N(k)$，就总有$y_i\leqslant a_n\leqslant y_{i+1}$。换句话说，取$\forall m,n\geqslant N(k)$，则$y_i\leqslant a_m\leqslant y_{i+1}$，$y_i\leqslant a_n\leqslant y_{i+1}$。既然如此，$a_n$与$a_m$的距离自然不大于$y_{i}$与$y_{i+1}$的距离。也就是说，自$a_{N(k)}$开始，数列任两项之间的距离都小于等于$y_i$与$y_{i+1}$的距离，也就是$r_k$。
...
综上所述，单调递增数列要么趋于正无穷大，要么有极限。类似可以证明：单调递减数列要么趋于负无穷大，要么有极限。合起来说就是：
...
确界的概念可以推广到一般的集合。给定非空的数集$S$，$S$的上（下）确界就是它的上（下）界集合中的最小（大）元素，分别记作$ \text{\texttt{上}}(S)$和$\text{\texttt{下}}(S)$。
可以证明（见附录$B.1$）：
...
我们说点的运动产生了线。具体来说，设$\gamma$是连接两点$A$、$B$的曲线。定义$\gamma$为实数到平面中点的映射，把闭区间$[\,0;1\,]$中的数映射到平面中的点。$0$对应起点$A$，$1$对应终点$B$。

来看最基本的例子。以下映射：
$$ \gamma : \;\,\, t \mapsto (\cos{2\pi t}, \,\, \sin{2\pi t})$$
描述了一个圆。它的起点$\gamma(0)$是$(1,0)$，终点$\gamma(1)$也是$(1,0)$。

直线段的长度对应了我们知道的实数。那么，如何理解曲线（段）的长度呢？我们曾经用以下公理刻画曲线长度。
...
$C_\gamma$的元素是$\gamma$上任意依次选取点得到的折线的长度。它是正实数集$\mathbb{R}^+$的子集。如果$C_\gamma$是有界集合，那么根据定理\label{tm:2-2-10}，$C_\gamma$有上确界。而曲线的长度，就定义为$C_\gamma$的上确界。

$C_\gamma$是否总是有界集呢？这似乎是个显然的事实，其实不然。数学家已经发现了长度“无限”的曲线。公理中的$c$如果存在，即$C_\gamma$是有界集，就说$\gamma$是\textbf{可求长曲线}。

\begin{ex}
    考虑单位圆$\odot(O, 1)$，在圆上按逆时针方向取$n$个点：
    $$ A_1, A_2, \cdots , A_n $$
    分别对应
    $$0,\frac{\alpha}{n}, \frac{2\alpha}{n}, \cdots , \frac{(n-1)\alpha}{n}.$$
    其中$\alpha$指周角的大小。计算线段$A_1A_2$的长度。考虑三角形$A_1OA_2$，它是等腰三角形，作高$OH_1$交$A_1A_2$于$H_1$，可以算出
    $$ |A_1H_1| = \sin{\frac{\alpha}{2n}}.$$
    因此，
    $$ |A_1A_2| = 2\sin{\frac{\alpha}{2n}}.$$
    同理可以计算$A_2A_3$等线段的长度，每段长度都与$A_1A_2$的长度相同。
    因此折线$A_1A_2\cdots A_nA_1$的总长度是$2n\sin{\frac{\alpha}{2n}}$。这就是圆内接正$n$边形的周长。
\end{ex}

\begin{figure}[h]
    \vspace{4pt}
    \centering
    % \includegraphics[width=0.5\textwidth]{tu/割圆术1.png}
    % \includegraphics[width=0.4\textwidth]{tu/割圆术2.png}
    ...
    \begin{scope}[shift={(0,0)}]
        ...
        \def\cs{0.04};
        \coordinate (O) at (0,0);
        \coordinate (A1) at (0:\R);
        \coordinate (A2) at (\ang:\R);
        \coordinate (B1) at ($(0:\R) + (90:\y*\R)$);
        \coordinate (B2) at ($(0:\R) - (90:\y*\R)$);
        \coordinate (C1) at (0.5*\ang:\R);
        \coordinate (H) at ($(A1)!0.5!(A2)$);
        \coordinate (D1) at ($(A1)+(90:\z*\R)$);
        \coordinate (D2) at ($(A2)-(90+\ang:\z*\R)$);
        \draw[thick] (O) -- (A1) (O) -- (A2);
        \draw[fill=Thistle!70,opacity=0.7] (H) -- ($(H)+(-90+0.5*\ang:\c*\R)$) -- ($(H)+(-90+0.5*\ang:\c*\R)+(-180+0.5*\ang:\c*\R)$) -- ($(H)+(-180+0.5*\ang:\c*\R)$) -- cycle;
        \draw[color=Green,fill=LimeGreen!60!yellow,opacity=0.5] (A1) rectangle ($(A1) + (-\c*\R,\c*\R)$);
        \draw[color=Green,fill=LimeGreen!60!yellow,opacity=0.5] (A2) -- ($(A2)+(-90+\ang:\c*\R)$) -- ($(A2)+(-90+\ang:\c*\R)+(-180+\ang:\c*\R)$) -- ($(A2)+(-180+\ang:\c*\R)$) -- cycle;
        \draw[fill=orange!50,opacity=0.7] (C1) -- ($(C1)+(0.5*\ang:\cs*\R)$) -- ($(C1)+(0.5*\ang:\cs*\R)+(-90+0.5*\ang:\cs*\R)$) -- ($(C1)+(-90+0.5*\ang:\cs*\R)$) -- cycle;
        \draw[thick,blue] (A1) -- (A2);
        \draw[dashed,thick,ForestGreen] (O) -- (B2);
        \draw[thick,ForestGreen] (O) -- (B1);
        \draw[dashed,thick,orange!80] (O) -- (D1) (O) -- (D2);
        \draw[thick,LimeGreen!60!yellow] (B2) -- (B1) -- (A2);
        \foreach \k in {1,2,3,4}{
            \draw[thick,color=blue] (\k*\rt:\R) -- (\k*\rt+\ang:\R) (\k*\rt:\R) -- (\k*\rt-\ang:\R);
            \draw[very thick,color=LimeGreen!60!yellow] ($(\k*\rt:\R) + (\k*\rt-\rt:\y*\R)$) -- ($(\k*\rt:\R) + (\k*\rt+\rt:\y*\R)$);
            \draw[thick,color=LimeGreen!60!yellow] ($(\k*\rt-\ang:\R) + (\k*\rt-3*\ang:\y*\R)$) -- ($(\k*\rt-\ang:\R) + (\k*\rt+\ang:\y*\R)$);
        }
        \foreach \k in {0,1,...,7}{
            \draw[thick,color=Red] (\k*\ang:\R) -- (\k*\ang+0.5*\ang:\R) (\k*\ang:\R) -- (\k*\ang-0.5*\ang:\R);
        }
        \draw[thick] (O) circle (\R);
        % \draw[thick,Red] (A2) -- (C1) -- (A1);
        \draw[thick,BrickRed] (D1) -- (D2) (A1) -- (D1) (A2) -- (D2);
        \draw[fill=gray,thick] (O) circle (2pt);
        \foreach \k in {1,2,3,4}{
            \draw[fill=ProcessBlue] (\ang+\k*\rt:\R) circle (2pt);
            \draw[fill=ProcessBlue] (\k*\rt:\R) circle (2pt);
            \draw[fill=Green] ($(\k*\rt:\R) + (\k*\rt+\rt:\y*\R)$) circle (2pt);
            \draw[fill=Green] ($(\k*\rt:\R) + (\k*\rt-\rt:\y*\R)$) circle (2pt);
        }
        \foreach \k in {0,1,...,7}{
            \draw[fill=yellow] (\k*\ang+0.5*\ang:\R) circle (1.5pt);
        }
        \draw[fill=Purple] (H) circle (2pt);
        \draw[fill=yellow] (C1) circle (1.5pt);
        \draw[fill=Maroon] (D1) circle (1.5pt);
        \draw[fill=Maroon] (D2) circle (1.5pt);
        \node at ($(O)+(-\c*\R,\c*\R)$) {$O$};
        \node[blue] at ($(A1)+(\c*\R+\cs*\R,-\c*\R)$) {$A_1$};
        \node[blue] at ($(A2)+(\c*\R+\cs*\R,+\c*\R)$) {$A_2$};
        \node[OliveGreen] at ($(B2)+(\c*\R+\cs*\R,-\cs*\R)$) {$B_n$};
        \node[OliveGreen] at ($(B1)+(\c*\R+\cs*\R,+\cs*\R)$) {$B_1$};
        \node[RoyalPurple] at ($(H)+(-\c*\R-\cs*\R,+\cs*\R)$) {$\mbox{\footnotesize $H_1$}$};
    \end{scope}
    \begin{scope}[shift={(9,0)}]
        ...
        \def\y{0.4142};
        \coordinate (O) at (0,0);
        \draw[OliveGreen,thick] (-\R,-\R) rectangle (\R,\R);
        \foreach \k in {1,2,3,4}{
            \draw[very thick,color=blue,fill=Cerulean!60,opacity=0.3] (\k*\ang:\R) -- (0.5*\ang+\k*\ang:\R) -- (\k*\ang+\ang:\R) -- cycle;
            \draw[very thick,color=LimeGreen!60!yellow] ($(\k*\ang:\R) + (\k*\ang-\ang:\y*\R)$) -- ($(\k*\ang:\R) + (\k*\ang+\ang:\y*\R)$);
            \draw[very thick,color=LimeGreen!60!yellow] ($(\k*\ang-0.5*\ang:\R) + (\k*\ang-1.5*\ang:\y*\R)$) -- ($(\k*\ang-0.5*\ang:\R) + (\k*\ang+0.5*\ang:\y*\R)$);
        }
        \draw[thick] (O) circle (\R);
        \foreach \k in {1,2,3,4}{
            \draw[fill=OliveGreen] (0.5*\ang+\k*\ang:1.4142*\R) circle (2pt);
            \draw[fill=blue] (\k*\ang:\R) circle (2pt);
            \draw[fill=ProcessBlue] (0.5*\ang+\k*\ang:\R) circle (2pt);
            \draw[fill=LimeGreen!60!yellow] ($(\k*\ang:\R) + (\k*\ang+\ang:\y*\R)$) circle (2pt);
            \draw[fill=LimeGreen!60!yellow] ($(\k*\ang:\R) + (\k*\ang-\ang:\y*\R)$) circle (2pt);
        }
        \draw[very thick,color=Blue,fill=blue!50!Turquoise!60,opacity=0.3] (0:\R) -- (\ang:\R) -- (2*\ang:\R) -- (3*\ang:\R) -- cycle;
    \end{scope}
    \end{tikzpicture}
    \captionsetup{justification=centering}
    \caption*{\texttt{圆内外接正多边形逼近单位圆。边数倍增时，\\内接正多边形周长和面积增加，外接正多边形周长和面积减少。}}
\end{figure}

同理，取圆外接正$n$边形$B_1B_2\ldots B_n$，可以算得其周长为$2n\tan{\frac{\alpha}{2n}}$。

考虑$n=2^m$。观察圆内接正$2^m$边形和正$2^{m+1}$边形，可以发现，当$m$不断增大时，折线的长度不断增加。同样地，观察圆外接$2^m$边形和正$2^{m+1}$边形可以发现，$m$不断增大时，折线的长度不断减少。换句话说，我们有两个数列：
$$ \forall \, m > 2, \quad a_m = 2^m\sin{\left(\frac{\alpha}{2^m}\right)}, \quad b_m = 2^{m}\tan{\left(\frac{\alpha}{2^m}\right)}. $$
其中$\{a_m\}_{m\in\mathbb{Z}^+}$单调严格递增，$\{b_m\}_{m\in\mathbb{Z}^+}$单调严格递减\footnote{$m\leqslant 2$时$a_m,b_m$并没有定义，但我们可以将其补成单调数列，比如规定$a_1=0,a_2=4,b_1=10,b_2=9$。这不妨碍我们讨论它们的大体行为。}。并满足：
$$ \forall \, m, \quad a_m < a_{m+1} < b_{m+1} < b_m.$$
这说明两个数列都是有界数列。比如我们可以说$a_n$总小于$b_3 = 8$。而它们的通项之差为：
\begin{align*}
    2^{m}\tan{\left(\frac{\alpha}{2^m}\right)} - 2^m\sin{\left(\frac{\alpha}{2^m}\right)} &= 2^m\sin{\left(\frac{\alpha}{2^m}\right)}\left(\frac{1}{\cos{\left(\frac{\alpha}{2^m}\right)}} - 1\right) \\
    &= 2^m\sin{\left(\frac{\alpha}{2^m}\right)}\left(\frac{1}{\sqrt{1 - \sin^2{\left(\frac{\alpha}{2^m}\right)}}} - 1\right) \\
    &< b_3 \cdot \left(\frac{1}{\sqrt{1 - \sin^2{\left(\frac{\alpha}{2^m}\right)}}} - 1\right)
\end{align*}
$m$趋于无穷时，$\frac{\alpha}{2^m}$和$\sin{\left(\frac{\alpha}{2^m}\right)}$都趋于$0$，因此上式最后括号里的差趋于$0$。这说明$\{a_m\}_{m\in\mathbb{Z}^+}$和$\{b_m\}_{m\in\mathbb{Z}^+}$有相同的极限。这个极限就是单位圆的周长，它分别是$\{a_m\}$的上确界和$\{b_m\}$的下确界。我们按惯例将其记为$2\pi$。

我们希望把这个极限写成更一般的形式：
$$ \lian{n\to\infty}\, n\sin{\left(\frac{\alpha}{n}\right)} = 2\pi.$$
这是一个重要性质。我们引入弧度制，将周角的大小记为$2\pi$后，就可以把这个极限写成：
$$ \lian{n\to\infty} \frac{\sin{\frac{\pi}{n}}}{\frac{\pi}{n}} = 1. $$
直观上可以解释为：只要弧长足够小，它和对应弦长的比值会趋于$1$。或者说，圆弧越短就越“直”。

如何证明这个结论呢？让我们继续讨论圆内外接正多边形。这一次我们研究它的面积。圆内接正$n$边形的面积是$\frac{n}{2}\sin{\frac{\alpha}{n}}$，外接正$n$边形的面积是$n\tan{\frac{\alpha}{2n}}$。对于$n=2^m$的情况，我们发现两者的面积也构成和周长类似的数列，一个严格单调递增，一个严格单调递减。于是，用同样的方法可以说明，两者有共同的极限，也就是单位圆的面积。即：\textbf{单位圆的面积是}$\pi$。

把单位圆$\odot(O,1)$按照内外接正$n$边形的顶点和中心连线均匀分成$n$个小扇形，可以看到，小扇形的面积介于三角形$OA_1A_2$和$OB_1B_n$的面积之间。小扇形的面积是圆面积$\pi$的$n$分之一，因此可以得到：
$$ \forall \,n,\quad \frac{1}{2}\sin{\frac{2\pi}{n}} < \frac{\pi}{n} < \frac{1}{2}\tan{\frac{2\pi}{n}}. $$
取$n = 2m$就得到：
\begin{align*}
    \forall \,m,\quad \sin{\left(\frac{\pi}{m}\right)} &< \frac{\pi}{m} < \tan{\left(\frac{\pi}{m}\right)} = \frac{\sin{\left(\frac{\pi}{m}\right)}}{\cos{\left(\frac{\pi}{m}\right)}} \\
    \cos{\left(\frac{\pi}{m}\right)} &< \frac{\sin{\left(\frac{\pi}{m}\right)}}{\frac{\pi}{m}} < 1.
\end{align*}
$m$趋于无穷大时，$\frac{\pi}{m}$趋于$0$，于是$\cos{\left(\frac{\pi}{m}\right)}$趋于$1$，因此根据同归定理，
$$ \lian{n\to\infty} \frac{\sin{\frac{\pi}{n}}}{\frac{\pi}{n}} = 1. $$

这个结论还可以推广到任何趋于$0$的数列：
\begin{tm}{\textbf{弦弧等极}}
    $$\lian{n\to\infty} a_n = 0 \,\,\implies \lian{n\to\infty} \,\,\frac{\sin{a_n}}{a_n} = 1. $$
\end{tm}

\begin{proof}
    我们希望把$a_n$化归为$\frac{\pi}{m}$的形式，即找到使$a_n\approx \frac{\pi}{p_n}$的正整数$p_n$。首先，$\frac{\sin{(-a_n)}}{-a_n} = \frac{-\sin{a_n}}{-a_n} = \frac{\sin{a_n}}{a_n}$。因此可以假定$a_n$总大于$0$。而$a_n$最终趋于$0$，所以我们可以假设$a_n$都足够小，比如都小于$1$。

    对于$a_n\in(0;1)$，记$p_n$为使得$a_n p_n > \pi$的最小正整数。则有：
    $$ a_n(p_n - 1) \leqslant \pi < a_np_n. $$
    $p_n > \frac{\pi}{a_n}$，所以$a_n$趋于$0$时，$p_n$趋于正无穷大，因而$\sin{\frac{\pi}{p_n}}$趋于$0$，$\frac{\sin{\frac{\pi}{p_n}}}{\frac{\pi}{p_n}}$趋于$1$。这已经接近要证明的结论了。下面我们用它构造$\frac{\sin{a_n}}{a_n}$的上下界，再证明上下界都趋于$1$。这样，根据同归定理，$\frac{\sin{a_n}}{a_n}$就趋于$1$了。
    
    首先，按照弧度制，弧长为$2a_n$的圆弧，弦长为$2\sin{a_n}$。因此
    $$\frac{\sin{a_n}}{a_n} < \frac{a_n}{a_n} = 1.$$

    另一方面，由于正弦函数在$[\,0;1\,]$上严格单调递增，
    $$a_n p_n > \pi \implies \,\frac{\pi}{p_n} < a_n  \,\implies \,\sin{\left(\frac{\pi}{p_n}\right)} < \sin{a_n}. $$
    而按定义：$p_n - 1 \leqslant \frac{\pi}{a_n}$。因此
    $$ \frac{p_n - 1}{\pi}\cdot\sin{\left(\frac{\pi}{p_n}\right)} < \frac{\sin{a_n}}{a_n} < 1.$$
    右侧上界是常数$1$，而左侧下界的极限为：
    $$ \lian{n\to\infty}\left(\frac{p_n - 1}{\pi}\cdot\sin{\left(\frac{\pi}{p_n}\right)}\right) = \lian{n\to\infty}\frac{\sin{\frac{\pi}{p_n}}}{\frac{\pi}{p_n}} - \frac{1}{\pi}\cdot\lian{n\to\infty}\sin{\left(\frac{\pi}{p_n}\right)} = 1 - \frac{1}{\pi}\cdot 0 = 1.$$
    上下界的极限都是$1$，于是根据同归定理，$\displaystyle \lian{n\to\infty} \,\,\frac{\sin{a_n}}{a_n} = 1.$
\end{proof}

\begin{xt}
    \mbox{} \\
    \indent 1. 验算圆外接正$n$边形的周长和面积。\\
    \indent 2. 通过用$\sin{\frac{x}{2}}$表示$\cos{x}$，求极限：
...
一般来说，第$n$行第$k$个数是$(a+b)^{n-1}$展开式中$a^{k-1}b^{n-k}$的系数，第$k+1$个数是$(a+b)^{n-1}$展开式中$a^{k}b^{n-k-1}$的系数，第$n+1$行第$k+1$个数是$(a+b)^{n}$展开式中$a^{k}b^{n-k}$的系数。把$(a+b)^n$看作$a+b$与$(a+b)^{n-1}$的乘积，那么$a^{k}b^{n-k}$来自前者的$b$与后者的$a^{k}b^{n-k-1}$项的积，以及前者的$a$与后者的$a^{k-1}b^{n-k}$项的积。因此，$a^{k}b^{n-k}$的系数自然是$a^{k}b^{n-k-1}$、$a^{k-1}b^{n-k}$两项的系数的和。换句话说：
\begin{quotation}
    $\forall \, n$，第$n$行第一个数是$1$。$\forall \, k \in \hin{1}{n}$，第$n+1$行第$k+1$个数是第$n$行第$k$个数与第$k+1$个数的和。
\end{quotation}

公元$1261$年，南宋数学家杨辉所著的《详解九章算术》中就出现了这个三角数阵，
我们把它称为\textbf{杨辉三角形}。在杨辉三角形中使用以上递推算法，我们可以方便地查出$(a+b)^n$各项的系数。
...
例如，展开$(a+b)^6$时，我们已经知道有$a^2b^4$这一项（$k=2$）。它的系数是在$6$个$a,b$中选择$2$次$a$、$4$次$b$的方法数，即组合数$\bino{6}{2}$。
于是，$(a+b)^6$的展开式中，$a^2b^4$这一项的系数是$\bino{6}{2} = 15$。

对特定的$k$来说，$a^kb^{n-k}$的系数来自于我们展开时选择了$k$次$a$、$n-k$次$b$。
因此，$a^kb^{n-k}$的系数就等于选出$k$个$a$和$n-k$个$b$的方法数。它等于组合数$\bino{n}{k}$。
也就是说，$(a+b)^n$的展开式中，$a^kb^{n-k}$的系数为$\bino{n}{k}$。这个系数叫做\textbf{二项式系数}。
我们可以完整地写出$(a+b)^n$的展开式：
$$(a+b)^n = \sum_{k=0}^n \bino{n}{k} a^kb^{n-k} = \sum_{k=0}^n \frac{n!a^kb^{n-k}}{k!(n-k)!} .$$
这个结论称为\textbf{二项式定理}，右边的式子称为$n$次二项展开式。其中为了方便，
约定$\bino{n}{0} = \frac{n!}{0!n!} = 1 = \bino{n}{n}$。

组合数是二项式展开中的系数，因此，可以用二项式定理反过来推导出组合数的一些基本性质。
比如，由前面的结论可知：
$$\bino{n}{k} = \bino{n}{n-k}, \quad \bino{n+1}{k} = \bino{n}{k} + \bino{n}{k-1}.$$
又如，令$a=b=1$，可得到：
$$\sum_{k=0}^n \bino{n}{k} = 2^n.$$
令$a=-1$、$b=1$，可得到：
$$\sum_{k=0}^n (-1)^k \bino{n}{k} = 0.$$

观察杨辉三角形，还可以发现二项式系数的性质：从$\bino{n}{0}=1$起，随着$k$增大，
$\bino{n}{k}$先是不断增大，然后不断减小，最终回到$\bino{n}{n} = 1$。
$n=2m+1$为奇数的时候，$\bino{n}{m}=\bino{n}{m+1}$是数值最大的；$n=2m$为偶数时，
$\bino{n}{m}$是数值最大的。这些性质可以用以下关系式解释：
$$ \bino{n}{k} = \frac{n!}{k!(n-k)!} = \frac{n-k+1}{k}\cdot \frac{n!}{(k-1)!(n-k+1)!} = \frac{n-k+1}{k}\bino{n}{k-1}.$$
$k < n-k+1$的时候，$\bino{n}{k} > \bino{n}{k-1}$；$k > n-k+1$的时候，$\bino{n}{k} < \bino{n}{k-1}$。分水岭是$\frac{n+1}{2}$。

具体来说，$\bino{n}{k}$大概有多大呢？我们知道，使得$\bino{n}{k}$最大的$k$在$n$的一半左右。
...
\begin{xt}
    ...
    \indent 2. 观察$(1+x)^{2n}$的系数，证明：$\sum_{k=0}^n \left(\bino{n}{k}\right)^2 = \bino{2n}{n}.$ \\
    \indent 3. 观察$a^n = (a-b+b)^n$的系数，证明：
                $$\forall \,\, 0 \leqslant m < n, \;\,\, \sum_{k=m}^n (-1)^{k-m} \bino{n}{k}\bino{k}{m} = 0.$$
    \indent 4. $7^{103}$模$5$的余数是多少？模$25$的余数是多少？

\end{xt}

\section{二项式的应用}

二项式在科学研究、生产生活中经常出现，有大量用途。这里举几个简单的例子。

\subsection{计算极限}

\begin{et}
    设实数$a>1$。证明，数列$\{a^n\}_{n\in\mathbb{Z}^+}$趋于正无穷大。
\end{et}

\begin{proof}
    记$h = a - 1$，则$h>0$。将$a^n = (1 + h)^n$展开可得：
    $$ a^n = (1+h)^n = 1 + nh + \cdots +  \bino{n}{k}h^k + \cdots + h^n.$$
    右边所有项都是正数。因此，总有$a^n > 1+nh$。因此，对任意数$A$，当$n>\frac{A-1}{h}$时，总有$a^n>1+nh>A$。这就说明$\{a^n\}_{n\in\mathbb{Z}^+}$趋于正无穷大。
\end{proof}

\begin{et}
    证明：如果$a>1$，那么：
    $$ \lian{n\to\infty}\; \frac{a^n}{n} = +\infty.$$
\end{et}
\begin{proof}
    同上。$n > 1$时，将$a^n = (1 + h)^n$展开可得：
    $$ a^n = (1 + h)^n = 1 + nh + \frac{n(n-1)}{2}h^2 + \cdots + \bino{n}{k}h^k + \cdots + h^n.$$
    右边所有项都是正数。因此总有：
    \begin{align*}
        \frac{a^n}{n} = \frac{1 + nh + \frac{n(n-1)}{2}h^2 + \cdots + h^n}{n} > \frac{(n-1)h^2}{2}.
    \end{align*}
    对任意正数$A$，取$N$为大于$\frac{2A}{h^2}$的整数。那么只要$n>N$，就有：
    $$ \frac{a^n}{n} > \frac{(n-1)h^2}{2} \geqslant \frac{Nh^2}{2} > A. $$
    这就说明
    $$ \lian{n\to\infty}\; \frac{a^n}{n} = +\infty.$$
\end{proof}
...
\begin{et}
    如果$a>1$，常数$k$为正整数，那么：
    $$ \lian{n\to\infty}\,\; \frac{a^n}{n^k} = +\infty.$$
\end{et}
\begin{proof}
    只需注意到：
    $$\frac{a^n}{n^k} = \left(\frac{\left(a^{\frac{1}{k}}\right)^n}{n}\right)^k.$$
    $a>1$时，$\displaystyle a^{\frac{1}{k}}$也大于$1$。于是数列$\displaystyle\left\{\frac{\left(a^{\frac{1}{k}}\right)^n}{n}\right\}$趋于正无穷大。而$k$是正整数，所以$\frac{a^n}{n^k}$作为前者的$k$次方，也趋于正无穷大。
\end{proof}
% \begin{proof}
%     将以上证明稍作修改。设$a = 1 + h$，则$h > 0$。$n>2k$时，总有：
%     \begin{align*}
%         \frac{a^n}{n^k} &= \frac{1 + nh + \cdots + \frac{n(n-1)\cdots(n-k)}{(k+1)!}h^{k+1} + \cdots + h^n}{n^k} \\
%         &> n\cdot\frac{n-1}{n}\cdot\frac{n-2}{n}\cdots\frac{n-k}{n}\cdot\frac{h^{k+1}}{(k+1)!}.
%     \end{align*}
%     依照假设$n>2k$，所以$1\leqslant i\leqslant k$时，$\frac{n-i}{n} > \frac{1}{2}$，于是：
%     $$ \frac{a^n}{n^k} > \frac{h^{k+1}}{2^k(k+1)!}\cdot n \,.$$
%     对任意正数$A$，取$N$为大于$\frac{2^k(k+1)!A}{h^{k+1}}$的整数。那么只要$n>N$，就有：
%     $$ \frac{a^n}{n^k} > \frac{h^{k+1}}{2^k(k+1)!}n > A. $$
%     这就说明
%     $$ \lian{n\to\infty}\;\, \frac{a^n}{n^k} = +\infty.$$
% \end{proof}
...
如果$|q| > 1$，那么$n$趋于无穷时，$|q|^{n-1}$趋于无穷。$a_1 \neq 0$时，$\{|a_n|\}$趋于正无穷大。
具体来说，如果$q$是正数，那么$a_n$在$a_1>0$时趋于正无穷大，在$a_1<0$时趋于负无穷大；如果$q$是负数，那么数列的奇数项和偶数项符号相反，
相邻两项的差是两者绝对值之和，而$|a_n|$趋于正无穷大，因此数列没有极限。

% 定义了实数的指数函数和对数函数后，我们还可以具体找出$N$。
% 考虑满足：
% $$ |a_1|\cdot|q|^{x-1} = M $$
% 的实数$x$。上式两边除以$|a_1|$，然后取对数，得到：
% $$ x = 1 + \log_{|q|}{\frac{M}{|a_1|}}. $$
% 由于$x\mapsto |q|^x$是严格递增函数，只需取大于$1 + \log_{|q|}{\frac{M}{|a_1|}}$的正整数$N$，就有
% $$ |q|^{N-1} \geqslant |q|^{x-1} = |q|^{\log_{|q|}{\frac{M}{|a_1|}}} = \frac{M}{|a_1|},$$
% 即
% $$ |a_1|\cdot|q|^{N-1} \geqslant M.$$

另一方面，如果$|q| < 1$，那么$\displaystyle\frac{1}{|q|} > 1$。按前面的结论，$\frac{1}{|q|^n}$趋于正无穷大，因此$|q|^n$趋于$0$，从而$\{|a_n|\}$也趋于$0$。
...
\begin{xt}
    ...
    \indent 2. 数列$\{x_n\}_{n\in\mathbb{Z}^+}$首项$ x_1 = 2$，且满足递推关系：
    \begin{align*}
...
$n$次抛掷中，要使得$k$次抛掷向上，有$\bino{n}{k}$种可能，因此，$n$次抛掷中有$k$次正面向上的概率就是$\bino{n}{k}p^k(1-p)^{n-k}$，
...
\begin{center}
    \begin{tabular}{| p{4em}<{\centering} | p{8em}<{\centering} |}
        ...
        \hline
        $0$ & $\bino{n}{0} p^0(1 - p)^n\quad$ \\  
        \hline
        $1$ & $\bino{n}{1} p^1(1 - p)^{n-1}\,$ \\  
        \hline
        $\vdots$ & $\vdots$  \\    [0.75ex] 
        \hline
        $n-1$ & $\bino{n}{n-1} p^{n-1}(1 - p)^{1}\quad$ \\ 
        \hline
        $n$ & $\bino{n}{n} p^n(1 - p)^{0}\quad$ \\ 
        \hline
    \end{tabular}
\end{center}
...
\begin{figure}[h]
    ...
    \begin{axis}[
        ...
        yticklabels={0, 0.025, 0.05, 0.075, 0.1, 0.125, 0.15, 0.175, 0.2}, 
        grid=both,
        grid style={dashed, gray!30},     
        tick label style={font=\small}
    ]
    % 直方图
    \addplot [fill=Cerulean!50, draw=CadetBlue, thick] coordinates {
        (0,  0.000) (1,  0.000) (2,  0.003) (3,  0.012) (4,  0.035)
        (5,  0.075) (6,  0.124) (7,  0.166) (8,  0.180) (9,  0.160)
        (10, 0.117) (11, 0.071) (12, 0.035) (13, 0.015) (14, 0.005)
...
\subsectionext{微扰估计}
\begin{et}
某酒厂培养的酵母在某种环境下每$6$个小时数量增加$1\%$。设初始培养基数为$100$个单位。$20$天后，酵母数量能否超过$200$个单位？
\end{et}
\begin{so}  
    ...
    根据二项式定理：
    $$(1 + d)^n = 1 + \bino{n}{1} d + \bino{n}{2} d^2 + \cdots + \bino{n}{n} d^n.$$
    
    上式右侧每一项都大于零。因此，只要其中部分项的和大于$2$，$(1 + d)^n$就大于$2$。我们从$k$较小的几项开始，计算每一项的大小。
    \begin{align*}
        \bino{n}{1} d = 80 \times 0.01 &= 0.8  \\
        \bino{n}{2} d = \frac{80 \times 79}{2\times 1} \times 0.01^2 &= 0.316  \\ 
        \bino{n}{3} d = \frac{80 \times 79\times 78}{3\times 2\times 1}80 \times 0.01^3 &= 0.08216 
    \end{align*}
    根据第一项的值可知：$(1 + d)^n > 1 + \bino{n}{1} d = 1.8$，类似地，根据前两项、前三项的值可知：$(1 + d)^n > 2.116$、$(1 + d)^n > 2.19816$。
...
我们知道，$(1+x)^n$展开式中，$x$的$k+1$次项是$x$的$k$次项乘以$\frac{(n-k+1)x}{k}$。因此，当$nx<1$的时候，展开式中$x$次数高的项的绝对值，会比$x$次数较低的项小得多。如果$|nx|$远小于$1$，那么$(1+x)^n$展开式的各项随着$x$次数增加，绝对值快速减小，$x$次数高的项相比于$x$次数低的项，小得可以忽略。于是，微扰估计时，我们主要估计前几项对问题的影响。

\begin{et}
    2020年，某企业向银行申请一笔人民币$10$亿元的贷款，贷款年利率为$5\%$，约定分$10$年还清。具体约定：银行自2021年1月1日零时将$10$亿元汇入企业账户。企业自2022年起每年1月1日向银行偿还一笔相同金额的现金。$10$年后恰好完全还清本金和利息。问题：企业每年偿还的现金应该是多少？
\end{et}
\begin{so}
    让我们从最后一年倒推。设每年还款$x$元；第$i$年末剩余的债务为$z_i$，初始债务记为$z_0$。第$10$年年末剩余的债务，恰好由最后一次还款付清，所以$z_{10} = x$。而第$10$年年末剩余的债务是由第$9$年还款后剩余的欠款，增加$5\%$利息后得到的。因此，倒推一年，第$9$年年末的债务$z_9$，减去$x$元还款，乘以$1+5\%=1.05$，就得到$z_{10}$：
    \begin{align*}
        (z_9 - x) \times 1.05 &= z_{10} \\
        z_9 &= \frac{z_{10}}{1.05} + x = \frac{x}{1.05} + x
    \end{align*}
    而第$9$年年末剩余的债务又是由第$8$年还款后剩余的欠款，增加$5\%$利息后得到的。因此，
    \begin{align*}
        (z_8 - x) \times 1.05 &= z_{9} \\
        z_8 = \frac{z_9}{1.05} + x &= \frac{1}{1.05} \times \left(\frac{x}{1.05} + x\right) + x = \frac{x}{1.05^2} + \frac{x}{1.05} + x.
    \end{align*}
    这个递推关系对之前的每一年都成立，因此最终得到初始债务和$x$的关系：
    \begin{align*}
        10 = z_0 = \frac{x}{1.05^{10}} + \cdots + \frac{x}{1.05^2} + \frac{x}{1.05} \,\,\mbox{（亿元）} \tag{注意：最初不需要还钱}
    \end{align*}
    等式右边是一个等比数列，因此可得：
    \begin{align*}
        10 &= \frac{x}{1.05^{10}}\cdot \sum_{k=0}^{9} 1.05^k = \frac{x}{1.05^{10}}\cdot \frac{1.05^{10} - 1}{1.05 - 1} \\
        x &= \frac{10\cdot 0.05 \cdot 1.05^{10}}{1.05^{10} - 1} .
    \end{align*}
    关键是计算$1.05^{10}$。按照二项式展开：
    \begin{align*}
        1.05^{10} &= (1 + 0.05)^{10} = \sum_{k=0}^{10} \bino{10}{k}\cdot 0.05^{k}
    \end{align*}
    注意到$\bino{10}{5} = 252$，因此第$6$项$\bino{10}{5}\cdot 0.05^5 = \frac{252}{32} \cdot 10^{-5} < 10^{-4}$，而之后的项都远小于它，因此只计算前$6$项，得到：
    $$ 1.05^{10} \approx 1 + \frac{10}{20} + \frac{45}{400} + \frac{120}{8000} + \frac{210}{160000} + \frac{252}{3200000} \approx 1.6289\, .$$
    据此算得
    $$ x \approx 1.2950\,\,\mbox{（亿元）}$$
\end{so}
实际还款额为人民币$1\dlim{2950}\dlim{4574}.97$元（精确到分），相差小于$5000$元。

\begin{xt}
    \mbox{} \\
    \indent 1. 甲国经济规模是乙国的3倍。预测甲国经济规模每年增长$2\%$，乙国每年增长$6\%$。按这个趋势估计，多少年后乙国经济规模赶上甲国？\\
    \indent 2. 某企业向银行申请一笔$1000$万元的贷款，年化贷款利率为$4\%$，约定分$20$年还清，每年偿还相等金额。计算每年的还款额。
\end{xt}
...
\begin{so}
    ...
    $$ \left(1+\frac{T-T_0}{T_0}\right)^5 \approx 1 + 5\cdot 0.00308 + 10\cdot 0.00308^2 = 1.0155 $$
    因此，升温后材料长度约为$25\cdot 1.0155 = 25.387$厘米，增加了$0.387$厘米。
    完整计算的结果约为$0.38699$厘米，误差在$1\%$以内，未造成额外误差。   
\end{so}

在这个例子里，$x\approx 0.003$，
$$\bino{5}{2}x^2 = \left(\frac{5-2+1}{2}x\right)\cdot \bino{5}{1}x^1$$
括号中的$\frac{5-2+1}{2}x$远小于$1$，因此$\bino{5}{2}x^2$远小于$\bino{5}{1}x^1$，
其后的$\bino{5}{3}x^3$、$\bino{5}{4}x^4$、$\bino{5}{5}x^5$更是如此。于是，我们可以根据精度要求，适当舍掉靠后的大部分项，
只保留靠前$x$次数较低的项，快速估计出近似值。

实践中，我们只保留前一到两项，精度不够时，再逐步尝试。比如先计算$1 + \bino{n}{1}x = 1 + nx$，
再计算$1 + \bino{n}{1}x + \bino{n}{2}x^2 = 1 + nx + \frac{n(n-1)}{2}x^2$。如果两者之差在精度要求的误差范围内，
就可以用后一个作为估计结果。
\begin{et}
...
\end{et}
\begin{so}
    将问题转化为求$2^{19}\cdot\left(1-\frac{2-1.9926}{2}\right)^{19}$。这时$n=19$，$x = -\frac{2-1.9926}{2} = -0.0037$。计算$1 + nx$和$1 + nx + \frac{n(n-1)}{2}x^2$的值，分别得到$0.9297$和$0.9320$。两者之差为$0.0023$，小于$1\%$，因此认为精度已达到要求。估计结果为$2^{19}\cdot 0.9320=488636.416$。经验证，精确结果为$488632.549\dots$，相对误差在$1\%$以内。
\end{so}
...
首先，我们要明确定义：当我们说“比较好”的时候，指的是什么。如果$x$不是有理数的时候，$f_{\mathbb{R}}(x) = 0$，那么我们说这样的函数$f_{\mathbb{R}}$不够好。为什么呢？让我们画出这个函数的图像。一般来说，要画一个实变函数，我们会用“描点连线”的办法，先求出函数在一系列点上的值，然后用直线把相邻的点连起来。我们认为，这样得到的图像，和函数的真实图像大致一样。
...
\begin{so}
    ...
    设$a_n = 2^\frac{1}{n}$，则$a_n^n = 2$。我们知道，给定正整数$n$，$x>0$时，$x\mapsto x^n$是递增函数。$1^n=1$，$2^n\geqslant 2$，所以总有$1<a_n\leqslant 2$。
    另一方面，$x>1$固定时，$x^\frac{1}{n}$随着$n$增大而减小，所以$a_n$随着$n$增大而减小，向$1$靠近。

    到底$a_n$和$1$有多接近呢？我们来计算$\left(1+\frac{1}{n}\right)^n$。

    根据二项式定理：
    \begin{align*}
        \left(1+\frac{1}{n}\right)^n &= 1 + \bino{n}{1}\cdot \frac{1}{n} + \bino{n}{2}\cdot \left(\frac{1}{n}\right)^2 + \cdots + \bino{n}{n}\cdot \left(\frac{1}{n}\right)^n  \\
        &> 1 + \bino{n}{1}\cdot \frac{1}{n} = 1 + \frac{n}{n} = 2. 
    \end{align*}
    这说明$1<a_n\leqslant 1 + \frac{1}{n}$。由于数列$\{1\}$和$\{1 + \frac{1}{n}\}$都收敛到$1$，根据同归定理，数列$\{a_n\}$也收敛到$1$。

    再考虑“另一边”的数列：$\forall \, n\in\mathbb{Z}^+,\,\, b_n = 2^{-\frac{1}{n}}$。它的通项是$a_n$的倒数，因此它收敛到$1$的倒数，也就是$1$。

    其他趋于$0$的有理数列$\{x_n\}_{n\in\mathbb{Z}^+}$是否也趋于$1$呢？我们可以仿照同归定理，拿它和$\{a_n\}$、$\{b_n\}$作比较。

    $\{a_n\}$递减收敛到$1$，所以对任意$r>0$，都有正整数$N$，使得$a_{N} = 2^{\frac{1}{N}} < 1 + r$。这时，
    $$b_N = \frac{1}{a_N} = \frac{1}{1 + r} > 1 - r.$$
    而$\{x_n\}$趋于$0$，所以存在正整数$n$，使得$m>n$时总有$|x_m| < \frac{1}{N}$，于是：
    $$1 - r < b_N = 2^{-\frac{1}{N}} < 2^{x_m} < 2^\frac{1}{N} = a_N < 1 + r.$$
    这就说明：$\{2^{x_n}\}_{n\in\mathbb{Z}^+}$趋于$1$。结论：
    \begin{quotation}
        只要有理数列$\{x_n\}_{n\in\mathbb{Z}^+}$趋于$0$，$\{f_{\mathbb{Q}}(x_n)\}_{n\in\mathbb{Z}^+}$就趋于$1$。
    \end{quotation}
\end{so}
...
\begin{df}\textbf{实变函数在一点的极限}
    \mbox{}\\
    设$f$是实变函数，$a$是实数。如果任何在$f$定义域中取值的数列$\{a_n\}_{n\in\mathbb{Z}^+}$，只要它趋于$a$，$\{f(a_n)\}_{n\in\mathbb{Z}^+}$就趋于同一个极限$b$，那么就说$b$是函数$f$在$a$处的\textbf{极限}。或者说自变量趋于$a$时，$f$\textbf{趋于}或\textbf{收敛到}$b$，记作：
    $$  \lian{x \to a} f(x) = b. $$
\end{df}
从定义可知：
\begin{enumerate}
    \item $f$在一点若有极限，则有唯一的极限。
    \item $f$不必在有极限处有定义。但如果$f$在一点$a$处有极限且有定义，那么由于常数列$a_n = a$趋于$a$，$f$在$a$处的极限必定是$f(a)$。我们说$\boldsymbol{f}$\textbf{在}$\boldsymbol{a}$\textbf{处连续}。
\end{enumerate}
\begin{df}{\textbf{实变函数在一点连续}}
    \mbox{}\\
    设$f$是实变函数，如果$f$在定义域内一点有极限，就说$\boldsymbol{f}$\textbf{在该点连续}。
\end{df}
用$f_{\mathbb{Q}}$的例子来说，$f_{\mathbb{Q}}$在$0$处有定义，有极限$1$，它在$0$处连续。

让我们来看一个简单的例子。
\begin{et}
    求函数$f: x\mapsto x^2$在$x=0$处的极限。若令$f(0) = 1$，即变成：
...
\end{et}
\begin{so}
    按照定义，我们考虑任意一个趋于$0$但不恒为$0$的数列$\{x_n\}_{n\in\mathbb{Z}^+}$，计算经过$f$映射后得到的数列$f(x_n) = x_n^2$的极限。按照极限的运算法则，数列$\{x_n^2\}_{n\in\mathbb{Z}^+}$也趋于$0$。这说明函数$f$在$0$处如果有极限，应该是$0$。$f(0)=0$时，这个极限就是$0$。但如果$f(0)=1$，那么考虑常数列$\{0\}$，它的极限是$0$，而经过$f$映射后变为常数列$\{1\}$，极限是$1$。这说明更改后的函数$f$在$0$处没有极限。
\end{so}

直观上，我们认为连续的函数在一点的极限，就是画函数图像时，曲线“应该经过”的地方。换句话说，如果函数$f$在实数$a$处有定义，那么当$f$图像上一点$x$的横坐标足够接近$a$时，它的纵坐标应该足够接近$f(a)$。最终，函数图像“应该经过”点$(a,\,f(a))$。函数在一点连续，就是它经过它“应该经过”的地方，把函数图像在该点前后的部分“连上”了。

实变函数在一点的连续性质是一个\textbf{局部}的性质。具体来说，我们要求函数$f$在实数$a$及其附近有定义，但“附近”可以是任意近。这说明，讨论函数在一点的连续性时，我们可以避开考虑函数在其他地方的性质。比如，我们要讨论$f$在$1$处是否连续，可以不考虑$f$在$1.001$处的性质，因为我们可以只考虑区间$(0.9999;1.0001)$。

整体来看，如果函数$f$在给定区间的任一处都连续，就说$f$在该区间\textbf{处处连续}，或者说$f$是该区间上的\textbf{连续函数}，该区间是$f$的\textbf{连续区间}。连续性质是对生活中“连续的线”概念的抽象。如果函数在某个区间上连续，那么它的图像在直观上是一条连续不断的线，不会有“断点”。

数列除了趋于某个极限，还可能趋于正负无穷大。对应地，函数在一点也有类似的性质。
\begin{df}
    给定实数$a$和实变函数$f$，如果任何在$f$定义域中取值的数列$\{a_n\}_{n\in\mathbb{Z}^+}$，只要它趋于$a$，$\{f(a_n)\}_{n\in\mathbb{Z}^+}$就趋于正（负）无穷大，就说函数$\boldsymbol{f}$\textbf{在}$\boldsymbol{a}$\textbf{处趋于正}（\textbf{负}）\textbf{无穷大}；或者说自变量趋于$a$时，$f$\textbf{趋于正}（\textbf{负}）\textbf{无穷大}：
    $$  \lian{x \to a} f(x) = \pm\infty. $$
\end{df}
按照前述定义，如果$f$在$a$处有定义，那么$f$在$a$处趋于$f(a)$。因此，$f$在一点趋于无穷大，就说明$f$在该点没有定义。

\begin{et}
    计算函数$f: x\mapsto \displaystyle\frac{1}{x^2}$在$0$处的极限。
\end{et}
\begin{so}
    考虑任意趋于$0$的非零数列$\{x_n\}$。按照定义，对任意正数$A$，由于$\frac{1}{\sqrt{A}} > 0$，
    总能找到正整数$N$，使得$n>N$时总有$|x_n| < \frac{1}{\sqrt{A}}$，也就是说$f(x_n) = \displaystyle\frac{1}{x_n^2} > A$。这说明$f$在$0$处趋于正无穷大。
\end{so}

讨论函数在一点的极限，也可以不依赖数列。
\begin{df}\textbf{实变函数在一点的极限}\\
    设$f$是实变函数，$a, b$是实数。如果对任何$r>0$，都存在$d>0$，
    使得$f$定义域中任何数$x$到$a$的距离只要小于$d$，$f(x)$到$b$的距离就小于$r$，
    就说$b$是函数$f$在$a$处的\textbf{极限}。也可以说自变量趋于$a$时，$f$\textbf{趋于}或\textbf{收敛到} $b$。
\end{df}
这种定义不需要讨论所有趋近点$a$的数列。它与无穷数列的极限的定义相仿。两者都要找到关于$r$的“门槛”，区别在于无穷数列极限要找的是正整数$N$，这个定义里要找的是正实数$d$。把数列看作$\mathbb{N}$（或$\mathbb{Z}^+$）到$\mathbb{R}$的映射，那么可以说两种极限定义出自同一种思想，只是定义域结构上的区别导致了差异：无穷数列并无法真正触碰到“无穷远处”，而函数自变量趋近$a$时可以触及$a$。讨论具体性质时，这个差异就会表现出来。

下面用新定义来证明$f:x\mapsto x^2$在$x=0$处连续。
\begin{proof}
    对任意$r>0$，取$d = \sqrt{r}$，那么只要$x$到$0$的距离小于$d$（$|x - 0| < d$），就有：
    $$|f(x) - 0| = x^2  = |x - 0|^2 < d^2 = r.$$
    这就说明$f$在$x=0$处的极限是$0 = f(0)$。于是$f$在$0$处连续。
\end{proof}

为了方便说话，我们把$(a-d;a+d)$称为$a$的$d$–\textbf{邻区间}，记作$U(a,\,d)$，它可以描述“与$a$邻近”、“在$a$附近”的概念。那么，$|x - a| < d$就等价于$x\in U(a,\,d)$。以上定义也可以说成：“$f$定义域中任何在邻区间$U(a,\,d)$的数，取值都在邻区间$U(b,\,r)$里”。

下面证明这两种定义是等价的。
\begin{proof}
    给定实数$a$、$b$和实变函数$f$。

    \textbf{首先证明}数列定义的极限可以推出距离定义的极限：如果对$f$定义域中任意趋于$a$的数列$\{a_n\}$，$\{f(a_n)\}$都趋于$b$，那么对于$f$定义域中任意正数$r$，都存在正数$d$，使得只要$x\in U(a,\,d)$，就有$f(x)\in U(b,\,r)$。

    使用反证法来证明。反设有正数$r$，使得上述的$d$不存在。那么，对任何$d>0$，总有某个$x_d\in U(a,\,d)$，
    使得$ |b - f(x_d)| \geqslant r$。我们取一组特殊的$d$，比如$d = \frac{1}{n}$，对应的$x_d$记作$x_n$，就得到一个数列：$\{x_n\}_{n\in\mathbb{Z}_+}$。
    
    数列$\{x_n\}$按定义趋于$a$。同时，$f(x_n)$与$b$的差总大于$r$。于是我们构造出趋于$a$的数列$\{x_n\}$，使$\{f(x_n)\}$不趋于$b$，矛盾！因此原命题成立。

    \textbf{再证明}距离定义的极限可以推出数列定义的极限：如果对于任意正数$r$，都存在正数$d$，使得对任何$x\in U(a,\,d)$，总有$f(x)\in U(b,\,r)$，那么，只要数列$\{a_n\}$趋于$a$，$\{f(a_n)\}$就趋于$b$。

    对任何趋于$a$的数列$\{a_n\}$，我们要证明$\{f(a_n)\}$趋于$b$。给定正数$r$，我们需要找到正整数$N$，使得$n>N$时总有$|f(a_n) - b| < r$。
    
    根据定义，存在正数$d$，使得只要$|x - a| < d$，总有$|f(x) - b| < r$。
    因此，我们只需要让$n>N$时总有$|a_n - a| < d$即可。而由于$\{a_n\}$趋于$a$，按照定义，对于正数$d$来说，存在正整数$N_d$，使得$n>N_d$时总有$|a_n - a| < d$。于是这个$N_d$就是我们要找的正整数$N$。

\end{proof}

\begin{sk}
    \mbox{} \\
    \indent 1. 考虑实变函数$f$和实数$a$。实数$d$大于$0$。设$f$在$U(a,\,d)$上有定义。试用邻区间的概念给出“函数$f$在$a$处趋于正（负）无穷大”的定义。\\
    \indent 2. 定义去心极限：设有实变函数$f$和实数$a, b$。如果对任何$r>0$，都存在$d>0$，使得只要$x\in(a-d;a)\cup(a;a+d)$，就有$f(x)\in(b-r;b+r)$，就说$f$在$a$处的\textbf{去心极限}是$b$。去心极限和极限有什么不同？
\end{sk}
...
\begin{xt}
    ...
    \indent 2. 考虑实变函数$f$和实数$a, b$。定义：如果对任何$r>0$，都存在$d>0$，使得只要$x\in(a-d;a\,]$，就有$f(x)\in(b-r;b+r)$，
    就说$f$在$a$处的\textbf{左极限}是$b$。如果$f$在$a$处有定义，那么$b=f(a)$。我们说$f$在$a$处\textbf{左连续}。\\
    \indent 2.1. 参照左极限和左连续的定义，给出函数\textbf{右极限}和\textbf{右连续}的定义。\\
    \indent 2.2. 参照极限的数列定义，给出左极限和右极限的数列定义。\\
    \indent 2.3. 证明：函数$f$在$a$处有极限，当且仅当它在$a$处有左极限和右极限，且左极限等于右极限。\\
    \indent 3. 定义去心极限：设有实变函数$f$和实数$a, b$。如果对任何$r>0$，都存在$d>0$，使得只要$x\in(a-d;a)\cup(a;a+d)$，就有$f(x)\in(b-r;b+r)$，
    就说$f$在$a$处的\textbf{去心极限}是$b$。\\
    \indent 3.1. 考虑函数：
    $$f:x\mapsto \begin{cases}
    x & x^2 \neq 0 \\
    1 & x = 0.
    \end{cases}
    $$
    \indent \qquad 证明$f$在$0$处有去心极限。\\
    \indent 3.2. 去心极限和极限的定义有什么不一样？\\
    \indent 3.3. 写出去心极限的数列定义，与极限的数列定义相比，有何不同？\\
    \indent \qquad 函数$f$把任何非零实数映射为$\displaystyle x\sin{\frac{\pi}{x}}$，$f(0) = 0$。函数$g$把任何非零实数映射到$0$，$g(0) = 1$。\\
    \indent 3.4. 证明：$f$在$0$处连续，$g$在$0$处有去心极限。\\
    \indent 3.5. 考虑通项$a_n = \frac{1}{n}$的数列$\left\{a_n\right\}_{n\in\mathbb{Z}^+}$。$\{g(f(a_n))\}$是否有极限？\\
    \indent 3.6. 考虑通项$b_n = \frac{2}{2n+1}$的数列$\left\{b_n\right\}_{n\in\mathbb{Z}^+}$。$\{g(f(b_n))\}$是否有极限？\\
    \indent 3.7. 以上结果说明$g\circ f$有怎样的性质？这说明什么问题？
\end{xt}

\section{极限的基本性质}
与数列极限一样，函数在一点的极限也可以进行运算。考虑函数$f,g$。设它们在某点$a$有极限，极限分别是$u, v$。按照定义，任意数列$\{a_n\}$趋于$a$时，$\{f(a_n)\}$趋于$u$，$\{g(a_n)\}$趋于$u$，因此$\{f(a_n) + g(a_n)\}$趋于$u + v$。这说明$f + g$在$a$处有极限，极限是$u + v$。同理，$f - g$在$a$处有极限，极限是$u - v$；$f \cdot g$在$a$处有极限，极限是$u \cdot v$。
$v\neq 0$时，$f\div g$在$a$处有极限，极限是$u \div v$。

再来看复合函数的极限。若实变函数$f$在$a$处的极限是$b$，$g$在$b$处的极限是$c$，那么按照定义，对任意$r>0$，首先存在$d>0$，使得$|y - a|<d$时总有$|g(y) - c| < r$；而对于$d$又有$s>0$使得$|x - a|<s$时总有$|f(x) - b| < d$。于是$|x - a|<s$时总有
$$|g(f(x)) - c| < r.$$
这说明$g\circ f$在$a$处有极限$c$。
...
\begin{align*}
    ...
    \lian{x\to a} (g\circ f)(x) &=  \lian{y \to \lian{x \to a} f(x)} g(y)
\end{align*}
根据连续和极限的关系，如果函数$f,g$在$a$处有连续，就是指$f,g$在$a$处有定义且有极限。那么，按照极限的运算律，$f\pm g$、$f\cdot g$在$a$处连续。$g(a)\neq 0$的话，$f\div g$在$a$处连续。若$g$在$f(a)$处有定义，那么$g\circ f$在$a$处连续。

以上的运算律这对我们计算复杂函数在一点的极限很有帮助。因为很多复杂函数可以看作简单函数四则运算以及复合的结果。
\begin{et}
    设$m$是正整数，求函数$f:x\mapsto (x+2)^m$在$x=0$处的极限。
\end{et}
\begin{so}
    考虑$g_1: x\mapsto x^m$，则$g_1 = h^m$，其中$h$是恒等函数。因此，对任意实数$a$，$f$在$a$处的极限，是$h$在$a$处极限的$m$次方。恒等函数$h$在$a$处的极限是$a$。所以，$g_1$在$a$处的极限等于$a^m$。

    又考虑$g_2: x\mapsto x + 2$，$g_2$是$h$与常函数$2$的和。所以对任意实数$a$，$g_2$在$a$处的极限是$h$在$a$处的极限与$2$的和，即$a+2$。
    
    $f = g_1\circ g_2$，所以$f$在$0$处的极限为：
    $$ \lian{x\to 0} f(x) = \lian{x\to 0} (g_1\circ g_2)(x) = \lian{y \to \lian{x \to 0} g_2(x)} g_1(y) =  \lian{y \to 2} g_1(y) = 2^m. $$
...
\begin{et}
    求函数$f:x\mapsto \displaystyle\frac{1}{x}$在无穷远处的极限。
\end{et}
\begin{so}
...
% 学习数列极限的时候，我们讨论过数列之商的极限。当作为分母的数列极限为$0$时，情况复杂，包含有我们称为不定式的情形。函数之商的极限也是如此。设函数$f, g$分别在一点$a$处有极限$u, v$。$v\neq 0$时，$f\div g$在$a$处的极限是$\frac{u}{v}$。如果$v = 0$，那么和数列一样，需要分为$u=0$和$u \neq 0$的情况讨论。

% 首先讨论$u\neq 0$的情况。和数列的情形类似，必须加上$g$在$a$附近恒不为$0$的假设，才能处理$f\div g$在$a$处的极限。比如，如果有$d>0$，使得$0<|x-a|<d$时$g(x)$总大于$0$，那么$\frac{1}{g}$在$a$处趋于正无穷。
...
\begin{xt}
    ...
    \indent 1.3. 函数$f:x\mapsto \displaystyle\frac{x}{1 + x}$在$x = 0$处的极限。\\
    \indent 1.4. 函数$f:x\mapsto \displaystyle\frac{2^x}{x^2}$在$x = 0$处的极限。\\
    \indent 2. 验证以下关于函数极限的性质。\\
    \indent 2.1. 证明：设函数$f$和$g$在点$a$处分别有极限$u,v$。如果有$d>0$使得$0<|x-a|<d$时总有$f(x)\leqslant g(x)$，则$u \leqslant v$。\\
    \indent 2.2. 第一问中的小于等于号能否改为小于号？\\
    \indent 2.3. 设函数$f$和$g$在点$a$处有相同极限$u$，$h$在$a$附近有定义。证明：如果有$d>0$使得$0<|x-a|<d$时总有$f(x)\leqslant h(x) \leqslant g(x)$，则$h$在$a$处有极限$u$。\\ \indent 2.4. 第三问中的小于等于号能否改为小于号？\\
    \indent 3. 设函数$f$和$g$对任意大的实数有定义。\\
    \indent 3.1. 证明：如果函数$f$和$g$在正无穷远处有极限$a, b$，则$f + g$、$f - g$、$f \times g$在正无穷远处也有极限，
    分别是$a + b$、$a - b$、$a \times b$。如果$b\neq 0$，那么$f \div g$在正无穷远处有极限$a \div b$。\\
    \indent 3.2. 证明：如果函数$f$和$g$在无穷远处都趋于正无穷大，那么$f + g$、$f \times g$在无穷远处也趋于正无穷大。如果$f$趋于正无穷大，$g$趋于负无穷大，那么$f - g$在无穷远处趋于正无穷大、$f \times g$在无穷远处趋于负无穷大。\\
    \indent 3.3. 如果函数$f$和$g$在无穷远处都趋于负无穷大，那么$f + g$、$f \times g$在无穷远处有何性质？\\
    \indent 3.4. 如果有某数$A$使得$x>A$时$f(x)$总小于等于$g(x)$，那么$f$和$g$在无穷远处的行为有何规律？\\
    \indent 3.5. 如果$f$和$g$在无穷远处有相同的极限$u$，或同时趋于正（负）无穷大；有函数$h$和数$A$使得$x>A$时总有$f(x)\leqslant h(x)\leqslant g(x)$，那么$h$在无穷远处的行为有何特征？\\
    \indent 4. 计算以下极限。\\
    \indent 4.1. $x\mapsto \displaystyle \frac{1}{x} - 1$在正无穷远处的极限。\\
    \indent 4.2. $x\mapsto \displaystyle 1 - \frac{2 - 3x}{1 + x}$在负无穷远处的极限。\\
    \indent 4.3. $x\mapsto \displaystyle\frac{x^2}{1 + x}$在无穷远处的极限。\\
    \indent 5. 考虑正弦函数$\sin$，它在无穷远处是否有极限？以此构造一个在$0$处没有极限的有界函数。
\end{xt}

\section{连续函数的基本性质}

再来看连续函数。连续函数有哪些呢？容易证明：常函数和恒等函数是连续函数。通过极限的运算法则，我们可以证明，对任何一元整式$P$，函数$x\mapsto P(x)$是连续函数，在$\mathbb{R}$上连续。

再来看另一类基本的函数：三角函数。可以证明，正弦函数和余弦函数都是连续函数，在$\mathbb{R}$上连续。首先证明正弦函数是连续函数。考虑任意实数$a$，我们证明正弦函数在$a$处连续。
...
使用和差化积公式。
\begin{align*}
    |\sin{x} - \sin{a}| = \left|2\cos\left(\frac{x+a}{2}\right)\sin\left(\frac{x-a}{2}\right)\right| \leqslant 2 \left|\sin\left(\frac{x-a}{2}\right)\right|. 
\end{align*}
我们知道，$u>0$时，$\sin{u}$小于$u$。而$u<0$时，$|\sin{u}| = -\sin{u} = \sin{-u} < -u$。
也就是说，总有$|\sin{u}| < |u|$。因此，
\begin{align*}
    |\sin{x} - \sin{a}| \leqslant 2 \left|\sin\left(\frac{x-a}{2}\right)\right| <  \left|x-a\right|. 
\end{align*}
...
对任何实数$x$，总有$\sin{x} = \cos{\left(\frac{\pi}{2} - x\right)}$。从复合函数的角度来看，余弦函数可以看作正弦函数和$x\mapsto \frac{\pi}{2} - x$的复合函数。这两个函数都在定义域上处处连续，因此其复合函数：余弦函数在定义域上也处处连续。

\begin{et}
    求函数$f:x\mapsto \displaystyle\frac{\sin{x}}{x}$在$0$处的极限。
\end{et}
\begin{so}
    我们已经证明过，任何非零数列$\{a_n\}$趋于$0$的时候，$\displaystyle\frac{\sin{a_n}}{a_n}$趋于$1$。
    因此，$f$在$0$处的极限是$1$。要注意的是$f$在$0$处没有定义。
\end{so}

继续研究$f$，$x\neq 0$时，容易验证$f$作为两个连续函数之商，总是连续函数。而如果我们定义$f(0) = 1$，那么$f$在$0$处的极限就等于它在$0$处的值。因此，可以定义新函数：
$$
\bar{f}\mapsto \begin{cases}
    \displaystyle\frac{\sin{x}}{x} & \mbox{如果}x\neq 0 \\
    1 & \mbox{如果}x= 0
\end{cases}
$$
$f$的定义域是非零实数，而$\bar{f}$的定义域是全体实数。$\bar{f}$在$x\neq 0$时等于$f$，而且在$0$处也连续。我们说$\bar{f}$是$f$的\textbf{连续延拓}。其中“延拓”是指把函数定义域扩展到更大的母集，并保持在原集合上取值不变。如果函数的延拓保持了原函数的某种良好性质，就说它是带有这种性质的延拓。比如$\bar{f}$是$f$延拓到全体实数的结果，并保持了连续性质，所以称为连续延拓。
...
直观上，我们在平面上画出$f$的图像。这是一条从$(a;f(a))$出发，到$(b;f(b))$截止，连续不断的线。直线$y = v$把平面分成两侧。而$f$的图像中，点$(a;f(a))$和点$(b;f(b))$分别在直线两侧。

介值定理告诉我们，连续函数的图像，作为连续不断的线，为了“穿过”直线$y = v$，总会与它相交。

更进一步，我们可以证明：\textbf{闭区间上的连续函数，值域也是闭区间}。具体来说，函数$f$在闭区间$[\,a;b\,]$上连续，那么有$a\leqslant x_{\text{小}} , x_{\text{大}} \leqslant b$，使$y_{\text{小}} = f(x_{\text{小}})$和$y_{\text{大}} = f(x_{\text{大}})$分别是$f$在$[\,a;b\,]$上的最小值和最大值。而介值定理告诉我们：$\forall y\in[\,y_{\text{小}};y_{\text{大}}\,]$，都有$c\in[\,a;b\,]$使得$f(c) = y$。这就说明$f$在$[\,a;b\,]$上能取到$[\,y_{\text{小}};y_{\text{大}}\,]$中一切值。

\begin{sk}
    \mbox{} \\
    \indent 1. 设实变函数$f$在区间$I$上连续且严格单调递增，把$I$映射到区间$J$。那么$f$的反函数$g$是否在$J$上连续？\\
    \indent 2. 举例说明，如果函数$f$在$[\,a;b\,]$上不连续，那么可能有某个数$v$介于$f(a)$和$f(b)$之间，并且$(a;b)$中任何数$u$经$f$映射的值都不等于$v$。\\
    \indent 3. 任何在$\mathbb{R}$上处处连续的函数的图像，是否都把平面分成两部分？在两侧各取一点，是否能把它们用曲线连起来，而不碰到函数的图像？
\end{sk}
...
\begin{xt}
    ...
    \indent 2.1. $x\mapsto \displaystyle\frac{x}{x + 1}$在$0$处连续。\\
    \indent 2.2. $x\mapsto \displaystyle\frac{2 + 2x - 3x^2}{x - 1}$在$0$处连续。\\
    \indent 2.3. $x\mapsto \displaystyle\frac{1}{x^2 + 1}$在$\mathbb{R}$上连续。\\
    \indent 2.4. 如果$a\neq 2$，那么$x\mapsto \displaystyle\frac{1 - x}{2 - x}$在$a$处连续。\\
    \indent 3. 证明：正切函数、余切函数在其定义域上是连续函数。\\
    \indent 4. 考虑函数：
    $$
        g_a: \;\,\,x \mapsto \left\{ \begin{array}{cc}
            \displaystyle\frac{1 - \cos{x}}{x^2},&\quad \mbox{如果}x\neq 0 \\
            a &\quad \mbox{如果}x = 0. 
        \end{array}\right.
    $$
    实数$a$取什么值的时候，$g_a$在$0$处连续？\\
    \indent 5. 证明：如果函数$f$在$[\,a;b\,]$上连续且严格单调，那么$f(a)$和$f(b)$之间任一个数$v$总恰是$(a, \,\,b)$中一个数$u$经$f$映射的值：$f(u) = v$。\\
    \indent 6. 实变函数$f$在点$a$处连续，$f(a)>0$。证明：$f$在$a$附近大于$0$。
\end{xt}
...
\begin{proof}
    ...
    根据前面的证明，我们知道$\{2^{\frac{1}{n}}\}$、$\{2^{-\frac{1}{n}}\}$趋于$1$。所以对$\frac{s}{2^M}>0$，总能找到$N_0$，使得$n>N_0$时总有
...
最后，我们可以验证这样定义的$f_{\mathbb{R}}$确实是连续函数（证明见附录B第一节末尾）。

这样，我们就把定义在有理数集上的函数$f_{\mathbb{Q}}$延拓为定义在实数集上的函数$f_{\mathbb{R}}$。
...
\begin{figure}[h]
    ...
    \begin{scope}[shift={(0,0)}]
        ...
        \def\ymax{6.4};
        % 坐标轴（带箭头）
        \foreach \k in {-1,1,2,...,6} {
            \draw[help lines,gray!60] (\xmin,\k) -- (\xmax,\k) ;
        }
        \foreach \k in {-5,-4,...,-1,1,2,...,8} {
            \draw[help lines,gray!60] (\k,\ymin) -- (\k,\ymax);
        }
        \draw[-Stealth, thick] (\xmin, 0) -- (\xmax,0) node[above] {$x$};
        \draw[-Stealth, thick] (0,\ymin) -- (0,\ymax) node[right] {$y$};
        \foreach \k in {1,2,...,8} { 
            \node at (\k,-0.3) {$\mbox{\scriptsize $\k$}$};
        }
        \foreach \k in {1,2,...,6} { 
            \node at (-0.3,\k) {$\mbox{\scriptsize $\k$}$};
        }
        \node at (-0.3,-0.3) {$\mbox{\scriptsize $0$}$};
        \node at (-0.4,-1) {$\mbox{\scriptsize $-1$}$};
        \foreach \k in {-5,-4,-3,-2,-1} {  % ,
            \node at (\k-0.15,-0.3) {$\mbox{\scriptsize $\k$}$};
        }
        % 作图
        \draw[CadetBlue, very thick] (\xmin, 1) -- (\xmax, 1);
        \draw[RoyalPurple, very thick, domain=\xmin:2.69, samples=200]
        plot (\x,{exp(\x*ln(2))});
        \draw[OliveGreen, very thick, domain=\xmin:1.16, samples=200]
        plot (\x,{exp(\x*ln(5))});
        \draw[red!80!black, very thick, domain=\xmin:7.1, samples=200]
        plot (\x,{exp(\x*ln(1.3))});  
        \draw[Goldenrod!70!orange, very thick, domain=\xmin:\xmax, samples=200]
        plot (\x,{exp(\x*ln(0.7))});
        \draw[Thistle, very thick, domain=-2.68:\xmax, samples=200]
        plot (\x,{exp(\x*ln(0.5))});
        \draw[Cerulean, very thick, domain=-1.34:\xmax, samples=200]
        plot (\x,{exp(\x*ln(0.25))});
        ...
        \def\ygp{0.45};
        \def\sip{0.02};
        \draw[very thick, fill=white] (3.8,5.25) rectangle (5.8,6.15);
        \node at (4.8, 5.7) {$\mbox{$y =  a^x$}$};
        \draw[very thick, fill=white] (\lb-0.1,\la) rectangle (8.2,\la+2*\ypd+6*\ygp);
        \draw[Cerulean,ultra thick] (\lb,\la+\ypd) -- (\lb+\ba,\la+\ypd);
        \draw[Thistle,ultra thick] (\lb,\la+\ypd+1*\ygp) -- (\lb+\ba,\la+\ypd+1*\ygp);
        \draw[Goldenrod!70!orange,ultra thick] (\lb,\la+\ypd+2*\ygp) -- (\lb+\ba,\la+\ypd+2*\ygp);
        \draw[CadetBlue,ultra thick] (\lb,\la+\ypd+3*\ygp) -- (\lb+\ba,\la+\ypd+3*\ygp);
        \draw[red!80!black,ultra thick] (\lb,\la+\ypd+4*\ygp) -- (\lb+\ba,\la+\ypd+4*\ygp);
        \draw[RoyalPurple,ultra thick] (\lb,\la+\ypd+5*\ygp) -- (\lb+\ba,\la+\ypd+5*\ygp);
        \draw[OliveGreen,ultra thick] (\lb,\la+\ypd+6*\ygp) -- (\lb+\ba,\la+\ypd+6*\ygp);
        \node at (7.35,\la+\ypd+\sip) {$\mbox{\footnotesize $a=0.25$}$};
        \node at (7.25,\la+\ypd+1*\ygp+\sip) {$\mbox{\footnotesize $a=0.5$}$};
        \node at (7.25,\la+\ypd+2*\ygp+\sip) {$\mbox{\footnotesize $a=0.7$}$};
        \node at (7.11,\la+\ypd+3*\ygp+\sip) {$\mbox{\footnotesize $a=1$}$};
        \node at (7.25,\la+\ypd+4*\ygp+\sip) {$\mbox{\footnotesize $a=1.3$}$};
        \node at (7.11,\la+\ypd+5*\ygp+\sip) {$\mbox{\footnotesize $a=2$}$};
        \node at (7.11,\la+\ypd+6*\ygp+\sip) {$\mbox{\footnotesize $a=5$}$};
    \end{scope}
    \end{tikzpicture}
...
底数不等于$1$时，指数函数总是严格单调的函数。所以，我们还可以定义指数函数的反函数。
我们把底数为$a$的指数函数的反函数称为底数为$a$的\textbf{对数函数}，记为$\log_a$。
$$ \log_a : \,\, x \mapsto \log_a(x). $$
如果要讨论某个对任何（有效）底数都成立的性质，我们也可以忽略下标$a$，将对数函数记为$\log$。

指数函数的值域是$(0;+\infty)$，所以对数函数的定义域是$(0;+\infty)$。给定正数$u$，$\log_a(u)$就是使得$a^v = u$的唯一实数$v$。

$$ a^v = u \quad\mbox{当且仅当} \quad v = \log_a(u). $$
...
\begin{figure}[h]
    \vspace{4pt}
    \centering
    % \includegraphics[width=0.8\textwidth]{tu/对数函数1.png}
    \begin{tikzpicture}[scale=0.6]
    \begin{scope}[shift={(0,0)}]
        \def\xmin{-4.2};
        \def\xmax{11.5};
        ...
        \def\ymax{7};
        % 坐标轴（带箭头）
        \foreach \k in {-6,-5,...,-1,1,2,...,6} {
            \draw[help lines,gray!60] (\xmin,\k) -- (\xmax,\k) ;
        }
        \foreach \k in {-4,-3,...,-1,1,2,...,11} {
            \draw[help lines,gray!60] (\k,\ymin) -- (\k,\ymax);
        }
        \draw[-Stealth, thick] (\xmin, 0) -- (\xmax,0) node[above] {$x$};
        \draw[-Stealth, thick] (0,\ymin) -- (0,\ymax) node[right] {$y$};
        ...
        \foreach \k in {1,2,...,5} { 
            \node at (-0.3,\k) {$\mbox{\scriptsize $\k$}$};
            \node at (-0.45,-\k) {$\mbox{\scriptsize $-\k$}$};
            \ifnum\k<5
              \node at (-\k-0.15,-0.3) {$\mbox{\scriptsize $-\k$}$};
            \fi
        }
        \node at (-0.3,-0.3) {$\mbox{\scriptsize $0$}$};
        \node at (-0.3,6) {$\mbox{\scriptsize $6$}$};

        % 作图
        \draw[RoyalPurple, very thick, domain=0.0155:\xmax, samples=200]
        plot (\x,{ln(\x)/ln(2)});
        \draw[OliveGreen, very thick, domain=0.00006:\xmax, samples=200]
        plot (\x,{ln(\x)/ln(5)});
        \draw[red!80!black, very thick, domain=0.208:6.4, samples=200]
        plot (\x,{ln(\x)/ln(1.3)});
        \draw[Goldenrod!70!orange, very thick, domain=0.085:8.5, samples=200]
        plot (\x,{ln(\x)/ln(0.7)});
        \draw[Thistle, very thick, domain=0.01:\xmax, samples=200]
        plot (\x,{ln(\x)/ln(0.5)});
        \draw[Cerulean, very thick, domain=0.0001:\xmax, samples=200]
        plot (\x,{ln(\x)/ln(0.25)});
        ...
        \def\sip{0.02};
        \def\bu{0.9};
        \draw[very thick, fill=white] (7,5) rectangle (10.8,6.6);
        \node at (8.9, 5.8) {$\mbox{\small $y = \log_a{(x)}$}$};
        \draw[very thick, fill=white] (\lb-0.3,\la) rectangle (-0.7,\la+2*\ypd+5*\ygp);
        \draw[Cerulean,ultra thick] (\lb,\la+\ypd) -- (\lb+\ba,\la+\ypd);
        \draw[Thistle,ultra thick] (\lb,\la+\ypd+1*\ygp) -- (\lb+\ba,\la+\ypd+1*\ygp);
        \draw[Goldenrod!70!orange,ultra thick] (\lb,\la+\ypd+2*\ygp) -- (\lb+\ba,\la+\ypd+2*\ygp);
        \draw[red!80!black,ultra thick] (\lb,\la+\ypd+3*\ygp) -- (\lb+\ba,\la+\ypd+3*\ygp);
        \draw[RoyalPurple,ultra thick] (\lb,\la+\ypd+4*\ygp) -- (\lb+\ba,\la+\ypd+4*\ygp);
        \draw[OliveGreen,ultra thick] (\lb,\la+\ypd+5*\ygp) -- (\lb+\ba,\la+\ypd+5*\ygp);
        \node at (\lb+\ba+\bu+0.2+0.13,\la+\ypd+\sip) {$\mbox{\footnotesize $a=0.25$}$};
        \node at (\lb+\ba+\bu+0.2,\la+\ypd+1*\ygp+\sip) {$\mbox{\footnotesize $a=0.5$}$};
        \node at (\lb+\ba+\bu+0.2,\la+\ypd+2*\ygp+\sip) {$\mbox{\footnotesize $a=0.7$}$};
        \node at (\lb+\ba+\bu+0.2,\la+\ypd+3*\ygp+\sip) {$\mbox{\footnotesize $a=1.3$}$};
        \node at (\lb+\ba+\bu,\la+\ypd+4*\ygp+\sip) {$\mbox{\footnotesize $a=2$}$};
        \node at (\lb+\ba+\bu,\la+\ypd+5*\ygp+\sip) {$\mbox{\footnotesize $a=5$}$};
    \end{scope}
    \end{tikzpicture}
...
\textbf{再次强调}：\textbf{对数函数的底数不能是}$1$。

作为指数函数的反函数，对数函数满足以下性质：
$$ \forall a, b > 0, \quad\log(a\cdot b) = \log(a) + \log(b). $$

指数函数和对数函数来自天文学的需求。天文学家处理各种天体之间的距离时，需要记录数值差别非常大的数据。这些数值的精确程度不高，但它们之间的比例关系比较重要。为了方便处理这些数据，天文学家希望把乘除法运算转换为加减法运算，创造了对数法，通过查对数表来把大数的乘除法转为加减法。

从今天的角度来看，对照数表蕴含的关系就是指数和对数函数。对数就是实际计算问题中遇到的真数在表中对应的数，指数就是把对数作加减法后，指向的真数，也就是计算结果。计算过程用现代的记号表示，就是：
...
\end{tm}
\begin{proof}
    对于(1)：$\{a_n\} \leqslant \{b_n\}$，就是说$\forall\,n$，$a_n - b_n \leqslant 0$，因此根据定理\ref{tm:a-0-40}，其极限$x - y\leqslant 0$。同理可以证明(2)、(3)、(4)。
\end{proof}
...
\end{tm}
\begin{proof}
    设数列$\{a_n\}$是自敛数列。按照定义，对正数$1$来说，有正整数$N$，使得只要$n,m>N$，就有$|a_n - a_m| < 1$。这说明$n>N$时，
...
\begin{tm}\label{tm:a-0-65}
    自敛数列的子列还是自敛数列。
\end{tm}
\begin{proof}
    设数列$\{a_n\}$自敛。构造出子列$\{a_n'\}$的映射$\phi$必满足$\phi(n)\geqslant n$。对任意$r>0$，有$N$使得$n,m>N$时总有$|a_n - a_m| < r$，那么$n,m>N$时$\phi(n),\phi(m)>N$，于是$|a_n' - a_m'| = |a_{\phi(n)} - a_{\phi(m)}| < r$。即子列$\{a_n'\}$也自敛。
\end{proof}

\begin{tm}\label{tm:a-0-67}
    自敛数列不趋于有理数$a$，则其任意子列不趋于$a$。
\end{tm}
\begin{proof}
    设数列$\{a_n\}$自敛，则$\forall \, r > 0$，有正整数$N$使得$n,m>N$时总有$|a_n - a_m| < r$。反设有子列$\{a_{\phi(n)}\}$趋于$a$。则有正整数$M$使得$m>M$时总有$|a_{\phi(m)} - a| < r$。于是$n>N$时，取$m>N+M$，则$\phi(m)\geqslant m>N+M$，因而
    $$ |a_n - a| \leqslant |a_n - a_{\phi(m)}| + |a_{\phi(m)} - a| < r + r = 2r.$$
    这说明$\{a_n\}$趋于$a$，矛盾！
\end{proof}
...
\end{tm}
\begin{proof}
    对$T$中任意正数$r$，首先可以找到正整数$M$，使得$n>M$时总有$|b_n| < r$。于是，按照条件，存在正整数$N$，使得只要$m,k>N$，就有
    $$|a_k - a_m| < b_{M+1} \leqslant |b_{M+1}| < r.$$
    这说明$\{a_n\}$是自敛数列。
\end{proof}
...
\begin{tm}\label{tm:a-1-3}
    自敛有理数列经过四则运算仍然是自敛有理数列\footnote{约定：除法中作为被除数的数列的绝对值数列有大于$0$的下界。}。
\end{tm}
\begin{proof}
    \mbox{} \\
    \textbf{加减法}：设有元素在数集$T$中的自敛数列$\{a_n\}, \{b_n\}$。对$T$中任意正数$r$，记$r_1=\frac{r}{2}>0$，按照定义，有正整数$N_a, N_b$，
    使得只要$n,m>N_a$，就有$|a_n - a_m| < r_1$；只要$n,m>N_b$，就有$|b_n - b_m| < r_1$。
    因此，$n,m>N_a+N_b$时，总有$|a_n - a_m| < r_1$、$|b_n - b_m| < r_1$同时成立。于是
    ...
    $|a_n|$总小于$M_a$、$|b_n|$总小于$M_b$。
    对$T$中任意正数$r$，记$r_1 = \frac{r}{M_a + M_b}>0$。按照定义，有正整数$N_a, N_b$，使得只要$n,m>N_a$，就有$|a_n - a_m| < r_1$；只要$n,m>N_b$，就有$|b_n - b_m| < r_1$。
    因此，$n,m>N_a+N_b$时，总有$|a_n - a_m| < r_1$、$|b_n - b_m| < r_1$同时成立。
    于是：
    \begin{align*}
        |a_nb_n - a_mb_m| &= |(a_n - a_m)b_n + a_m(b_n - b_m)|  \\
        &\leqslant |(a_n - a_m)b_n| + |a_m(b_n - b_m)| = |a_n - a_m|\cdot |b_n| + |b_n - b_m|\cdot|a_m|  \\
        &< \frac{r}{M_a + M_b} \cdot M_b + \frac{r}{M_a + M_b} \cdot M_a = r. 
    \end{align*}
    因此数列$\{a_n b_n\}$也是自敛数列。\\
    \textbf{除法}：设有元素在数集$T$中的自敛数列$\{a_n\}, \{b_n\}$。，不妨设$b_n$的绝对值总大于$T$中元素$d$（$d > 0$）。对$T$中任意正数$r$，记$r_1 = d^2r$，按照定义，总有正整数$N$使得只要$n,m>N$，就有$|b_n - b_m| < r_1$。于是
    $$ \left| \frac{1}{b_n} - \frac{1}{b_m}\right| = \frac{|b_n - b_m|}{|b_m|\cdot |b_n|} < \frac{r_1}{d^2} = r. $$
    因此$\left\{\frac{1}{b_n}\right\}$是自敛数列。按照前面结论，数列$\{\frac{a_n}{b_n}\}$作为$\{a_n \cdot \frac{1}{b_n}\}$也是自敛数列。
\end{proof}
...
\begin{df}\textbf{实数的相等关系}\\
    ...
    $a = b$当且仅当$\{a_n - b_n\}$趋于$0$。称之为\textbf{相等关系}。
\end{df}
...
考虑所有自敛有理数列的集合$\mathcal{R}$，将$\mathcal{R}$中元素按极限相等分成子集。每个数列恰属于一个这样的子集。这些子集构成了$\mathcal{R}$的分划。我们说每个子集是一个等价类。每个实数恰好对应一个等价类，每个等价类也恰好对应一个实数。因此，我们可以用这些等价类来表示实数。
...
\begin{tm}\label{lm:a-1-10}
    两个等价类要么交集为空，要么相等。    
\end{tm}
也就是说，如果某等价类有一个元素在另一个等价类中，那么两个等价类是同一个等价类。因此，两个实数$a,b$不等，说明交集为空。进一步地，如果$a<b$，应该意味着交集为空；并且从$a,b$中各自任取一个自敛列$\{a_n\}$和$\{b_n\}$，从某一项开始，都总有$a_n < b_n$。为了方便，我们把这个自然的定义转化为一个更好用的判断标准。
\begin{tm}\label{tm:a-1-12}
    设有实数$a,b$，$a<b$（或说$b>a$）当且仅当对任意从$a,b$中各取的数列$\{a_n\}$、$\{b_n\}$，存在有理数$r>0$、正整数$N$，使得$n>N$时总有$a_n+r<b_n$。
\end{tm}
\begin{proof}
    首先证明正向：从$a,b$中各取数列$\{a_n\}$、$\{b_n\}$，$a<b$说明$\{a_n - b_n\}$不趋于$0$。于是依定理\ref{tm:a-0-67}，$\{a_n - b_n\}$的任何子列不趋于$0$。\\
    反设不存在$r,N$使得$n>N$时总有$a_n+r<b_n$，也就是说对任意$r,N$，都能找到$n>N$使得$a_n+r\geqslant b_n$，那么由于从某个$M$开始，$n>M$时总有$a_n < b_n$，我们取如下子列：$n_1$是大于$M$的第一个使得$a_n<b_n\leqslant a_n+1$的下标$n$，$n_2$是大于$n_1$的第一个使得$a_n<b_n\leqslant a_n+\frac{1}{2}$的下标$n$，以此类推，$n_k$是大于$n_{k-1}$的第一个使得$a_n<b_n\leqslant a_n+\frac{1}{k}$的下标$n$。这样取出的$\{a_n - b_n\}$的子列趋于$0$。矛盾！于是对$\{a_n\}$、$\{b_n\}$，存在$r,N$使得$n>N$时总有$a_n+r<b_n$。\\
    再证明反向：既然对任意$\{a_n\}$、$\{b_n\}$，存在$r,N$使得$n>N$时总有$a_n+r<b_n$，$\{a_n - b_n\}$按定义不趋于$0$，且$n>N$时总有$a_n<b_n-r<b_n$。因此$a<b$。
\end{proof}
...
\begin{tm}\label{tm:a-1-15}
    ...
    \begin{enumerate}
        ...
        \item \textbf{三分律}：$\forall a, b$，$a < b$，$b < a$，$a = b$恰有一个成立。
        \item \textbf{传递性}：$\forall a, b, c$，如果$a < b$、$b < c$，则$a < c$。
    \end{enumerate}
\end{tm}
\begin{proof}
    \mbox{} \\
    \textbf{自背性}： $\forall a$，$a$与自身交集不为空，因此$a<a$不成立。\\
    \textbf{三分律}： $\forall a, b$，如果$a = b$，那么两者对应同一个等价类，从上一条性质可知，$a<b$、$b<a$都不成立。\\
    如果$a = b$不成立，那么存在分别收敛到$a,b$的自敛数列$\{a_n\}, \{b_n\}$，差数列$\{a_n - b_n\}$不趋于$0$。也就是说，存在有理数$r>0$，使得对任何正整数$m$，都有$n_m>m$，使得$|a_{n_m} - b_{n_m}| > r$。于是我们可以这样取得$\{a_n\}$、$\{b_n\}$的子列：首先令$m=1$，按以上条件找到$n_1>1$，再找到$n_2>n_1$，$n_3>n_2$等等。这样取出的子列$\{a_n'\}$、$\{b_n'\}$，每一项作差都大于$r$：要么$a_{n}' > b_{n}' + r$，要么$a_{n}' < b_{n}' - r$。然而依定理\ref{tm:a-0-65}，$\{a_n'\}$、$\{b_n'\}$都是自敛数列，所以有正整数$N$使得只要$n,m>N$，就有$|a_n' - a_m'| < r$、$|b_n' - b_m'| < r$。若有$n,m>N$使得$a_{n}' < b_{n}' - r$且$a_{m}' > b_{m}' + r$，那么：
    $$ r > b_{n}' - b_m' = b_{n}' - a_{n}' + (a_{n}' - a_m') + (a_m' - b_m') > r + (-r) + r = r.$$
    矛盾！也就是说，$n>N$后，要么永远有$a_{n}' < b_{n}' - r$，要么永远有$a_{n}' > b_{n}' + r$。依据定理\ref{tm:a-1-12}，前者说明$a<b$，后者说明$b<a$。\\
    综上，我们证明了$\forall a, b$，$a < b$，$a > b$，$a = b$恰有一个成立。\\
    \textbf{传递性}：$\forall a, b, c$，如果$a < b$、$b < c$，考虑任意收敛到$a,b,c$的自敛有理数列$\{a_n\}$、$\{b_n\}$、$\{c_n\}$，那么有$r_1, r_2>0$和正整数$N_1, N_2$，使得自敛数列$\{b_n - a_n\}$、$\{c_n - b_n\}$分别自第$N_1$项和$N_2$项往后总大于$r_1, r_2$。
    因此，对任意$n>N_1 + N_2$，
    $$ c_n - a_n = c_n - b_n + b_n - a_n > r_1 + r_2.$$
    这说明$a<c$。
\end{proof}

定义其他不等关系：$a>b \Leftrightarrow b<a$；$a\leqslant b \Leftrightarrow a < b$或$a = b$；$a\geqslant b \Leftrightarrow a > b$或$a = b$。

\begin{tm}\label{tm:a-1-18}
    若实数$a<b$，那么$a,b$之间总有有理数。
\end{tm}
\begin{proof}
    首先定义什么是“之间有有理数”。对实数$a<b$和有理数$c$，如果从$a,b$中任取数列$\{a_n\}$、$\{b_n\}$，都有$N$使得$n>N$时总有$a_n < c < b_n$，就说$c$在$a,b$之间。

    设实数$a<b$，则对从$a,b$中任取的自敛数列$\{a_n\}$、$\{b_n\}$，总有$r>0$和正整数$N$使得$n>N$时$a_n + r < b_n$。而由自敛性质可知有正整数$N_a, N_b$使得$ n,m > N_a$时总有$|a_n - a_m| < \frac{r}{2}$、$ n,m > N_b$时总有$|b_n - b_m| < \frac{r}{2}$。于是取$M = N+N_a+N_b$，$c = \frac{a_{M} + b_{M}}{2}$。由于$M>N$，$b_{M} - a_{M} > r$，于是：
    \begin{align*}
       \forall \,n > M,\quad a_n &= a_n - a_{M} + a_{M} < \frac{r}{2} + a_{M} < \frac{b_M - a_M}{2} + a_M = c, \\
        b_n &= b_n - b_{M} + b_{M} > -\frac{r}{2} + a_{M} > \frac{a_M - b_M}{2} + b_M = c. 
    \end{align*}
    这就说明有理数$c$在$a,b$之间。
\end{proof}
...
\begin{proof}
    ...
    \begin{align*}
        ...
        \{a_nb_n - a_n'b_n'\} &\to 0  \\
        \left\{\frac{a_n}{b_n} - \frac{a_n'}{b_n'}\right\} &\to 0  
    \end{align*}
    ...
    \end{align*}
    对任何有理数$r>0$，$r_1 = \frac{r}{2M}>0$，因此有正整数$N_a,N_b$，
    使得$n>N_a$时总有$|a_n - a_n'| < r_1$，$n>N_b$时总有$|b_n - b_n'| < r_1$。
    取$N=N_a+N_b$，就有
    ...
    另一方面，由于$b>0$，根据定理\ref{tm:a-1-12}，有有理数$d$、正整数$N$使得只要$n>N$，就有$b_n>d$、$b_n'>d$。
    \begin{align*}
    ...
    \end{align*}
    对任何有理数$r>0$，$r_1 = \frac{rd^2}{M+d}>0$，因此有正整数$N_a,N_b$，
    使得$n>N_a$时总有$|a_n - a_n'| < r_1$，$n>N_b$时总有$|b_n - b_n'| < r_1$。
    取$N=N_a+N_b$，则：
    ...
    (4). 考虑$a - b + b$等价类中的元素，它能写成形如$\{a_n - b_n + b_n'\}$的数列，其中$\{a_n\}$收敛到$a$，$\{- b_n + b_n'\}$收敛到$0$，$\{a_n - b_n + b_n'\}$与$\{a_n\}$相差一个无穷小，因此收敛到$a$。
    % 反之，收敛到$a$的数列$\{a_n\}$总能写成$\{a_n - b_n + b_n\}$的形式，其中$\{b_n\}$是任意收敛到$b$的数列。
    这说明$a - b + b = a$。同理可证$a + b - b = a.$\\
    (5). 考虑$a \div b \times b$等价类中的元素，它能写成形如$\{a_n \div b_n \times b_n'\}$的数列，其中$\{a_n\}$收敛到$a$，$\{b_n\}$、$\{b_n'\}$收敛到$b$。
    考虑数列$\{\frac{a_n b_n}{b_n'} - a_n\}$。
    $\{a_n\}$自敛，所以有界，有有理数$M$使得$|a_n|$总小于$M$。$b\neq 0$，所以有有理数$d>0$、正整数$N$，使得只要$n>N$，就有$|b_n'| > d$。
    于是：
    \begin{align*}
       \forall\,n>N,\quad \left|\frac{a_n b_n}{b_n'} - a_n\right| = \frac{\left|a_n\right| \left|b_n - b_n'\right|}{\left|b_n'\right|}  < \frac{M}{d}\left|b_n - b_n'\right|. 
    \end{align*}
...
实数的相等关系是等价关系，因此等式两边是同一个等价类。于是实数相等关系自然与四则运算兼容：
...
\end{tm}
\begin{proof}
    设不等关系为“$<$”，不等式两边的值分别是实数$a<b$。\\
    (1). 在不等式两边加上（减去）实数$c$，
    左边得到$a\pm c$，右边得到$b\pm c$。对$a\pm c$、$b\pm c$中任意元素，它们分别能写成$\{a_n\pm c_n\}$、$\{b_n\pm c_n'\}$的形式。其中$\{a_n\}$、$\{b_n\}$取自$a$、$b$，$\{c_n\}$、$\{c_n'\}$取自$c$。\\%$\{a_n\pm c_n-(b_n\pm c_n')\} = \{(a_n - b_n) \pm  (c_n - c_n')\}$。\\
    $a<b$，因此存在有理数$r>0$和正整数$N$，使得$n>M$时总有$a_n+r<b_n$。另一方面$\{c_n - c_n'\}$是无穷小，所以对于$\frac{r}{2}>0$，存在正整数$N$，使得$n>N$时总有$|c_n - c_n'| < \frac{r}{2}$。于是$n>N$时总有
    \begin{align*}
        a_n\pm c_n-(b_n\pm c_n') = a_n - b_n \pm  (c_n - c_n') < -r + \frac{r}{2} = -\frac{r}{2}
    \end{align*}
    这说明$a\pm c < b\pm c$。\\
    (2). 设另有不等式$c<d$，下面证明$a + c < b + d$。对$a + c$、$b + d$中任意元素，它们分别能写成$\{a_n + c_n\}$、$\{b_n + d_n\}$的形式。其中$\{a_n\}$、$\{b_n\}$
    取自$a,b$，$\{c_n\}$、$\{d_n\}$取自$c,d$。\\%$\{a_n + c_n-(b_n + d_n)\} = \{a_n - b_n + (c_n - d_n)\}$。
    由于$a < b$，$c < d$，有正有理数$r_1,r_2$，正整数$N_1,N_2$，使得$n>N_1$时总有$a_n + r_1 < b_n$，$n>N_2$时总有$c_n +r_2 < d_n$。于是
    \begin{align*}
        a_n + c_n-(b_n + d_n) = a_n - b_n +  (c_n - d_n) < -r_1 - r_2 .
    \end{align*}
    这说明$a + c < b + d$。\\
    (3). 在不等式两边乘以实数$c$，左边得到$a\cdot c$，右边得到$b\cdot c$。$a\cdot c$、$b\cdot c$中任意元素，分别能写成$\{a_n\cdot c_n\}$、$\{b_n\cdot c_n'\}$的形式。其中$\{a_n\}$、$\{b_n\}$取自$a$、$b$，$\{c_n\}$、$\{c_n'\}$取自$c$。注意到
    \begin{align*}
        a_n\cdot c_n - b_n\cdot c_n' &= (a_n - b_n)\cdot c_n + b_n\cdot(c_n - c_n')  
    \end{align*}
    由于$a< b$，有有理数$r_1>0$，正整数$N$，使得$n>N$时总有$a_n + r_1 < b_n$。
    另一方面，自敛数列$\{b_n\}$有界，存在有理数$M$使得$|b_n|$总小于$M$。\\
    如果$c > 0$，那么存在有理数$r_2>0$，正整数$N'$，使得$n>N'$时总有$c_n > r_2$。
    而对于$\frac{r_1r_2}{2M}>0$，存在正整数$N_c$，
    使得$n>N_c$时总有$|c_n - c_n'| < \frac{r_1r_2}{2M}$。于是$n>N$时总有
    \begin{align*}
        a_n\cdot c_n - b_n\cdot c_n' &= (a_n - b_n)\cdot c_n + b_n\cdot(c_n - c_n')  \\
        &< -r_1r_2 + M\cdot\frac{r_1r_2}{2M} = -\frac{r_1r_2}{2} < 0 .
    \end{align*}
    这说明$a\cdot c < b\cdot c$。不等关系不变。\\
    类似地，如果$c < 0$，则存在有理数$r_2>0$，正整数$N'$，使得$n>N'$时总有$c_n + r_2 < 0$。取同样的$N_c$。这时有
    \begin{align*}
        a_n\cdot c_n - b_n\cdot c_n' &= (a_n - b_n)\cdot c_n + b_n\cdot(c_n - c_n')  \\
        &> r_1r_2 - M\cdot\frac{r_1r_2}{2M} = \frac{r_1r_2}{2} > 0 
    \end{align*}
    这说明$a\cdot c > b\cdot c$。不等式反号。\\
    如果不等关系为大于关系，由于$a>b$就是$b<a$，因此以上结论中把$a,b$互换，即可证明关于大于关系的结论。而大于等于、小于等于关系可看作“大于或等于”、“小于或等于”，因此附加验证等于的情形即可。  
\end{proof}
...
考虑前两节中数列的基本性质，它们的证明过程中只涉及四则运算与不等式，而我们已经证明，实数的四则运算与有理数一致，并与相等、不等关系兼容。因此，这些结论扩展到实数列的情况下也成立。
...
\end{tm}
\begin{proof}
    按照定义，实数$a$是某个自敛有理数列$\{a_n\}$的极限，因此，按照定义，对任何对任何有理数$r>0$，总存在正整数$N$，使得$n>N$时总有$|a_n - a| < r$。因此，选$\{a_n\}$中下标大于$N$的项即可。
\end{proof}
...
\begin{proof}
    ...
    首先证明$\{q_n\}$是自敛有理数列。考虑数列$\{c_n\}$，其中$c_n = \frac{1}{n}$。$\{c_n\}$趋于$0$。因此，对任意有理数$r>0$，由于$r_1 = \frac{r}{3} > 0$，总存在正整数$N_c$，使得$n>N$时总有$|c_n| < r_1$。另一方面，$\{a_n\}$是自敛数列，所以也存在正整数$N_a$，使得$n,m>N_a$时总有$|a_n - a_m| < r_1$。于是取正整数$N$为$N_c,N_a$中较大者，那么$n,m>N$时总有：
    \begin{align*}
...
\begin{proof}
    ...
    如果只有有限多个上峰，设其中最大的下标为$k_1$，那么按照定义，存在某个下标$k_2 > k_1$，使得$a_{k_2} \geqslant a_{k_1}$。而由于$a_{k_2}$也不是上峰，又可以找到某个下标$k_3 > k_2$，使得$a_{k_3} \geqslant a_{k_2}$。这个过程可以一直进行下去。所以我们得到了$\{a_n\}$的单调递增的子列。\\
...
\end{tm}
\begin{proof}
    根据定理\ref{tm:a-2-20}，有界数列总有单调子列。根据定理\ref{tm:a-2-10}，有界单调数列总有极限，所以该子列有极限。于是有界实数列总有收敛子列。
\end{proof}

从集合的角度来看，给定集合$S$与数$x$，如果对任意$r>0$，总有$y\in S$使得$|x - y| < r$，
就说$x$是$S$的\textbf{聚点}、\textbf{附点}或\textbf{极限点}。直观来看，这说明$S$中的元素聚拢到$x$，
或者说被$x$“吸附”，或者说离$x$任意近的地方都有$S$中元素，或者说$S$中有一列元素收敛到$x$。把有界实数列看作集合，那么它有收敛子列，就等于说
...
因此，直观上我们讨论的是一系列连环嵌套的闭区间。我们把这样的一系列区间称为\textbf{区间套}。
区间的长度越来越小，趋近于$0$，最后归于一点。这个点是闭区间套所有区间的公共点。闭区间套定理告诉我们，只要闭区间套长度收敛到$0$，那么这些区间恰有一个公共点。

\begin{tm}\textbf{有限覆盖定理}\label{tm:a-2-50}\\
    给定$S\subseteq\mathbb{R}$，设$J = \{I_k\}$是一系列区间的集合。
    如果$S$中任何元素$x$都属于$J$中某个区间$I_k$，就说$\boldsymbol{J}$\textbf{覆盖}$\boldsymbol{S}$，$\boldsymbol{S}$\textbf{被}$\boldsymbol{J}$\textbf{覆盖}。
    如果$J$的元素都是开区间，就说$J$是$S$的\textbf{开覆盖}。\\    
...
\begin{proof}
    为了好说话，我们引入以下说法：
    \begin{itemize}
        \item 如果$S\subseteq[\,a; b\,]$可以被$J$中有限个开区间覆盖，就说$S$有限可盖；
        \item 否则就说$S$有限不可盖。
    \end{itemize}
    根据覆盖的定义，我们有以下结论：
    \begin{itemize}
        \item 如果$x\in[\,a; b\,]$，那么按定义，$x$属于$J$中某个开区间。
        \item 如果集合$S_1$有限可盖，$S_2$有限可盖。设它们分别被分别被$n_1$、$n_2$个区间覆盖。那么，这$n_1 + n_2$个区间就能覆盖$S_1$、$S_2$的并集。因此，$S_1$、$S_2$的并集有限可盖。
    \end{itemize}

    下面用反证法加二分法证明，$[\,a; b\,]$有限可盖。

    反设$[\,a; b\,]$有限不可盖。把$[\,a; b\,]$二等分，分为区间$\left[\,a; \frac{a+b}{2}\,\right]$和$\left[\,\frac{a+b}{2}; b\,\right]$。
    则这两个区间至少有一个有限不可盖。
    不妨设$\left[\,a; \frac{a+b}{2}\,\right]$有限不可盖，将它记为$[\,a_1; b_1\,]$，$[\,a; b\,]$记为$[\,a_0; b_0\,]$。
    继续将$[\,a_1; b_1\,]$等分成两个闭区间，
    其中又至少有一个有限不可盖。如此操作，就得到一个长度趋于$0$的闭区间套$\{[\,a_k; b_k\,]\}_{k\in\mathbb{N}}$，每个都有限不可盖。

    根据闭区间套定理，这些闭区间有公共点$c\in[\,a; b\,]$。按定义，$c$属于$J$中某个开区间$(x; y)$。为了好说话，设$r$为$y-c$、$c-x$中较小者，则$(c-r; c+r)\subseteq (x; y)$。

    另一方面，由于闭区间套长度趋于$0$，我们总能找到某个有限不可盖的闭区间$c\in [\,a_n; b_n\,]$，使得
    $$ b_n - a_n < r.$$
...
\chapter{从极限到连续函数}
\section{函数与极限}
\begin{tm}{\textbf{有界集合必有确界}}\label{tm:b-0-0}\\
    有上（下）界的非空实数集合必有上（下）确界。
\end{tm}
\begin{proof}
    考虑非空实数集合$S$，设它有上界$M$。设$S$的上界集合为$S^{\text{上}}$，我们要证明$S$有上确界，即$S^{\text{上}}$有最小元素。

    考虑$S$中元素$a$。如果$S$中没有元素比$a$大，那么$a\in S^{\text{上}}$。比$a$小的数按定义不是$S$的上界，因此$a$就是$S$的上确界。

    如果$S$中有元素比$a$大，换句话说，闭区间$[\,a; M\,]$中有$S$的元素。接下来我们用二分法构造区间套，找出$S$的上确界。

    把集合元素想象成河水，我们要测量河的“水位”。把闭区间$[\,a; M\,]$分成两半。如果上半“有水”，那么“水位”在上半部分，否则在下半部分。比而照之，再把所在的部分分为两半……每步操作都取旧区间的一半，新得到的区间是旧区间的子区间，区间长度减半，因此区间长度趋于$0$。这个区间套的每个区间左端都不是集合的上界，右端都是集合的上界。左端构成递增数列，右端构成递减数列，它们的差趋于$0$。根据区间套定理，这个区间套最终归于一点$x$。

    由于区间套中每个区间右端都是集合$S$的上界，即都大于等于集合中任何元素，所以它的极限$x$也大于等于$S$中任何元素。因此，$x$是$S$的上界。
    
    另一方面，$S$的任何上界$y$都大于等于$S$中任何元素，因此，它大于等于区间套中每个区间的左端。也因此，根据定理\ref{tm:a-0-40}，它大于等于$x$。这说明$x$是最小的上界。这样我们就找到了$S$的上确界。
...
\begin{df}\textbf{实变函数在一点的极限}
    \mbox{}\\
    设$f$是实变函数，$a$是实数。如果任何在$f$定义域中的数列$\{a_n\}_{n\in\mathbb{Z}^+}$，只要它趋于$a$，$\{f(a_n)\}_{n\in\mathbb{Z}^+}$就趋于同一个极限$b$，那么就说$b$是函数$f$在$a$处的\textbf{极限}。或者说自变量趋于$a$时，$f$\textbf{趋于}或\textbf{收敛到}$b$，记作：
    $$  \lian{x \to a} f(x) = b. $$
\end{df}
函数的极限是连续性质的体现：
\begin{df}{\textbf{函数在一点连续}}\\
    若实变函数$f$在定义域中一点有极限，则称它在该处\textbf{连续}。若函数在集合$S$中每处都连续，则称它在$S$上\textbf{处处连续}或简称在$S$上连续。。
\end{df}
要注意的是，定义极限时，我们默认总有$f$定义域中的数列趋于$a$，因此$a$应该是$f$定义域的极限点。我们把这一点视为默认前提，但也有教材会要求明确写出。

为了更细致地分析函数在一点附近的性质，还需要定义：
\begin{df}\textbf{实变函数在一点的去心极限}
    \mbox{}\\
    设$f$是实变函数，$a$是实数。如果任何在$f$定义域中的数列$\{a_n\}_{n\in\mathbb{Z}^+}$，只要它的通项总不等于$a$且趋于$a$，$\{f(a_n)\}_{n\in\mathbb{Z}^+}$就趋于同一个极限$b$，那么就说$b$是函数$f$在$a$\textbf{附近的极限}，也叫做$f$在$a$处的\textbf{去心极限}，记作：
    $$  \lim_{x \to a} f(x) = b. $$
\end{df}
去心极限是极限的“弱化”版本，它仅仅讨论自变量在一点附近的性质。因此，函数在一点有去心极限，不一定有极限。一个简单的例子是函数在一点取值“脱线”的情况：把连续函数在一点的值改成别的值，它在该点附近仍然趋于“应该经过”的地方，但函数值已经不是这个值了。比如：
$$
f: \, x\, \mapsto \begin{cases}
    1 & \mbox{如果}x \neq 0 \\
    7 & \mbox{如果}x = 0
\end{cases} 
$$
$f$是把常函数$x\mapsto 1$在$0$处的值改为$7$的结果。显然$f$在$0$处没有极限，因为常数列$\{0\}$映射后的极限是$7$，与趋于$0$的非零数列映射后的极限不同。但按照去心极限的定义，$f$在$0$附近有极限$1$，和函数值不相等。去心极限也可以视为从函数定义域中去掉该点后讨论的极限。很多教材会把去心极限视为标准的极限定义，读者应当注意区分。

去心极限的定义自然延伸到去心左极限和去心右极限。设$f$是实变函数，$a$是实数。如果任何在$f$定义域中的数列$\{a_n\}_{n\in\mathbb{Z}^+}$，只要它的通项总小（大）于$a$且趋于$a$，$\{f(a_n)\}_{n\in\mathbb{Z}^+}$就趋于同一个极限$b$，那么就说$b$是函数$f$在$a$\textbf{左（右）旁的极限}，也叫做$f$在$a$处的\textbf{左（右）去心极限}。
左右去心极限分别记作：
$$  \lim_{x \to a^-} f(x) = b \quad \mbox{和} \quad \lim_{x \to a^+} f(x) = b. $$

为了便于讨论去心极限，我们引入去心邻区间的概念：设$d>0$，则称集合$(a-d;a)\cup(a;a+d)$为$a$的$d$\textbf{-去心邻区间}，简称\textbf{去心邻区间}，记作$\kon{a}{d}$。用去心邻区间可以这样定义去心极限：
\begin{df}{\textbf{实变函数在一点的去心极限}}\\
    设$f$是实变函数，$a, b$是实数。如果对任何$r>0$，都存在$d>0$，
    使得对$f$定义域与$a$的$d$-去心邻区间交集中的任意$x$，$f(x)$到$b$的距离小于$r$，
    就说$b$是函数$f$在$a$\textbf{附近的极限}或$a$处的\textbf{去心极限}。    
\end{df}
用去心极限还能定义去心连续和去心左（右）连续：
\begin{df}
    若实变函数$f$在$a$处有去心极限，就说$f$在$a$处\textbf{去心连续}。若$f$在$a$处有\textbf{左（右）去心极限}，就说$f$在$a$处\textbf{左（右）去心连续}。
\end{df}

如果不同函数在同一处的函数值总有相同的大小关系，那么它们的极限也有和数列极限类似的性质：
\begin{tm}{\textbf{同归定理}}\label{tm:b-0-20}
    设实变函数$f$、$g$、$h$在点$a$的（去心）邻区间$U$上有定义，且在$U$上总有$g \leqslant f \leqslant h $，即：
    $$ \forall \,\,x\in U,\;\,\, g(x) \leqslant f(x) \leqslant h(x).  $$
    那么，如果$g$、$h$在点$a$都有（去心）极限$l$，则$f$在点$a$也有（去心）极限$l$。
\end{tm}
\begin{proof}
    首先讨论去心版本：我们可以假设在$f$、$g$、$h$的定义域中“挖去”点$a$，这样在$a$处的去心极限就和极限一致。

    按照定义，对任意正数$r>0$，存在$d_g>0$，使得只要$x\in\kon{a}{d_g}$，就有$|g(x) - l|<r$。
    同理，有$d_h>0$，使得只要$x\in\kon{a}{d_h}$，就有$|h(x) - l|<r$。这说明，
    取$d$为$d_g$、$d_h$的较小者。只要$x\in\kon{a}{d}$，就有
    $$ l - r < g(x) \leqslant f(x) \leqslant h(x) < l + r. $$
    因此，$x$趋于$a$时，$f(x)$也趋于$l$。

    再讨论不去心的版本：如果$g$、$h$在点$a$都有极限$l$，那么$f$在$a$处的值只能是$l$。于是结论成立。
\end{proof}

要注意的是：去心版本的同归定理能够更灵活地处理具体问题，比不去心版本更“强力”。比如，函数$f$在$a$处的值如果改变，那么不去心版本的条件“在$U$上总有$g \leqslant f \leqslant h $”不再满足，但去心版本仍然可用。

和单调数列一样，单调函数也有更好的极限性质。我们知道，有界单调数列总收敛。对于函数来说，有界的单调函数也有类似的收敛性质。
\begin{tm}{\textbf{单调函数的极限}}\label{tm:b-0-30}\\
    只要条件允许，单调函数在定义域中任一点旁总有左极限和右极限。
\end{tm}
首先来理解一下这个表述。
考虑一个单调递增函数。如果函数在一点$a$及其左旁有定义，那么从左边靠近点$a$时，函数值不断增大，就和单调递增数列一样。
而函数在$a$处有定义，所以左旁的函数值总比它小。这就和有上界的单调递增数列的情况类似。可以证明，函数值总会趋于一个极限。
要注意的是，这个极限是去心极限，不一定是$f(a)$。$f(a)$只起到划出上界的作用。
\begin{proof}
    考虑单调递增函数$f$，设其定义域为$\mathcal{D}$。对$\mathcal{D}$中任何元素$a$，考虑$\mathcal{D}^{\text{左}} = \{x\in\mathcal{D} \, | \, x < a\}$。只要$\mathcal{D}^{\text{左}}$不是空集，考虑$f$在$\mathcal{D}^{\text{左}}$上的值域$I_a^{\text{左}}$。根据函数的单调性质，非空集合$I_a^{\text{左}}$有上界$f(a)$。因此，根据定理\ref{tm:b-0-0}，它有上确界$b$。根据上确界的定义，$b\leqslant f(a)$。
    下面证明$b$是$f$在$a$左旁的极限。

    根据$b$的定义，对任意$r>0$，$b - r$不是$I_a^{\text{左}}$的上界，因此总有$x_r\in D^{\text{左}}$使得$f(x_r)>b-r$。
    而$f$是单调函数，所以对任意$x\in(x_r;a)\cap\mathcal{D}$，总有：
    $$ b - r < f(x_r) \leqslant f(x) \leqslant b.$$
    这就说明$b$是$f$在$a$左旁的极限。 

    同理，考虑单调递增函数$f$在$a$右旁的表现，用类似的方法可以证明它在$a$右旁有极限。
    对于单调递减函数，也可以用类似的方法证明。
\end{proof}

考虑将$f_{\mathbb{Q}}:\,x\mapsto 2^x$延拓到实数域上的问题。我们可以把$f_{\mathbb{R}}$写成：
$$
    f_{\mathbb{R}} (x) = \left\{\begin{array}[]{cc}
        2^x & x\in\mathbb{Q} \\
        \lian{u \to x} 2^u & x \notin \mathbb{Q}
    \end{array}\right.
$$
这个定义是可行的。下面证明$f_{\mathbb{R}}$保存了$f_{\mathbb{Q}}$的良好性质。

\begin{tm}{\textbf{指数函数严格单调递增}}
    若有实数$a<b$，则$f_{\mathbb{R}} (a) < f_{\mathbb{R}}(b)$。
\end{tm}
\begin{proof}
    若$a,b\in\mathbb{Q}$，结论显然。否则，考虑$a<q_1<q_2<b$，$q\in\mathbb{Q}$。由定理\ref{tm:a-1-18}可知$q_1,q_2$存在。按定义，
    $$ f_{\mathbb{R}} (a) = \lian{u \to a} 2^u = \lian{n\to\infty} 2^{u_n}. $$
    其中$\{u_n\}$是任意趋于$a$的有理数列。考虑$q_1-a>0$，有正整数$N$使得$n>N$时总有
    $$ |u_n - a| < q_1 - a.$$
    这说明$n>N$时总有$u_n < q_1$，从而有$2^{u_n} < 2^{q_1}$。于是根据定理\ref{tm:a-0-40}，$f_{\mathbb{R}} (a)\leqslant 2^{q_1}$。

    同理可证，$f_{\mathbb{R}} (b)\geqslant 2^{q_2}$。因此，
    $$ f_{\mathbb{R}} (a)\leqslant 2^{q_1} < 2^{q_2} \leqslant f_{\mathbb{R}} (b). $$
\end{proof}

\begin{tm}{\textbf{指数函数的基本性质}}\\
    $ \forall a, b \in \mathbb{R}, \;\,\, f_{\mathbb{R}}(a + b) = f_{\mathbb{R}}(a) \cdot f_{\mathbb{R}}(b).$
\end{tm}

\begin{proof}
    按定义，无论$a,b$是有理数还是无理数，都有
    \begin{align*}
         f_{\mathbb{R}}(a) \cdot f_{\mathbb{R}}(b) &= \lian{u \to x} 2^u \cdot \lian{v \to x} 2^v \\
         &= \lian{n\to\infty} 2^{u_n} \cdot \lian{n\to\infty} 2^{v_n} \\
         &= \lian{n\to\infty} 2^{u_n+v_n}
    \end{align*}
    其中$\{u_n\}$是某个趋于$a$的有理数列，$\{v_n\}$是某个趋于$b$的有理数列。而$\{u_n+v_n\}$趋于$a+b$，因此
    $$f_{\mathbb{R}}(a) \cdot f_{\mathbb{R}}(b) = \lian{n\to\infty} 2^{u_n+v_n} = f_{\mathbb{R}}(a + b).$$
    注意在论证$f_{\mathbb{R}}$定义良好时已经论证了极限的唯一性。这里不需要再论证所有趋于$a+b$的有理数列都能写成$\{u_n+v_n\}$的形式了。
\end{proof}

最后证明$f_{\mathbb{R}}$的连续性质。设$a$为任意实数。由于$f_{\mathbb{R}}$单调递增，根据定理\ref{tm:b-0-20}，$f_{\mathbb{R}}$在$a$处有左极限$a^-$和右极限$a^+$。只需证明$a^-=a^+$。考虑数列$\left\{a-\frac{1}{2n}\right\}_{n\in \mathbb{Z}^+}$和$\left\{a+\frac{1}{2n}\right\}_{n\in \mathbb{Z}^+}$，它俩一个从左边趋近$a$，一个从右边趋近$a$，因此函数值分别趋于$a^-$和$a^+$。另一方面：
$$ \forall \, n\in \mathbb{Z}^+,\quad f_{\mathbb{R}} \left(a+\frac{1}{2n}\right) = f_{\mathbb{R}} \left(a - \frac{1}{2n}\right) \cdot f_{\mathbb{R}} \left(\frac{1}{n}\right) = f_{\mathbb{R}} \left(a - \frac{1}{2n}\right) \cdot  2^{\frac{1}{n}}.   $$
取极限即得：
$$ a^+ = a^- \cdot 1 = a^-.$$
这就说明$f_{\mathbb{R}}$在$a$处连续。
...
\begin{proof}
    ...
    $\left[\,a;\frac{a+b}{2}\,\right]$和$\left[\,\frac{a+b}{2}; b\,\right]$。$f$不能在这两个区间上都有上界，也就是说，至少在其中一个区间上无上界。

    不妨设$f$在$\left[\,a;\frac{a+b}{2}\,\right]$上无上界，我们把它记为$[\,a_1; b_1\,]$。
    ...
    $x$在$[\,a; b\,]$中，$f(x)$也是实数。$f$在$x$处连续，因此，对于正实数$1$，有$d>0$，使得$[\,x-d;x+d\,]$中任何数$u$都满足
    $$ |f(u) - f(x)| < 1.$$
...
\begin{proof}
    记$f$的值域为$\mathcal{V} = \{f(x) \, | \, x\in[\,a;b\,]\}$。根据定理\ref{tm:b-1-0}，
    ...
    由于$u\in[\,a;b\,]$，$f$在$u$处连续，因此，$\{f(u_{\phi(n)})\}$收敛到$f(u)$。但收敛数列的极限是唯一的。所以，$f(u) = v_{\text{大}}$。$u$就是我们要找的$u_{\text{大}}$。
...
\begin{proof}
    ...
    存在正整数$N$，使得$n>N$时总有$|f(a_n) - f(a)| < r$，于是$f(a_n) > f(a) - r = b$。矛盾！
    因此$f(a)\leqslant b$。

    同理可以证明第二个性质。
\end{proof}

可以注意到这个定理和定理\ref{tm:a-0-40}的相似之处。类似结论可以推广到同函数两点、不同函数同一点和不同函数两点的不等关系。
...
\begin{xt}
    ...
    \indent\;\;\,\, $f$是有界函数。\\
    \indent 3. 设$f$是在$\mathbb{R}$上处处连续的周期函数。我们希望证明：\\
    \indent\;\;\,\, 如果$f$不是常函数，那么$f$有最小正周期$T>0$。\\
