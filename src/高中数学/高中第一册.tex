...
\begin{figure}[h]
    ...
    \begin{scope}[shift={(0,0)}]
        ...
        \draw[fill=orange!50,opacity=0.7] (C1) -- ($(C1)+(-90+0.5*\ang:\cs*\R)$) -- ($(C1)+(-90+0.5*\ang:\cs*\R)+(-180+0.5*\ang:\cs*\R)$) -- ($(C1)+(-180+0.5*\ang:\cs*\R)$) -- cycle;
        ...
        \draw[thick,Red] (D1) -- (D2) (A1) -- (D1) (A2) -- (D2);
...
\begin{proof}
    ...
    $p_n > \frac{\pi}{a_n}$，所以$a_n$趋于$0$时，$p_n$趋于正无穷大，因而$\sin{\frac{\pi}{p_n}}$趋于$0$，$\frac{\sin{\frac{\pi}{p_n}}}{\frac{\pi}{p_n}}$趋于$1$。这已经接近要证明的结论了。下面我们用它构造$\frac{\sin{a_n}}{a_n}$的上下界，以使用同归定理。
    ...
    于是根据同归定理，$\displaystyle \lian{n\to\infty} \,\,\frac{\sin{a_n}}{a_n} = 1.$
\end{proof}
...
\end{df}
这种定义不需要讨论所有趋近点$a$的数列，而是从任意正数$r$出发，说明总能找到某个数$d$，满足相关条件。
...
$$ |a_1|\cdot|q|^{N-1} \leqslant r,$$
那么对所有比$N$大的整数$n$，$|a_n| = |a_1|\cdot|q|^{n-1}$都比$r$小。于是$|a_n|$趋于$0$。
