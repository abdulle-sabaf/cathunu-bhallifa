...
\begin{figure}[h]
    ...
    \begin{scope}[shift={(0,0)}]
        ...
        \def\r{0.15};
        % 坐标轴（带箭头）
        \draw[-Stealth] (\xmin, \ymin) -- (\xmax+0.6,\ymin);
        \draw[-Stealth] (0,\ymin) -- (0,\ymax);
        % \node at (-0.6, \ymax) {$\mbox{\footnotesize $a_n$}$};
        \node at (\xmax+1, \ymin) {$\mbox{\footnotesize $n$}$};
        % 作图          
        % 散点数据
        \def\points{
        ...
        \def\afterpoints{
          (17.40,4.82),(17.60,4.83),(17.80,4.84),(18.00,4.85),
          (18.20,4.86),(18.40,4.88),(18.60,4.89),(18.80,4.90),(19.00,4.91),
          (19.20,4.92),(19.40,4.93),(19.60,4.95),(19.80,4.96),(20.00,4.97)
        }

        % 刻度与数字
        \foreach \x in {2,4,6} {
            \draw (-0.1,\x) -- (0.1,\x); 
            \node[black] at (-0.35,\x) {$\mbox{\scriptsize $\x$}$};
        }
        ...
        \draw[CadetBlue,very thick] (0,6.4) -- (\xmax,6.4);
        \draw[RoyalPurple,dashed,very thick] (0,6.4) -- (\xmax,6.4);
        \draw[dashed, Red, thick] (0,4.8) -- (\xmax,4.8);
        \draw[dashed, OliveGreen!50!green, thick] (0,5.2) -- (\xmax,5.2);
        % 绘制散点
        \foreach \p in \points {
          \fill[white, draw=blue] \p circle (2pt);
        }
        \foreach \p in \afterpoints {
          \fill[Goldenrod, draw=Sepia] \p circle (2pt);
        }
        \fill[Red, draw=BrickRed] (17.20,4.80) circle (2.1pt); 
        \node[RoyalPurple] at (\xmax+0.6, 6.4) {$\mbox{\footnotesize $M$}$};
        \node[Magenta] at (-0.5, 0) {$\mbox{\footnotesize $a_1$}$};
        \node[Red] at (-0.5, 4.8) {$\mbox{\footnotesize $y_{i}$}$};
        \node[OliveGreen] at (\xmax+0.9, 5.2) {$\mbox{\footnotesize $y_{i+1}$}$};
    \end{scope}
    \end{tikzpicture}
\end{figure}
...
记$y_i=a_1 + i r_k$是“往$M$走”第$i$步达到的值。如果对某个$i$，$y_i$不是上界，而在第$i+1$步，$y_{i+1}=a_1 + (i+1) r_k$是上界。也就是说，有某一项$a_{N(k)}$大于等于$y_i$，
却小于$y_{i+1}$。由于数列单调递增，自$a_{N(k)}$开始的每一项都在$y_i$与$y_{i+1}$之间。这说明任两项的距离都小于$r_k$。
...
综上所述，单调递增数列要么趋于正无穷大，要么有极限。类似可以证明：单调递减数列要么趋于负无穷大，要么有极限。
...
其中$\{a_m\}_{m\in\mathbb{Z}^+}$单调严格递增，$\{b_m\}_{m\in\mathbb{Z}^+}$单调严格递减\footnote{$m\leqslant 2$时$a_m,b_m$并没有定义，但我们可以将其补成单调数列，比如规定$a_1=0,a_2=4,b_1=10,b_2=9$。}。并满足：
