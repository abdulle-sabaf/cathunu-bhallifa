...
为了理解数轴上的点集，我们引进了区间的概念。区间是一种基本的点集。为了研究曲线在一点附近的情况，我们引入了邻区间和邻域的概念。邻域可以用来方便地描述“附近”这件事。对于平面上乃至空间中的点集，我们也引入一些基本的概念，帮助我们描述定义在平面和空间上的映射。
...
\begin{align*}
    (a_1;b_1)\times(a_2;b_2)\times\cdots\times(a_n;b_n) &= \{(x_1, x_2, \cdots, x_n) \, | \, \forall i\in\hin{1}{n},\,\, a_i < x_i < b_i\} \\
    [a_1;b_1]\times[a_2;b_2]\times\cdots\times[a_n;b_n]\; &= \{(x_1, x_2, \cdots, x_n) \, | \, \forall i\in\hin{1}{n},\,\, a_i \leqslant x_i \leqslant b_i\} 
\end{align*}
...
\begin{df}{\textbf{赋长空间}}
    ...
    \begin{align*}
        \mathbb{V} \;&\rightarrow \;\mathbb{R}_0^+ \\
...
\begin{df}{\textbf{赋距空间}}
    设有集合$S$及映射$\eth$：
    \begin{align*}
        S\times S \;&\rightarrow \; \mathbb{R}_0^+ \\
        (x, y) \;&\mapsto \; \eth(x, y)
    \end{align*}
    满足以下条件：
    \begin{enumerate}
...
\begin{df}
    ...
    这样定义的内积也叫点积，写作$\mathbf{u}\cdot \mathbf{v}$。它对应的长度是：
...
\begin{figure}[h] 
    ...
    \begin{axis}[
        ...
        xlabel={$x$}, ylabel={$y$},
        xmin=-0.1, xmax=2,
        ymin=-0.6,ymax=9.8,        % [0, π/2] ≈ [0, 1.57]
        samples=200,          % 足够采样点（n 大时需更多）
        axis lines=middle,
        xtick = {pi/8, pi/4, 3*pi/8, pi/2},
        xticklabels = {},
        ytick = {2,4,6,8},
        yticklabels = {2,4,6,8},
        grid=both,
    ...
    \addplot[
                thick,
                domain=0:pi/2,
                color=converge_Blue!20, 
                smooth
            ] {sin(deg(x)) * (cos(deg(x)))};
    \foreach \n in {10,20,...,200} { 
        \pgfmathsetmacro{\x}{(\n - 10)/190}
        \pgfmathsetmacro{\y}{20 + 80*(\n - 10)/190}
        \pgfmathtruncatemacro{\xint}{100-100*\x}
        ...
        \edef\temp{
            \noexpand\addplot[
                thick,
                domain=0:pi/2,
                color=converge_Blue!\xint!converge_blue!\yint, 
                smooth
            ] {\n * sin(deg(x)) * (cos(deg(x)))^\n};
        }
        \temp
...
\begin{xt}
    ...
    \indent 2. 定义在$\mathbb{R}_0^+$上的函数列$\{f_n\}$的通项为：
...
\begin{proof}
    ...
    我们要证明：$J$是其任意元素的邻域。考虑$x\in J$，$\forall i\in\hin{1}{n}$，$x\in K_i$，因此$K_i$是它的邻域。
...
\begin{tm}{\textbf{实数集的经典结构}}\label{tm:4-1-0}
    在实数集$\mathbb{R}$上，考虑所有有界开区间的集合$\mathcal{K}$：
    $$ \mathcal{K} = \{ (a;b) \, | \, a, b \in \mathbb{R} \}$$
...
按闭包的定义，我们也可以说，集合$B$在$A$中稠密，就是说$A$的任一点都是$B$的极限点。
...
\begin{proof}
    若集合$B$在集合$A$中稠密，那么$A$中任一点都是$B$的极限点。
...
\begin{tm}\label{tm:b-1-20}
    如果$U_1,U_2,\cdots, U_n$是$\mathbb{R}$上的经典邻则的开集，那么\textbf{架积集合}：
...
\begin{df}{\textbf{函数的模}}
    考虑由定义在集合$S\subseteq\mathbb{R}$上的实函数构成的平直空间$\mathbb{V}$。定义从$\mathbb{V}$到非负实数集合$\mathbb{R}_0^+$的映射：
    \begin{align*}
        \mathbb{V} \; &\rightarrow  \; \mathbb{R}_0^+ \\
        f &\mapsto \|f\|
    \end{align*}
...
则：
$$ B_a = \{|a| u \, | \, u \in B\} \subseteq \mathbb{R}_0^+. $$
...
\begin{xt}
    \mbox{} \\
    \indent 1. 定义平直空间$\mathbb{V}$中的“距离”映射$d$为满足以下条件的$\mathbb{V}^2 \rightarrow \mathbb{R}_0^+$的映射：\\
...
如果幂级数的收敛半径为无穷大，那么$A_1 = A_2 = A_3 = A_4 = \mathbb{R}_0^+$。
