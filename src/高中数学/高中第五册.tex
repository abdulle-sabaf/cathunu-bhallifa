...
\begin{tm}\label{tm:1-1-10}
    ...
    \end{itemize}
\end{tm}
也就是说，经典空间中实变向量函数的极限、连续、可微性质，可以直接用各个作为分量的实函数的极限、连续、可微性质来描述。其中的极限、连续、可微性质，指的是经典长度意义下的极限、连续、可微性质，但对于等价的长度，比如$\ell_1$、$\ell_{\infty}$，仍然成立。

实变向量函数的微变运算是否与实函数的微变类似呢？我们不难验证以下结论：

两个实变向量函数的和（或差）在某处的微变，等于两者在该处微变的和（或差）：
$$ \forall f, g, \quad \partial (f \pm g)(t) = \partial f(t) \pm \partial g(t). $$

两个实变向量函数的乘积的微变为：
$$ \forall f, g, \quad \partial (f \cdot g)(t) = f(t) \cdot \partial g(t) + g(t) \cdot \partial f(t). $$

两个实变向量函数的商的微变为：
$$ \forall f, g, \quad \partial \left( \frac{f}{g}\right)(t) = \frac{g(t)\cdot \partial f(t) - f(t)\cdot \partial g(t)}{g^2(t)}. $$
和之前关于实函数除法的微变类似，这个式子成立的条件是$g(t)$的任何坐标分量都不为$0$。

实变向量函数$f$和实函数$g$的复合的微变为：
$$ \partial (f \circ g) (t) = \partial f(g(t)) \cdot \partial g(t). $$

这里只验证乘积和复合：
\begin{proof}
    设经典空间为$\mathbb{R}^n$，对实数$t$来说，函数$f,g$的函数值分别是：
    $$ f(t) = (f_1(t), f_2(t), \cdots, f_n(t)), \quad g(t) = (g_1(t), g_2(t), \cdots, g_n(t)). $$
    其中$f_1, f_2, \cdots, f_n$和$g_1, g_2, \cdots, g_n$分别是$f,g$的分量实函数。于是$(f \cdot g)(t)$定义为：
    $$ (f \cdot g)(t) = (f_1(t)g_1(t), f_2(t)g_2(t), \cdots, f_n(t)g_n(t)). $$
    因此，乘积$f\cdot g$的微变就是：
    \begin{align*}
        \partial (f \cdot g)(t) &= (\partial (f_1(t)g_1(t)), \partial (f_2(t)g_2(t)), \cdots, \partial (f_n(t)g_n(t))) \\
        ...
        &= f(t) \cdot \partial g(t) + g(t) \cdot \partial f(t).
    \end{align*}
    ...
    \begin{align*}
        ...
        &= \partial f(g(t)) \cdot \partial g(t)
    \end{align*}
...
实变复函数指的是自变量为实数、值为复数的映射。如果我们把复数看作$\mathbb{R}$上的平直空间，把复数看作向量，那么实变复函数也是一种实变向量函数。因此，我们可以讨论实变复函数作为实变向量函数的性质。

从平直空间的角度来看，复平面就是二维的实平直空间，复数的模就是经典长度$\ell_2$，因此，我们可以定义相应的内积。而这样定义的内积又和平面上的经典内积一样。于是，复数的幅角就对应经典内积产生的角度。
...
作为经典空间，复平面也是完备空间。因此，我们也可以定义实变复函数的极限和连续性质。
...
实变复函数的微变运算也可以参照实变向量函数的微变运算。实变复函数的和与差的微变，与实变向量函数一样。但实变复函数乘积和商的微变运算则需要验证，因为复函数值乘积的定义使用复数乘法。
...
可以看到，实变复函数的商的微变，计算方式与实函数一样。成立的条件是分母$g$在$t$处不为$0$。至此我们发现，实变复函数的微变的四则运算与实函数一致。我们还可以验证，实变复函数与实函数的复合函数的微变，计算方式与实函数也一致。这说明，实变复函数的微变的运算与实函数相同。
