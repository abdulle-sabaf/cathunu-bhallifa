...
也就是说，在这点附近的最佳近似直线是水平直线。函数在这点附近几乎是水平。

那么，函数怎样的情况下会变得水平呢？一种情况就是“掉头”，即函数从左到右逼近这一点时，
函数值不断变大，但过了这一点，就不断变小了。或者反过来，从左到右逼近这一点时，不断变小，
但过了这一点，就不断变大了。

也就是说，函数在这一点附近达到局部的极大值或极小值。
...
$$ f(x) = {\color{magenta} f(a) + k(x - a)} + {\color{ForestGreen} g(x)}. $$
...
这个定理可以通过多次使用微变零值定理和中值定理得到（见附录$B$），可以看作微分中值定理的增强版本。
下面通过实际例子来看它的应用。
...
\begin{df}{\textbf{无穷的阶}}
    ...
    \indent 就说$f$在$a$附近\textbf{受限于}$g$。记为：
    $$ f \oveq{a} \Olim{g}$$
    \indent 3. 如果$f$在$a$附近受限于$g$，而$g$也受限于$f$，就说两者等阶。\\
...
的和。而我们已经证明了这个余项函数受限于$h\mapsto h^{n+1}$，因此是$P_n$任一项的高阶无穷小。
...
\begin{sk}
    ...
    \indent 2. 你认为，我们已知的关系中，什么关系和函数“可忽略”、“受限于”的关系最像？给出你的理由。\\
...
不难验证，前面提及的$x\mapsto \sqrt{x}$和$x\mapsto x^2$分别在$[0,1]$严格上凸、严格下凸。
...
\begin{figure}[h]
    \centering
    \includegraphics[width=0.8\textwidth]{研究函数2.png}
\end{figure}
...
\begin{ex}
    ...
    % \indent 证明：$\mathbb{K}_3[x]$中的元素可以唯一表示成$\{1, x, x^2, x^3\}$的直组合。
...
\begin{tm}\label{tm:4-3-70}
    ...
    且$\mathbb{U} \cap \mathbb{W} = \{\mathbb{0}\}$
...
\begin{xt}
    ...
    \indent 1.2. 有限维空间的直无关向量族的元素个数不小于空间的维数。\\
...
\begin{df}{\textbf{级数}}
    ...
    称为数列$\{a_n\}$的\textbf{级数}，简记为$\sum a_n$。$a_n$称为\textbf{级数的通项}。
...
正项级数是最方便研究的级数。对一般级数的研究，通常会转化到正项级数。

正项级数的部分和数列是单调递增数列。我们知道，单调递增数列要么有极限，要么趋于正无穷。
这两种情况分别对应级数收敛、级数发散。

举例来说，级数$\sum 2^{-n}$是正项级数。它的部分和$H_n$为：
$$ H_n = 1 + \frac{1}{2} + \cdots + \frac{1}{2^n}.$$
用归纳法可以证明：$H_n = 2 - 2^{-n}$。因此，部分和数列$\{H_n\}$趋于$2$。
我们说级数$\sum 2^{-n}$收敛，级数和是$2$。

同理，级数$\sum r^{-n}$的部分和$H_n$为
$$ H_n = 1 + \frac{1}{r} + \cdots + \frac{1}{r^n} = \frac{1 - r^{n+1}}{1 - r}.$$
因此，$r>1$时，部分和数列$\{H_n\}$趋于$\frac{1}{1 - r}$，级数和是$\frac{1}{1 - r}$。

研究级数收敛，和研究数列收敛，有什么不同呢？实际上，级数是研究数列收敛的工具。
...
因此，研究数列是否收敛，就看它的差分数列。然后研究差分数列的级数是否有级数和。
...
因此，部分和$H_n$可以写为：
$$ H_n = 1 + \sum_{i=1}^n \frac{1}{(n+1)^2} < 1 + \sum_{i=1}^n \left(\frac{1}{n} - \frac{1}{n+1}\right) = 2 - \frac{1}{n+1} < 2. $$
部分和数列有界，因此收敛。
...
$t_1 = 1$时，需要更精确的分析，这里就不继续展开了。我们把以上的内容总结一下：
...
最简单的方法是考虑“绝对值”。我们级数的绝对值定义为给通项取绝对值得到的级数。
...
\begin{proof}
    ...
    如果后者收敛，那么对应的部分和数列是自敛数列。因此，对任意$r>0$，存在正整数$N$，
    只要$n>m>N$，就有
    $$ \sum_{i=m+1}^n |a_i| < r. $$
    因此
    $$ \left|\sum_{i=m+1}^n a_i \right| \leqslant \sum_{i=m+1}^n |a_i| < r. $$
    这说明级数$\sum a_n$的部分和数列是自敛数列。也就是说，级数$\sum a_n$收敛。
\end{proof}
...
考虑部分和数列的子列：$\{H_{2m}\}_{m\in\mathbb{N}}$和$\{H_{2m+1}\}_{m\in\mathbb{N}}$。
我们证明两者都趋于同一个极限$X$，因此部分和数列也趋于$X$。
...
以上的情形可以稍做推广：
\begin{tm}
    如果级数$\sum a_n$的通项正负交替，且绝对值单调递减，趋于$0$，那么级数收敛。
\end{tm}

\begin{sk}
    \mbox{} \\
    \indent 1. 如果级数的通项正负交替且趋于$0$，但绝对值不单调递减，级数是否也收敛？\\
    \indent 2. 如果级数$\sum a_n$的通项绝对值单调递减且趋于$0$，且有无穷多个正项和负项，是否收敛？正负项的排列满足怎样的条件时收敛？
    给出你的想法。
...
例如，物体受的力在一定时间内的效果，称为冲量。它是物体导致速度变化的因素。
设物体在$t$时刻受力为$F(t)$，那么，
...
\begin{proof}
    ...
    反之，如果$\partial f$在$I$上单调递增，那么任取$I$内三点$a < t < b$，根据微变中值定理定理，
    存在$a < c_1 < t < c_2 < b$，使得：
    ...
    反之，如果$\partial f$在$I$上严格单调递增，那么任取$I$内三点$a < t < b$，根据微变中值定理定理，
    存在$a < c_1 < t < c_2 < b$，使得：
...
\subsection{常见函数的局部展开}

\chapter{平直空间}

\section{基底与维数}

\begin{tm}\label{tm:c-1-20}
...
\begin{proof}
    ...
    即$\mathfrak{F}_{\mathbf{v}}^{(1)}$是直相关的。所以根据定理\ref{tm:c-1-20}，
    ...
    这时，通过和第一步同样的逻辑，我们知道$\mathfrak{F}_{\mathbf{v}}^{(j)}$是直相关的，所以根据定理\ref{tm:c-1-20}，
...
\begin{proof}
    ...
    根据定理\ref{tm:c-1-20}，可以从其中找出一个向量，把它从$\mathcal{F}_{\mathbf{v}}$移除后，
...
从上面的定理可以推出：
\begin{tm}\label{tm:c-1-50}
    任意有限生成空间都有基底。
\end{tm}

空间的任意生成集都可以缩减为它的基底。那么任意直无关的向量能否扩充为基底呢？
\begin{tm}\label{tm:c-1-60}
    可以通过适当地添加元素，把有限生成空间$\mathbb{V}$的每一组直无关的向量变成基底。
\end{tm}
...
\begin{proof}
    ...
    要么$\mathcal{F}_{\mathbf{v}}$中元素个数超过$n$，而这和定理\ref{tm:c-1-30}矛盾。

\end{proof}

我们甚至有更具体的结论：
\begin{tm}\label{tm:c-1-70}
    给定有限生成空间$\mathbb{V}$的子空间$\mathbb{U}$，存在另一个子空间$\mathbb{W}$，使得
    $$ \mathbb{V} = \mathbb{U} + \mathbb{W}.$$
    且$\mathbb{U} \cap \mathbb{W} = \{\mathbb{0}\}$
    $\mathbb{W}$称为子空间$\mathbb{U}$的\textbf{补空间}，一般记作$\mathbb{U}^c.$
...
关于子空间交集与和空间维数的定理：
\begin{tm}{\textbf{维数基本定理}}\label{tm:c-1-80}
    设$\mathbb{U}$和$\mathbb{W}$是有限维空间$\mathbb{V}$的两个子空间，则它们的维数满足以下关系：
    $$ \dim (\mathbb{U} + \mathbb{W}) = \dim \mathbb{U} + \dim \mathbb{W} - \dim (\mathbb{U} \cap \mathbb{W}).$$
\end{tm}

\begin{proof}
    记$\mathbb{T} = \mathbb{U} \cap \mathbb{W}$. 根据定理 \ref{tm:c-1-70}，
    ...
    \ref{tm:c-1-70}证明中的方式扩充为$\mathbb{U}$和$\mathbb{W}$时得到的向量族。
