...
已知被积函数，求积合函数，或某个区间上的积合，称为\textbf{求合}或\textbf{求积}。
...
为此，数学家发展了一些求积的转化技巧，把看起来显式不可积的被积函数转化为显式可积的函数。
...
$$
\begin{array}{ll}
    \mbox{{\bfseries 微变余项}} & \mbox{{\bfseries 积合余项}} \\
    & \\
    \displaystyle \frac{\partial^{n+1} f (c)}{(n+1)!}(x - a)^{n+1} & \displaystyle \int_a^x \frac{(x - t)^{n}}{n!} \partial^{n+1} f(t)\mathrm{d}t \\
\end{array}
$$
...
\begin{sk}
    ...
    \indent 2. 不连续的函数，能否定义积合？哪些可以？如何定义？\\
    \indent 3. 本节的构造过程是否说明，可以用阶梯函数逼近闭区间上的连续函数？
\end{sk}
