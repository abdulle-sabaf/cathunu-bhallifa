...
$$ \begin{bmatrix}
    x\\ y
\end{bmatrix}$$
也可以看成一个$2\times 1$矩阵（两行一列）。因此我们可以用矩阵记录向量经历直变换的过程：
$$ \begin{bmatrix}
    a_{11} & a_{12} \\ a_{21} & a_{22}
\end{bmatrix}\begin{bmatrix}
    u_1\\ u_2
...
可以验证，这样定义的映射是直变换。这说明，固定平面的基底时，
每个直变换恰好对应一个$2\times 2$矩阵。
...
\begin{xt}
    ...
    \indent 点的过程。\\
    \indent 2. 给定基底$B = (\mathbf{e}_1,\,\, \mathbf{e}_2)$和四个数$a_{11}$、$a_{12}$、$a_{21}$、$a_{22}$，补全映射$f$
    \indent 是直变换的证明。\\
    \indent 3. 写出以下直变换的矩阵；\\
    \indent 3.1. $f: \,\, (x,\,\, y) \mapsto (x + 2y,\,\, y - 3x)$ \\
    \indent 3.2. $f: \,\, f(\mathbf{e}_1) = -\mathbf{e}_1 - \mathbf{e}_2,\,\,\, f(\mathbf{e}_2) = \mathbf{e}_1 - \mathbf{e}_2$ \\
    \indent 3.3, $f: \,\, f(\mathbf{e}_1 + \mathbf{e}_2) = \mathbf{e}_1 - \mathbf{e}_2,\,\,\, f(\mathbf{e}_2 - \mathbf{e}_1) = \mathbf{e}_1 + \mathbf{e}_2$ \\
    \indent 4. 给定基底$B$，写出以下矩阵对应的点映射$f$，以及向量$\mathbf{v}$经过该映射的结果。\\
    \indent 4.1. $$
                [\,f\,]_B = 
                \begin{bmatrix}
                    5 & -1 \\ 0 & 2
...
\begin{proof}
    ...
    综上所述，$\mathrm{Ty}_{(\mathbf{a},\mathbf{b})}$是直变换。
\end{proof}
...
\begin{proof}
    ...
    综上所述，$\mathrm{Fs}_{(\mathbf{a},\mathbf{b})}$是直变换。
\end{proof}
...
\end{enumerate}
按照类似的方法，我们可以证明$s,t$取任何值的时候，$H_{(\mathbf{a},\mathbf{b})}^{s, t}$都是直变换：
\begin{proof}
    首先，对任意向量$\mathbf{u}$、$\mathbf{v}$，将其在基底$\mathbf{a}$、$\mathbf{b}$上分解：
    $$ \mathbf{u} = u_a \mathbf{a} + u_b \mathbf{b},\quad \mathbf{v} = v_a \mathbf{a} + v_b \mathbf{b}.$$
    于是，
    ...
    综上所述，$H_{(\mathbf{a},\mathbf{b})}^{s, t}$是直变换。
\end{proof}
...
\begin{sk}
    \mbox{} \\
    \indent 1. 零映射、恒等映射、位似映射、旋转映射、投影映射、反射映射，哪些是单射，哪些是满射，哪些是双射？\\
    \indent 2. 一个点映射是否可能既是旋转映射，又是反射映射？既是旋转映射，又是位似映射？反射映射，又是位似映射？\\
    \indent 3. 零映射、恒等映射、位似映射、旋转映射、投影映射、反射映射是否有逆映射？如果有的话，是什么样的映射？
\end{sk}
