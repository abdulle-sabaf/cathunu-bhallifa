...
\begin{sk}
    \mbox{} \\
    \indent 1. 映射$f$把平面上的点映射到平面上的点。如果$f$总把直线变成直线，$f$是否总能表示成直映射和平移映射的复合？\\
    \indent 2. 直映射在不同基底上的矩阵是否总是不一样？对于不同的基底，同一个矩阵是否总对应不同的直映射？
\end{sk}
...
\begin{xt}
    \mbox{} \\
    \indent 1. 证明：基底变换总是满射。直映射是基底变换，当且仅当它是满射。\\
    \indent 2. 设基变换的矩阵为：
    $$
    \begin{bmatrix}
        -2 & 1 \\ 1 & 2
    ...
    \indent 4. 设有平面上的直映射$f$。它在基底$B$上的矩阵为：
    $$
    \left[f\right]_{B} = 
    \begin{bmatrix}
        2 & 1 \\ 0 & -1
