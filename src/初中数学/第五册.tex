...
通过两个前提得出一个结论。换句话说，三段论包含三个判断，
每个判断中的主语和断语分别涉及一个概念。

\section{三段论的结构}
我们规定，三段论中一共涉及三个概念。每个概念分别在两个判断中各出现一次、例如：
\begin{quotation}
...
\begin{enumerate}
    ...
    \end{ex*}
    \item[2.] 中项至少周延一次。\\
    中项是连接小项和大项的“桥梁”。中项在两个前提里都不周延，那么两个前提中涉及的可能是中项不同的部分。这样中项就无法连接小项和大项了。 
    \begin{ex*}
        以下的三段论是错误的： \\
...
从这些规则，可以导出几个比较简单的判定标准。要注意这几个判定标准只是“一票否决”，
即便三段论符合这些标准，也不一定正确。
\begin{enumerate}
    \item 只有当两个前提中至少有一个全判断，结论才可以是全判断。
\end{enumerate}
