...
我们学习过推理。通过已经知道是真的判断，得知另一些判断的真假。这种思考称为\textbf{推理}。下面先来回顾一些基本的推理方法。
...
\begin{figure}[H] %this figure will be at the right
    ...
    \begin{scope}[shift={(0,0)}]
        ...
        \def\c{0.87};
        \coordinate (O) at (0,0);
        \coordinate (A) at (-2*\c,2*\c);
        \coordinate (B) at (2*\c,2*\c);
        \coordinate (C) at (0,-2.6*\c);
        \coordinate (D) at (-3*\c,-2.6*\c);
        \draw[color=BurntOrange,fill=Apricot] (O) circle (\R cm) node[black] {$\mbox{\footnotesize\textbf{吴孟达}}$};
        \draw[color=BurntOrange,fill=Apricot] (A) circle (\R cm) node[black] {$\mbox{\footnotesize\textbf{朱咪咪}}$};
        \draw[color=BurntOrange,fill=Apricot] (B) circle (\R cm) node[black] {$\mbox{\footnotesize\textbf{徐锦江}}$};
        \draw[color=BurntOrange,fill=Apricot] (C) circle (\R cm) node[black] {$\mbox{\footnotesize\textbf{吴京}}$};
        \draw[color=BurntOrange,fill=Apricot] (D) circle (\R cm) node[black] {$\mbox{\footnotesize\textbf{古天乐}}$};
        \draw[Stealth-Stealth] (-0.5*\c,0.5*\c) -- ($(A)+(0.5*\c,-0.5*\c)$);
        \node at (-1.9*\c, 0.9*\c) {$\mbox{\scriptsize 《百变星君》}$};
        \draw[Stealth-Stealth] (0.5*\c,0.5*\c) -- ($(B)+(-0.5*\c,-0.5*\c)$); 
        \node at (1.9*\c, 0.9*\c) {$\mbox{\scriptsize 《鹿鼎记》}$};
        \draw[Stealth-Stealth] (0,-\R) -- ($(C)+(0,\R)$) node[midway,right] {$\mbox{\scriptsize 《流浪地球》}$};
        \draw[Stealth-Stealth] ($(C)-(\R,0)$) -- ($(D)+(\R,0)$) node[midway,above] {$\mbox{\scriptsize \,《蜀山传》}$};
        \draw[Stealth-Stealth] ($(B)-(\R,0)$) -- ($(A)+(\R,0)$) node[midway,above] {$\mbox{\scriptsize \,《唐伯虎点秋香》}$};
        \draw[Stealth-Stealth] ($(A)+(-\R,0)$) -- ($(A)+(-2*\R,0)$) node[left] {$\cdots$};
        \draw[Stealth-Stealth] ($(B)+(\R,0)$) -- ($(B)+(2*\R,0)$) node[right] {$\cdots$};
        \draw[Stealth-Stealth] ($(C)+(\R,0)$) -- ($(C)+(2*\R,0)$) node[right] {$\cdots$};
        \draw[Stealth-Stealth] ($(D)+(-0.8*\R,0.6*\R)$) -- ($(D)+(-1.2*\c,0.9*\c)$) node[above left] {$\ddots$};
    \end{scope}
    \end{tikzpicture}
\end{figure}
...
要更进一步分析数据，可以通过\textbf{加权平均数}、\textbf{分位数}和\textbf{直方图}。

加权平均数是平均数的推广，指对每个样本的值乘以一个系数（称为\textbf{权重}）来描述它的“重要程度”，然后求和，最后除以权重之和。比如，我们想知道男生身高的平均数时，可以把每个男生的身高值乘以权重$1$，把女生的身高值乘以权重$0$，求和之后除以权重之和（也就是男生的人数），就得到加权平均数。
...
\begin{ex}
    ...
    这个映射称为\textbf{随机变量的方差}。比如，对以上掷骰子试验的随机变量$f$，它的期望为：
...
另一种表示随机变量分布的方法，是\textbf{概率质量函数}。
概率质量函数从随机变量的值出发描述它遵循的分布，把$X$中元素值映射到“随机变量等于该值”这一事件的概率。
比如，投掷骰子的随机变量$f$的分布，可以用以下的概率质量函数表示：
\begin{align*}
p_f \,\,\, : \,\,\,  \{10,20,30,40,50,60\} &\rightarrow \qquad\qquad\qquad [0; 1]  \\
  x\qquad\qquad &\mapsto \,\, \mathbb{P}(\{s \in \{1,2,3,4,5,6\} \,|\, f(s) = x\})  
\end{align*}

概率分布不同，随机变量$f$的概率质量函数也不同。
例如，投骰子的分布是均匀分布时，$p_f(10) = \frac{1}{6}$；如果$1$朝上的概率是$50\%$，
那么$p_f(10) = \frac{1}{2}$。

概率质量函数是另一种描述随机变量的方法，可以看作如何描述随机变量的值的分布，因此，我们也把概率质量函数叫做随机变量的值遵循的分布，或者随机变量值的分布。

使用概率质量函数，也可以表示随机变量的期望和方差。对$X$中每个元素$x$，考虑使$f(s) = x$的$s$。我们发现：
\begin{align*}
...
这是因为当$x$遍历$X$时，使$f(s) = x$的$s$也遍历了$S$，没有重复也没有遗漏\footnote{不至于混淆时，\(p_f(x)\)也写作\(\Pr{f=x}\)。}。

同理可以推出，随机变量$f$的方差可以写成：
...
一般来说，随机变量的期望是一种方便的描述预期结果的方法，和平均数很像。和平均数的区别在于，随机变量的期望和各个终态的概率有关。概率越大的终态，对期望的影响越大，反之亦然。如果把各个终态看作已知数据，那么这种描述数据的方法就是\textbf{加权平均数}。加权平均数是指对每个数据值乘以一个系数（称为权重）来描述它的“重要程度”，然后求和，最后除以权重之和。要注意区分的是：随机变量的期望和数据的加权平均数的区别在于，随机变量的期望是一种预估方法，根据终态的概率估计预期结果，而数据的加权平均是对已知数据的描述方法。虽然两者的计算方式相同，但它们的含义不同。

再来看方差的作用。在投骰子的例子里，考虑以下两种概率分布：
\begin{enumerate}
    \item $p_1 = p_6 = 40\%$，$p_2 = p_3 = p_4 = p_5 = 5\%$。
    \item $p_3 = p_4 = 40\%$，$p_1 = p_2 = p_5 = p_6 = 5\%$。
\end{enumerate}
两种情况下，随机变量的期望相同，都是$35$，但显然这两种分布有很大区别。而方差可以描述这两种分布的区别。
计算两种情况的方差，分别得到$525$和$85$。可以观察到，期望相同的情况下，分布的方差越大，结果远离期望值的概率越大，方差越小，结果远离期望值的概率越小。随机变量的方差和数据的方差计算方法相同，但它们的含义不同。随机变量的方差是对预期结果的描述方法，而数据的方差是对已知数据的描述方法。

通过期望和方差，我们可以对概率分布有粗略的了解。

\begin{sk}
    \mbox{} \\
    \indent 1. 概率映射和随机变量有什么不同？ \\
    \indent 2. 比较随机变量的期望和样本数据的平均数。它们有什么相似之处？有什么不同？\\
    \indent 3. 比较随机变量的方差和样本数据的方差。它们有什么相似之处？有什么不同？\\
    \indent 4. 随机变量的分布改变，它的期望一定改变吗？\\
    \indent 5. 随机变量的分布改变，它的方差一定改变吗？ \\
    \indent 6. 随机变量的期望和方差的不同写法有哪些要注意的地方？你觉得每种写法好在哪里？   
\end{sk}
...
\begin{xt}
    ...
    \indent 1.3. 写出该随机变量的概率质量函数。\\
    \indent 1.4. 写出该随机变量的期望和方差。\\
    \indent 2. 投掷一枚骰子，假设$1$点向上和$5$点向上的概率分别是$40\%$，其余点数向上的概率是$5\%$，
    如果以点数的$3$倍为随机变量，求它的期望和方差。\\
    \indent 3. $X$是$\mathbb{Q}$的子集。证明$S$上的$X$–随机变量$f$的方差也可以写成：
...
如果$B$的概率是$0$，那么讨论$B$发生时$A$的概率比较困难。这时我们暂时不定义条件概率$\mathbb{P}(A \, | \, B)$。
...
\begin{xt}
    ...
    $$\forall i\in\hin{1}{k}, \,\,\, \mathbb{P}(B_i \, | \, A) = \frac{\mathbb{P}(A \, | \, B_i) \mathbb{P}(B_i)}{\mathbb{P}(A \, | \, B_1) \mathbb{P}(B_1) + \mathbb{P}(A \, | \, B_2) \mathbb{P}(B_2) + \cdots + \mathbb{P}(A \, | \, B_k) \mathbb{P}(B_k)}.$$ 
...
\begin{sk}
    ...
    那么，独立重复实验次数足够多时，随机变量的方差能否用结果来估计呢？你有什么想法？
\end{sk}
