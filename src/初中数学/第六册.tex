...
\section{角度与长度}
根据平面的根本性质，任何向量都可以用两个不共线向量表示。接下来，我们仿照角度，给出两个向量之间的一种关系。
...
因此，$f_2$也是内积。我们把符合直观经验的内积$f$称为\textbf{经典内积}，一般称内积都默认指经典内积；
...
对照上面正弦的表达式，可以发现这个面积等于$x_By_A - x_Ay_B$。我们就把对应的映射称
$$ (A, B) \mapsto x_By_A - x_Ay_B. $$
为向量$A，B$的面积。向量的面积和内积，分别对应正弦和余弦。
