...
\begin{wrapfigure}[7]{r}{0.42\textwidth} %this figure will be at the right
    \vspace{-42pt}
    \flushright
    \includegraphics[width=0.4\textwidth]{旋转变换例1.png}
\end{wrapfigure}

如果将$P$按角$\alpha$旋转，那么与$x$轴正半轴张成的角应该从$\theta$变为$\theta + \alpha$。
也就是说，用角度来描述旋转运动，就是：$t \mapsto \theta + t\alpha$。回到具体位置上，
...
\end{figure}
如下图所示，一点$P$在极坐标系中的坐标称为极坐标，它由模长和辐角组成。模长就是点$P$到原点$O$的距离，
辐角是$x$轴正半轴到$\vv{OP}$所构成的角。比如，直角坐标系中坐标为$(1,\,\,1)$的点，在极坐标系中可以表示为
$(\sqrt{2},\,\, 45^\circ)$。
...
多面体相邻两面相交的部分总是线段，这些线段叫做\textbf{多面体的棱}。三条或以上的棱交会在共同的端点上。
这些点也是多面体相邻的几个面的共同点，称为\textbf{多面体的顶点}。

数一数，上图中的多面体有几条棱？有几个顶点？
...
让我们在平面上还原我们看到的立体事物。为什么图中的$A$显得远，$B$显得近？
...
叫做\textbf{作图法}，因为传统上我们用画图的方法来表现现实世界中的立体形状。大部分写实绘画的作图法中都会用到透视法，
让平面中表现的立体景象显得更真实。
...
一点透视指图中有一个灭点。观察一点透视的图画，你觉得图中哪些线是现实世界中的平行线？相比现实，它们有什么变化？

% 单点透视例子

两点透视指图中有两个灭点。观察两点透视的图画，你觉得图中哪些线是现实世界中的平行线？相比现实，它们有什么变化？

% 两点透视例子

三点透视指图中有三个灭点。观察三点透视的图画，你觉得图中哪些线是现实世界中的平行线？相比现实，它们有什么变化？
...
\begin{sk}
    ...
    \indent 2. 如何理解单点透视、两点透视、三点透视之间的区别和联系？为什么它们都可以反映真实的立体景象？\\
