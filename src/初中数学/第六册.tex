...
\begin{center}
    \begin{tabular}{ p{4em}<{\centering} !{\color{white} \vrule width 2pt} p{7em}<{\centering} !{\color{white} \vrule width 2pt} p{7em}<{\centering} }
        ...
        \noalign{{\color{white}\hrule height 2pt}} % \hline\hline
        \rowcolor{gl} $2005$-$01$ & $\,\,\,7.9$ & $10.3$ \\  
        \noalign{{\color{white}\hrule height 2pt}}% \hline
        \rowcolor{gd} $2005$-$02$ & $10.0$ & $27.5$ \\ 
        \noalign{{\color{white}\hrule height 2pt}}% \hline
...
从列表中难以看出数据之间的联系。如果把相关的数据按时间顺序排列，形成下图：
% 时间序列
\begin{figure}[H] %this figure will be at the right
    \vspace{8pt}
    \centering
...
图中将任务用圆盘表示，圆盘直径和任务预计用时成正比。用箭头把每个任务指向它的后继任务（必须完成前者才能开始后者）。
