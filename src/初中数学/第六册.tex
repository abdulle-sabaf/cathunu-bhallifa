...
考虑以下变换：
\begin{align}
    \mathrm{Ty}_U : \,\, \gamma\,\,&\rightarrow \,\, \gamma \notag \\
    P &\mapsto \,\, (P\,|\,U)U \notag
\end{align}
用一个例子来看这个变换的作用。给定向量$U(1,\,\,0)$，点$P(x_P, \,\, y_P)$经过变换，
变成$(x_P,\,\, 0)$。我们注意到，它把任何点变为和$U$共轴的向量。

直观上看，如果把$x$轴看作地面，那么$(x_P,\,\,0)$可以看作$(x_P,\,\,y_P)$在竖直阳光下的影子。
所以，我们把这样的变换称为（正）\textbf{投影变换}。指定一个方向，投影变换把点垂直投影到这个方向上，
把点集变为它在这个方向上的“影子”。用数学的语言来说，投影变换把向量变换为指定方向的分量。
我们一般把$U$称为投影变换的（单位）方向向量。

设$S$是标准单位正方形，$S$中任一点$(x,\,\,y)$变换后为$(x,\,\,\ 0)$。
因此，$S$变成从原点到$(1,\,\,0)$的线段。
这就是标准单位正方形在$(1,\,\,0)$方向的投影。

如果方向向量换成$\left(\frac{1}{2},\,\,\frac{\sqrt{3}}{2}\right)$，
那么$(x,\,\,y)$将变为$\left(\frac{1}{2}x + \frac{\sqrt{3}}{2}y\right)(\frac{1}{2},\,\,\frac{\sqrt{3}}{2})$。
也就是说，$S$变为从原点到$\left(\frac{\sqrt{3} + 1}{4},\,\,\frac{3 + \sqrt{3}}{4}\right)$的线段。
这就是标准单位正方形在$\left(\frac{1}{2},\,\,\frac{\sqrt{3}}{2}\right)$方向的投影。

我们学习过点和图形的轴对称。把图形变成它的轴对称图形，这样的变换叫做\textbf{反射变换}。

给定数轴$l$，设$U$是$l$的单位方向向量。设平面上的点$P$关于$l$的对称点是$Q$。
那么，$PQ\perp l$。记直线$PQ$与$l$交于$H$，则$H$是$P$到$l$的垂足。
$\vv{OH}$是$\vv{OP}$在$l$上的投影，而$\vv{PH}$和$\vv{QH}$是大小相等、方向相反的向量。
因此：
$$ \vv{OH} = \mathrm{Ty}_U(P) = (P\,|\,U)U $$

反射变换可以表示为：
\begin{align}
    \mathrm{Fs}_{U} : \,\, \gamma\,\,&\rightarrow \,\, \gamma \notag \\
    P &\mapsto \,\, 2(P\,|\,U)U - P \notag
\end{align}
其中$U$是对称轴所在直线的单位方向向量。
...
举例来说，设方向向量为$U\left(-\frac{\sqrt{2}}{2},\,\,\frac{\sqrt{2}}{2}\right)$，标准单位正方形经历变换$\mathrm{Fs}_U$，
点$(x,\,\,y)$变为$(-y,\,\,-x)$，即它关于直线$x+y=0$的对称点。于是$\mathrm{Fs}_U$将标准单位正方形变换为它关于$x+y=0$的对称图形，即
...
\begin{sk}
    ...
    \indent 3. 如果平面中任两点之间的距离经过某个变换后都不变，就说这个变换是\textbf{保距变换}。本节提到的变换中，哪些变换是保距变换？\\
    \indent 4. 如果平面中任意角经过某个变换后角度都不变，就说这个变换是\textbf{保角变换}。本节提到的变换中，哪些变换是保角变换？
\end{sk}
