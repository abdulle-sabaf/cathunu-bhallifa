...
\begin{ex}
    ...
    这个映射称为随机变量的方差。比如，对以上掷骰子试验的随机变量$f$，它的期望为：
...
使用概率密度函数，也可以表示随机变量的期望和方差。对$X$中每个元素$x$，考虑使$f(s) = x$的$s$。我们发现：
\begin{align}
...
\begin{sk}
    ...
    \indent 2. 比较随机变量的期望和样本数据的平均值。它们有什么相似之处？有什么不同？\\
    \indent 3. 随机变量分布改变，它的期望一定改变吗？\\
    \indent 4. 随机变量分布改变，它的方差一定改变吗？ \\
    \indent 5. 随机变量的期望和方差的不同写法有哪些要注意的地方？你觉得每种写法好在哪里？   
\end{sk}
...
\begin{xt}
    ...
    \indent 2. $X$是$\mathbb{Q}$的子集。证明$S$上的$X$-随机变量$f$的方差也可以写成：\\
...
% 条件概率表格
\begin{center}
    \begin{tabular}{| p{5em}<{\centering} | p{4em}<{\centering} | p{4em}<{\centering} |}
        \hline
                & 下雨 & 不下雨 \\ [0.5ex] 
        \hline
        台风靠近 & $90\%$ & $10\%$ \\ 
...
这时，如果要讨论该校某个学生的身高超过$170$厘米的概率，那么我们可以计算这$100$次实验中身高超过$170$厘米的次数，
除以总样本数$100$，作为我们的估计值。

为什么可以这么做呢？对于这$100$次独立重复实验，我们可以定义随机变量$X_1, X_2, \cdots, X_{100}$：
如果第$i$次实验中，学生身高超过$170$厘米，则$X_i$的值为$1$，否则为$0$。
这些随机变量相互独立，而且遵循同一个二项分布。
我们说这$100$个随机变量是\textbf{独立同分布}的，把这个二项分布称为关于事件“学生身高超过$170$厘米”的二项分布。
一般来说，如果随机变量把事件$A$中元素映射到$1$，把其他终态映射到$0$，就说这个随机变量遵循关于事件$A$的二项分布
，简记为$\mathbf{1}_A$。

对于这样的一组随机变量，我们有以下的定理：
\begin{tm}\textbf{大数定律}
    设随机变量$X_1, X_2, \cdots, X_{N}$相互独立，且都遵循关于事件$A$的二项分布，
    那么$N$越大，它们的平均值：
    $$m_N = \frac{1}{N}\left(X_1 + X_2 + \cdots + X_{N}\right)$$
    就越接近$A$的概率。
\end{tm}

大数定律的证明较为复杂。我们会在以后的学习中给出。

假如$100$个学生的体检结果中，$33$人身高超过$170$厘米，那么我们可以说，从这份体检结果来看，
随机抽选该校一个学生，身高超过$170$厘米的概率大致是$33\%$。我们把$33$称为事件的频数，
除以$100$后得到的$33\%$称为事件的频率。大数定律告诉我们，随着独立重复实验的样本数量增加，
事件的频率会逐渐接近事件的概率。
