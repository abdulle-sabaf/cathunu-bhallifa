...
比如，设$S$是以$(0,\,\,0)$、$(1,\,\,0)$、$(1,\,\,1)$、$(0,\,\,1)$为顶点的正方形。
定义变换
$$f:\,\, (\mathbf{s}, \,\,t) \mapsto \mathbf{s} + t(0.5, \,\, -0.5). $$
$f$表示正方形中每一点$\mathbf{s}$沿着方向向量$(0.5, \,\, -0.5)$直线运动，即按该向量平移。
比如点$(0.2,\,\,0.35)$就沿着该方向直线运动到$(0.7, \,\,-0.15)$。

另一方面，对给定的$t$，我们可以定义$S(t) = \{f(x,\,\,t) \, | \, x\in S\}$为变换的过程中，
$S$在$t$时刻所处的位置。上一个例子中，$S(0.6)$就是一个以$(0.3,\,\,-0.3)$、$(1.3,\,\,-0.3)$、
$(1.3,\,\,0.7)$、$(0.3,\,\,0.7)$为顶点的正方形。

从例子中可以看到，按向量$\mathbf{u}$的平移变换可以表示为：
\begin{align}
    f: \,\, S\times[0,\,\,1] \,\,&\rightarrow \mathbb{R}^2 \notag \\
    (\mathbf{s},\,\, t) \quad &\mapsto \mathbf{s} + t\mathbf{u}. \notag
\end{align}

另一种比较简单的变换叫做\textbf{位似变换}。\textbf{位似}是一种特殊的相似关系。
...
\begin{align}
    f: \,\, S\times[0,\,\,1] \,\,&\rightarrow \mathbb{R}^2 \notag \\
    (\mathbf{s},\,\, t) \quad &\mapsto \mathbf{u} + (1 - kt)(\mathbf{s} - \mathbf{u}) = (1 - kt)\mathbf{s} + kt\mathbf{u}. \notag
\end{align}
其中$\mathbf{u}$是位似中心，$k$是终点时的相似比。
从$S$中一点$\mathbf{s}$来看，向量$\mathbf{s} - \mathbf{u}$不断放缩，最终变成原来的$k$倍大小。
我们说$S$按照$\mathbf{u}$放缩，最终变成原来$k$倍大小。

我们学习过点和图形的轴对称。把图形变成它的轴对称图形，这样的变换叫做反射变换。
反射变换可以表示为：
\begin{align}
    f: \,\, S\times[0,\,\,1] \,\,&\rightarrow \mathbb{R}^2 \notag \\
    (\mathbf{s},\,\, t) \quad &\mapsto 2(\mathbf{s}\cdot\mathbf{u})\mathbf{u} - \mathbf{s}. \notag
\end{align}
其中$\mathbf{u}$是对称轴所在直线的方向向量，长度为$1$。
