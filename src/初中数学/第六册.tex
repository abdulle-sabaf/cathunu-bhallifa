...
我们知道，平面中的点的坐标是唯一的。从向量的角度来说，就是用基底$A, B$表示向量的方法只有一种。
如果向量$P$既能写成$P = s_1A + t_1B$，又能写成$P = s_2A + t_2B$，那么$s_1 = s_2$、$t_1 = t_2$。
我们说，作为基底的向量$A,B$是\textbf{自由}的或\textbf{线性无关}的：
\begin{df}
    ...
    称为这组向量（关于系数$t_1, t_2, \cdots, t_n$）的\textbf{线性组合}。$t_1, t_2, \cdots, t_n$称为线性组合的系数。
    能表示为$\mathbf{a}_1, \mathbf{a}_2, \cdots , \mathbf{a}_n$的线性组合的向量，称为这组向量的\textbf{生成向量}，
    所有生成向量的集合称为这组向量的\textbf{生成集}。

    如果一组向量的生成向量只能用唯一的方式表示为它们的线性组合，就说这组向量是\textbf{自由}的或\textbf{线性无关}的，
    否则就说这组向量是\textbf{有约束}的或\textbf{线性相关}的。
\end{df}
从这些定义出发，我们容易想到一些基本的性质。比如，一组向量的生成集总会包括这些向量自身，也总包括零向量。

从平面的根本性质，可以得到：
\begin{tm}{\textbf{平面基本定理}}\label{tm:0-0-20}
    平面中不共线的两个向量总是自由的，生成集是整个平面。多于两个向量如果不共线，总是有约束的。
\end{tm}
\begin{proof2}
    从平面的根本性质第二条可以直接得出：平面中不共线的两个向量，生成集是整个平面，称为平面的基底。
    下面证明平面的基底总是自由的。
    
    使用反证法来证明。
    反设某组基底$A,B$是有约束的，即有某个向量$P$可以用两种形式表示成$A,B$的线性组合：
    ...
    $A, B$共线，矛盾！因此原命题成立：平面的基底总是自由的。

    多于两个向量$P_1, P_2, \cdots , P_n$，如果不共线，那么至少有两个向量不共线。记为$P_1, P_2$，则$P_1, P_2$构成了平面的基底。
    其他向量都可以表示成$P_1, P_2$的线性组合。
    
    考虑用这组向量生成零向量。
    首先，令所有系数为$0$。零向量总能表示为关于系数$(0,0,\cdots,0)$的线性组合。
    其次，在其他向量里面选一个，比如$P_k$，$P_k$可以表示为$P_1, P_2$的线性组合：$P_k = sP_1 + tP_2$。
    于是令$P_1,P_2$的系数分别为$s,t$，$P_k$的系数为$-1$，其他向量系数为$0$。零向量表示为：
    $$ \mathbf{0} = s\cdot P_1 + t\cdot P_2 + (-1)\cdot P_k + \sum_{i\neq 1,2,k} 0\cdot P_i$$
    零向量表示方式不唯一，说明这组向量是有约束的。
\end{proof2}

从证明中可以看出，零向量的表示方式是判别向量自由与否的方便方法。这是因为生成集里面某向量表示不唯一，等于说零向量表示不唯一。
而我们总知道零向量的一种表示方式：所有系数为$0$。这样，我们只需要讨论另一种表示方式是否存在。我们也可以把这个思考方法总结成：

\begin{tm}{\textbf{自由向量组的判定}}\label{tm:0-0-25}
    自由的向量组只能用系数全为零的方式表示零向量。只能用系数全为零的方式表示零向量的向量组是自由的。
\end{tm}

平面基底是自由的，也就是说，任何向量表示成基底的线性组合，方法是唯一的。
使用恰当的基底，可以从更好的角度看问题。
...
\begin{so}
    ...
    $\vv{BA}$和$\vv{BC}$不共线，所以构成平面的基底。
    根据平面基本定理，这两种表示方式都可以归于$\vv{BA}$和$\vv{BC}$的线性组合，即归根结底是同一个方式。
...
\begin{wrapfigure}[8]{r}{0.32\textwidth} 
    \vspace{-16pt}
    \flushright
    \includegraphics[width=0.3\textwidth]{内积1.png}
\end{wrapfigure}
...
\begin{wrapfigure}[8]{r}{0.32\textwidth} 
    \vspace{-16pt}
    \flushright
    \includegraphics[width=0.3\textwidth]{内积2.png}
\end{wrapfigure}
