...
我们把$n$个互不相同的物品排成一列的方法数目称为$n$\textbf{排列数}，记作$\mathrm{P}_n$。
比如，编号为$1,2,3$的$3$个小球排成一列的方法数目就叫做“$3$排列数”，记作$\mathrm{P}_3$。
...
我们把它称为$\boldsymbol{n}$\textbf{选}$\boldsymbol{m}$\textbf{排列数}，记作$\mathrm{P}_n^m$。显然，$\mathrm{P}_n^n$就是$\mathrm{P}_n$。我们可以用阶乘来表示$\mathrm{P}_n^m$：
...
（因为也可以看作从$5$个位置里选$3$个放红球），记作$\mathrm{C}_5^3$或$\displaystyle {5 \choose 3}$。

如果最左边的球是红球，那么剩下的$4$个位置要放$2$个红球、$2$个白球。于是，一共有$\mathrm{C}_4^2$种方法。计算可知：
$$ \mathrm{C}_4^2 = \frac{4!}{2!\times 2!} = \frac{24}{2\times 2} = 6.$$
即一共有$6$种方法。因此最左边的球是红球的概率是：
$$ \mathbb{P}(\mbox{最左边的球是红球}) = \frac{\mathrm{C}_4^2}{\mathrm{C}_5^3} = \frac{6}{10} = \frac{3}{5}.$$

一般来说，$n$选$m$组合数也可以用阶乘计算：
$$ \mathrm{C}_n^m = \frac{n!}{m!(n-m)!} $$

不难发现，$\mathrm{C}_n^m$和$\mathrm{P}_n^m$有以下关系：
$$ \mathrm{P}_n^m = m! \cdot \mathrm{C}_n^m. $$
...
\begin{so}
    ...
    $$ \mathrm{C}_6^4 = \frac{6!}{4!(6 - 4)!} = \frac{6\times 5\times \cancel{4}\times \cancel{3}\times \cancel{2}\times \cancel{1}}{\cancel{4}\times \cancel{3}\times \cancel{2}\times \cancel{1} \times 2\times 1} = \frac{30}{2} = 15.$$
    一共有$15$种方法。
\end{so}
...
\begin{so}
    ...
    首先计算两队同组的分法数。这时需要在剩余$18$只球队中再选$8$只球队与甲、乙同组，其余为另一组。因此一共有$\mathrm{C}_{18}^{8}$种分法。

    再计算两队不同组的分法数。这时需要在剩余$18$只球队中再选$9$只球队与甲队同组，而其余的就和乙队同组。
    因此一共有$\mathrm{C}_{18}^{9}$种分法。

    于是同组概率为：
    \begin{align*}
        \mathbb{P}(\mbox{甲乙不同组}) &= \frac{\mathrm{C}_{18}^{9}}{\mathrm{C}_{18}^{9} + \mathrm{C}_{18}^{8}} = \frac{\frac{18!}{9!\cdot 9!}}{\frac{18!}{9!\cdot 9!} +  \frac{18!}{8!\cdot 10!}} \\
...
\begin{so}
    ...
    $$ \mathrm{C}_{9}^3 = \frac{9!}{3!(9 - 3)!} = \frac{9 \times 8\times 7}{3\times 2\times 1} = \frac{504}{6} = 84. $$
    一共有$84$种方法。
\end{so}
...
\begin{so}
    ...
    因此，所求概率为：
    \begin{align*}
        \mathbb{P}(10\mbox{枚都是良品}) &= \frac{\mathrm{C}_{90}^{10}}{\mathrm{C}_{100}^{10}} = \frac{\frac{90!}{80!\cdot 10!}}{\frac{100!}{90!\cdot 10!}} \\
