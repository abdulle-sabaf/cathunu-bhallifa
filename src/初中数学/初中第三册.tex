...
\begin{sk}\label{sk:0-2-0}
    \mbox{} \\
    \indent 1. 能不能定义一条直线到另一条直线的距离？如果能，怎么定义？如果不能，为什么？
\end{sk}
...
\begin{proof}
    ...
    后得到的是有理数$r$。那么，$r$经过这些与有理数的加减乘除的逆运算（不包括除以$0$），得到无理数$t$。但我们知道，有理数之间的加减乘除运算总得到有理数。这就造成了矛盾！\\
...
先来看最简单的无理数：平方根。给定两个正数$q$和$r$，它们的平方根的乘积满足：
\begin{align*}
...
\begin{et}
    ...
    $$
    \begin{array}{lll}
       1).\;\; \displaystyle  \sqrt{15}\cdot\sqrt{60} &  \phantom{00000000} & 2).\;\; \displaystyle \sqrt[3]{9}\cdot\sqrt[3]{24} \\[2em]
       3).\;\; \displaystyle \sqrt[3]{12}\cdot\sqrt{3} &  \phantom{00000000} & 4).\;\; \displaystyle \sqrt[4]{324}\cdot\sqrt{2}
    \end{array}
    $$
\end{et}
...
\begin{sk}
    \mbox{}\\
    \indent 1. 为什么例题中计算$\sqrt[4]{324}\cdot\sqrt{2}$没用到$8$次方根？\\
    \indent 2. 方根的加减有没有规律？两个$k$次方根的和的$k$次方，有何特征？
\end{sk}

\sectionext{二次方根的估计}
有理数开平方的结果，叫做平方根或\textbf{二次方根}。
只要有理数$q$大于等于$0$，就存在二次方根$\sqrt{q}$。按照二次方根的定义，
$$\forall q\in\mathbb{Q}_0^+, \quad \left(\sqrt{q}\right)^2 = q.$$
...
\begin{sk}\label{sk:3-2-0}
    \mbox{}\\
    \indent 1. 对大于$2$的整数$k$，怎么求$k$次方根？说说你的想法。\\
    \indent 2. 本节的估计法中，为什么需要取平均数？有没有别的方法更新上下限？
\end{sk}

\begin{xt}\label{xt:3-2-0}
    \mbox{}\\
    \indent 1. 以下的二次方根大约有多大（精确到两位小数）？\\
    \indent 1.1. $\sqrt{3},\quad \sqrt{5},\quad \sqrt{7}$.\\
    \indent 1.2. $\sqrt{1.6},\quad \sqrt{9.2}$.\\
    \indent 1.3. $\displaystyle\sqrt{\frac{5}{4}}, \quad \sqrt{\frac{7}{3}}$.\\
    \indent 1.4. $\sqrt{2} + 1.6,\quad \sqrt{12.5} - 1$.\\
    \indent 2. 计算并估计以下的二次方根的值（精确到两位小数）：\\
    \indent 2.1. $\sqrt{2.4} \cdot \sqrt{9.6}$\\
    \indent 2.2. $\displaystyle\frac{\sqrt{7} - 2}{5}, \quad \frac{4\sqrt{3} + 1}{5}$\\
    \indent 2.3. $\displaystyle\sqrt{\frac{9}{10}}, \quad\sqrt{\frac{1.78}{0.92}} + 1  $\\
    \indent 2.4. $\displaystyle\frac{2 - \sqrt{3}}{\sqrt{3} - 1}, \quad \frac{1 + \sqrt{5}}{1 + \sqrt{3}}$
\end{xt}
